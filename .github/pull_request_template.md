...
- [ ] If I changed `supabase/migrations/**`, the change set also updates the promotion log or the rollback playbook, and every migration it adds has an entry in both (CI's `check:migration-safety` fails otherwise).
