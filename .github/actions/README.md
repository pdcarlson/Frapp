...
| [`infisical-secrets`](./infisical-secrets/action.yml) | The credential preflight plus the `Infisical/secrets-action` injection for one environment. Optional `preserve-nonempty` restores named env vars this injection left empty (production backup `prod` inject only). 15 call sites across 6 workflows — the roster is `EXPECTED` in `scripts/ci/__tests__/infisical-secrets-action.test.mjs`. |
