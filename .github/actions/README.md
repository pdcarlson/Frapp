...
| [`download-migration-snapshot`](./download-migration-snapshot/action.yml) | Fetches the newest migration snapshot `migration-snapshot.yml` published from `main`, with `GITHUB_TOKEN` (the job needs `actions: read`), and exports its path as `MIGRATION_SNAPSHOT`. How the credential-free PR gates learn what each database has applied (#2518). 3 call sites in `migration-drift-gate.yml`. |
