...
- Agent VMs expose `INFISICAL_SERVICE_TOKEN` / `INFISICAL_PROJECT_ID` (not `INFISICAL_API_KEY`), and cloud sandboxes cannot reach Infisical at all ([#1279](https://github.com/pdcarlson/Frapp/issues/1279)) — there, use `.env.local` + `npx supabase status -o env` instead.
