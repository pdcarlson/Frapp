...
3. **Arm a durable self-wake** (`send_later`, ~30–60 min out) and re-arm it on every wake until the PR merges or closes. This is load-bearing, not a nicety: GitHub is reliably reachable only via the MCP, and only while the session is awake (shell REST access is session-dependent and can be proxy-blocked), so a scheduled check-in is the only guaranteed wake for the states the webhook misses. If `send_later` fails with `MCP error -32003 … requires approval`, **stop — do not chase it**: it's a platform-level dead end for in-session trigger tools on the cloud surface (even the owner approving the prompt gets converted to a denial; even `list_triggers` reads fail — settled 2026-08-08, details in AGENT_INFRA § "Wake coverage"). Say so once and fall back to the `CI wake` comments (below) plus the user; anything needing a schedule is a real Routine created in the UI.
