...
- **Giving Paul instructions to follow: one step at a time, wait for his confirmation, no filler.** Terse numbered steps with exact commands — no preamble, no restating the question, no narrating what you are about to do. A wall of text is not more thorough, it is harder to act on, and a long block of steps is read as one thing and executed as none. This governs **instructions he executes**, not analysis: findings, trade-offs and the reasoning behind a recommendation still get stated in full. Flag anything unverified in the steps rather than presenting a derived command as a tested one.
