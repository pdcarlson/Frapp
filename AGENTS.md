...
- Size every fan-out by the [`multi-agent`](.claude/skills/multi-agent/SKILL.md) skill, ultracode included. `/diff-review` is the only review allowed to be big; everything else stays under 10 agents per workflow, one agent covers several angles or claims, and every workflow agent sets `effort` (`medium` or `high`, never the `xhigh` it would inherit under ultracode).
