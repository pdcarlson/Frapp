...
- **Giving Paul instructions to follow: one step at a time, wait for the outcome, no filler.** Terse numbered steps with exact commands, exact setting and secret names (never values), and what he should see when it worked — no preamble, no restating the question, no narrating what you are about to do. A numbered wall of nine steps is read as one thing and half-done as none. Two limits. It governs **instructions he executes**, not analysis: findings, trade-offs and the reasoning behind a recommendation are still stated in full. And it needs someone on the other end — a **one-shot delivery that cannot wait** (an end-of-run report, a `[human]` issue body) still carries every step, because there is no next turn to continue in and the one-interruption-per-run budget in [`file-follow-up`](.claude/skills/file-follow-up/SKILL.md) is what governs there.
- **Verify a command against the tool, not against this repo.** Run its `--help`, or the version's own manifest, before handing him something to paste. A runbook here told three sessions in a row to run `npx eas env:create`, which resolves to no package and names a deprecated subcommand; nothing in CI executes a documented command, so a stale one stays stale. Say which claims you verified and which you derived.
- **A question for him goes last in the message.** Not above a status report he has to scroll back through. Where an audit or implementation also owes the "debt spotted" note below, that note comes second-to-last and the question is still the final line. In a one-shot report the ask is the last thing read, which is the same rule [`file-follow-up`](.claude/skills/file-follow-up/SKILL.md) already states for that case.
...
Follow-up that does not belong in the current PR: [`.claude/skills/file-follow-up/SKILL.md`](.claude/skills/file-follow-up/SKILL.md). Human-only blockers: file per that skill **and** ask it, per [§ Filing is necessary but not sufficient](.claude/skills/file-follow-up/SKILL.md#filing-is-necessary-but-not-sufficient--end-the-run-by-asking) — which of the two channels you use depends on the blocker and on who is there to answer. An issue is durable, not an interruption, and filing alone reaches no one.
...
**Cursor Cloud and Claude Code web** are independent first-class agent environments (ADR-16 amendment 9). Neither is primary. Laptop/local is the third path.

**Cursor Cloud:** [`.cursor/environment.json`](.cursor/environment.json) is the public contract. Wait for `.cloud-sandbox-up.done` (or stop on `.cloud-sandbox-up.failed`). Cursor-specific instructions: [below](#cursor-cloud-specific-instructions).

**Claude Code web:** `.claude/hooks/session-start.sh` launches `scripts/cloud-sandbox-up.sh` in the background at session start. Wait for `.cloud-sandbox-up.done`, then `npm run start:dev -w apps/api`. Claude-specific instructions: [below](#claude-code-web-sandbox).

Shared bringup, egress, and failure troubleshooting: [`docs/internal/environment/CLOUD_SANDBOX.md`](docs/internal/environment/CLOUD_SANDBOX.md). Do not delete `.claude/**` or the Cursor adapter; both trees are live.
...
All skills live under [`.claude/skills/`](.claude/skills/). Cursor Cloud loads that tree; Claude Code uses it natively. Keep one skill tree — do not copy into `.cursor/skills/`. `/next` on Cursor: [`.cursor/commands/next.md`](.cursor/commands/next.md). `/next` on Claude: [`.claude/commands/next.md`](.claude/commands/next.md). Skills:
...
- **Skills:** [`.claude/skills/`](.claude/skills/) — Cursor loads that tree. Do not copy into `.cursor/skills/`. `/next`: [`.cursor/commands/next.md`](.cursor/commands/next.md).
- **Scheduled agents:** Claude Code Routines remain a live path. Cursor Automations are paste-ready in [`ROUTINES.md`](docs/internal/ci-cd/ROUTINES.md); do not enable the same routine on both (they would double-file). Do not enable Hygiene Scan without a healthy repo-backed stack.
...
3. Anything that needs a standing schedule is a Claude Code Routine or a Cursor Automation in the UI — do not dual-run the same job on both. See [`ROUTINES.md`](docs/internal/ci-cd/ROUTINES.md).
...
Claude Code is a first-class Frapp agent environment (ADR-16 amendment 9). Cursor Cloud is the other, independent one — not a replacement.
...
- **Skills / `/next`:** [`.claude/skills/`](.claude/skills/) and [`.claude/commands/next.md`](.claude/commands/next.md).
- **Scheduled agents:** Claude Code Routines in the web UI. Do not dual-run the same routine as a Cursor Automation.
- Full config: [`CLOUD_SANDBOX.md`](docs/internal/environment/CLOUD_SANDBOX.md). Local-only `.env.local` and SWC notes: [`LOCAL_DEV.md`](docs/internal/environment/LOCAL_DEV.md).

Do not delete `.claude/**` or `scripts/cloud-sandbox-up.sh`. Cursor `start` shares that bringup script.
