---
description: Use when building or modifying NestJS API endpoints, services, repositories, DTOs, or the OpenAPI contract artifacts. Covers the layered architecture, the 9-step new-endpoint workflow, the auth/guard chain, Supabase repository conventions, database migration steps, and rate limiting.
paths:
  - apps/api/src/**
  - packages/api-sdk/**
  - packages/validation/**
allowed-tools: Bash(npm run *), Bash(npx supabase *), Bash(npx turbo *), Read, Edit, Write, Grep, Glob
---
...
- `CustomThrottlerGuard` is registered globally in `app.module.ts` (`APP_GUARD`) for rate limiting. Auth and chapter guards are **not** global: apply them per-controller or per-route with `@UseGuards()` and `@UseInterceptors()` in the correct order. The typical chain is: `@UseGuards(SupabaseAuthGuard, ChapterGuard)` at controller level, then `PermissionsGuard` with explicit `@RequirePermissions()` / `@RequireAnyOfPermissions()` on **every** route that returns or accesses protected chapter data — **including GET/list** (use class-level defaults where it keeps behavior consistent; route-level `@RequirePermissions` is **merged** with the class list by `PermissionsGuard`, so both apply). `AuthSyncInterceptor` is only used on user, invite, notification, and chapter-create controllers via `@UseInterceptors(AuthSyncInterceptor)`.
