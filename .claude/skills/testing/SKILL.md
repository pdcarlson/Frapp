...
**A controller spec that declares guards uses `createUnguardedTestingModule()`**
...
provide a stub `'SUPABASE_CLIENT'` so the real guards can construct. A controller with **no** guards
(`health`, `webhook`) needs none of this — and `health.controller.spec.ts` provides the real
`SUPABASE_CLIENT` because the *controller* injects it, which is correct. Full rule, including all
four exceptions: [`docs/guides/testing.md` § 4](../../../docs/guides/testing.md#4-guards-and-interceptors).
