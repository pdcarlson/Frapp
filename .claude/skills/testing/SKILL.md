...
only when the code under test injects it (`health.controller.spec.ts` does, under the exported
constant, because `HealthController` itself injects it). A spec that constructs its controller with
`new` needs none of this and should stay that way. Full rule and the four exceptions:
