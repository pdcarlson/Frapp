...
# Cloud sandbox only (FRAPP_CLOUD_SANDBOX=1): launch the local stack in the background so
# the session is never blocked on the ~60-90s bringup. A /tmp lock prevents relaunching on
# session resume. Local sessions skip this entirely. See docs/internal/ci-cd/CLOUD_SANDBOX.md.
if [ "${FRAPP_CLOUD_SANDBOX:-}" = "1" ] && [ -f "$ROOT/scripts/cloud-sandbox-up.sh" ]; then
