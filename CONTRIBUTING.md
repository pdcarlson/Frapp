...
- Code review is a **local pre-push gate**, not CI. **Cursor Cloud (primary):** [`.cursor/hooks.json`](.cursor/hooks.json) `beforeShellExecution` with `failClosed: true` wrapping `.claude/hooks/pre-push-review-gate.sh`. **Claude Code (fallback):** `.claude/hooks/pre-push-review-gate.sh` via `.claude/settings.json`. Both block
