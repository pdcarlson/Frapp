...
| `docs-spec-sync`     | Docs/spec sync on the PR diff **and** structure across the whole tree (`scripts/check-docs-impact.mjs` + `scripts/check-docs-structure.mjs`; no docs app build). Structure enforces the placement map as data (`scripts/ci/lib/docs-structure.mjs`), so it can fail on a file this PR never touched — see ‡. A change with genuinely no docs impact can be waived with the `no-doc-change-needed` label |
