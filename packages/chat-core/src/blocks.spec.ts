...
describe("hiddenQuoteText (#2312 §1)", () => {
  it("quotes a parent the viewer may see", () => {
    expect(hiddenQuoteText(one(restRow("p1", FRIEND)), ready(), VIEWER)).toBe(
      null,
    );
  });

  it("hides a blocked member's raw parent that arrived over the echo", () => {
    // The live path: the server never masked this row, so only the client's
    // own list stands between the quote and the blocked member's words.
    const parent = one(echoRow("p1", BLOCKED));
    expect(hiddenQuoteText(parent, ready([BLOCKED]), VIEWER)).toBe(
      TOMBSTONE_TEXT,
    );
  });

  it("says a parent is hidden while the list cannot vouch for it", () => {
    const parent = one(echoRow("p1", FRIEND));
    expect(hiddenQuoteText(parent, loading(), VIEWER)).toBe(HELD_QUOTE_TEXT);
    expect(hiddenQuoteText(parent, unavailable(), VIEWER)).toBe(
      HELD_QUOTE_TEXT,
    );
  });

  it("uses the stale words for a masked copy left over from an unblock", () => {
    const parent = one(
      restRow("p1", BLOCKED, {
        sender_blocked: true,
    ...
    expect(
      hiddenQuoteText(parent, ready([], { unblocked: [BLOCKED] }), VIEWER),
    ).toBe(TOMBSTONE_STALE_TEXT);
  });
});
