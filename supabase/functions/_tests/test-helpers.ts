...
//
// The production entrypoints export their request handler and only call
// `Deno.serve(handler)` when `import.meta.main` is true, so tests can
// `import { handler }` and invoke it directly with a synthetic `Request`
// without spawning a real server. Supabase is mocked at the `createClient`
// boundary via the test import map.
//
//   * `buildMockClient(script)` returns a chainable mock matching the
//     supabase-js surface the handlers actually use
...
//   * Every method invocation is logged in `mock.calls` so the test can
