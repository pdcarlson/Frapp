...
- The deeply nested inline `onChange` event handlers for the checkboxes in the members table (in `apps/web/app/(dashboard)/members/page.tsx`) have been extracted into named functions (`toggleAllVisibleSelection` and `toggleMemberSelection`) to improve maintainability and readability without altering UI functionality.
