...
This document is the canonical statement of the engineering correctness rules that every part of the Frapp codebase must honor. It exists because several recurring defect classes — hardcoded identities, unguarded lookups, shared mutable seeds, NaN-prone inputs, missing empty states, non-semantic interactives — are easy to introduce and expensive to find later. Each rule below names the failure mode and the required correction. Treat this as a checklist when writing or reviewing code, and as the source of truth when a feature spec is ambiguous about correctness.
