...
The two status rows are the reason this table exists. Both are routine, both were rendering one generic toast, and they need **opposite** next actions — one says fetch a new invite, the other says you already have what you came for. `spec/behavior/onboarding.md` §Invite Token Rules fixes the codes; the 410, 409 and fallback strings are shared verbatim with [`apps/mobile/lib/onboarding/join-errors.ts`](../../../apps/mobile/lib/onboarding/join-errors.ts) and [`apps/web/components/auth/join-errors.ts`](../../../apps/web/components/auth/join-errors.ts), and this table is where they are specified. The Terms rows are different: their strings live once in `@repo/validation`, beside the rule for when to ask (#2302), which both apps share through `@repo/hooks`.
...
| Sign-out threw (web) | — | `Couldn't sign out. Retry in a moment, or close this tab to end the session.` Rare: only a thrown failure reaches it, such as the browser failing to clear its stored session. A failed server-side logout doesn't, because `signOutCurrentSession` ignores the `{ error }` auth-js returns and the local session is cleared regardless, so the member is signed out on this device either way. The controls come back so the member can retry, and stay locked while a successful sign-out navigates away. |
