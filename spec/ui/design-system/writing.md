...
| **410** — the account itself was deleted, its session not yet ended | — | `This account has been deleted. Sign out to continue.` (`TERMS_PROMPT_COPY.deleted`). Told apart from the invite 410s by its message, `ACCOUNT_DELETED_MESSAGE`, since a new invite wouldn't help. |
| Any other failure | — | The server's message, else `Couldn't join that chapter. Check the invite and try again.` |

The two status rows are the reason this table exists. Both are routine, both were rendering one generic toast, and they need **opposite** next actions — one says fetch a new invite, the other says you already have what you came for. `spec/behavior/onboarding.md` §Invite Token Rules fixes the codes; both apps render every row through one function, `joinErrorCopy` in [`packages/hooks/src/join-errors.ts`](../../../packages/hooks/src/join-errors.ts), and this table is where the strings are specified. The Terms strings live in `@repo/validation` (`LEGAL_ACCEPTANCE_LABEL`, `JOIN_TERMS_REQUIRED_COPY`), and the rule for when to ask in `@repo/hooks` (`useJoinTermsCheckbox`, `isTermsRequiredError`), #2302.
...
| Deletion didn't finish (web) | — | `Deletion didn't finish. Part of it may already have gone through, and running it again is safe. Try once more in a moment.` `/profile`'s toast says the same, inline here because the prompt covers the page. |
