...
| Pinned panel, pins hidden (web) | — | `<N> pinned message(s) is/are hidden by your block list.` below any pins it may show, in place of "Nothing pinned yet" when every pin is hidden; the menu's Pinned count includes them | web `components/chat/pins-popover.tsx` (`hiddenPinsText`) |
