...
| Delivery (web) | `apps/web/lib/hooks/use-chapter-theme.ts` | Mounted once by `DashboardShell`, so branding applies shell-wide. Maps the persisted `--signet-accent-*` roles onto the semantic names the Signet stylesheet defines, via `signetAccentSemanticVars` — all-or-nothing: a row persisted before the Signet map existed lacks those keys, and the house-gold defaults baked into `packages/theme/src/signet.css` stand until a save or recompute refreshes it (#1165 tracks the backfill). No per-token client-side fallback runs — contrast is guaranteed at generation time (§8). **Nothing else is applied**, and the allow-list is why that is safe: every legacy token was composited over bone, and a row written before the cutover still holds them, so blind iteration would paint a light-calibrated value onto the dark surface. |
