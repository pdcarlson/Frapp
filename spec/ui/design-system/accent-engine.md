...
- It also guarantees the **`accent-primary` fill at 3:1** (WCAG 1.4.11 non-text) against every step of the neutral ladder — `--background`, `--surface-1`, `--card` and `--popover` ([`foundations.md`](foundations.md) §2) — because several consumers use the fill as the only cue for a state: the switch track, the active tab underline, the focus border, poll selection. The reasoning, and the rejected alternatives, are on [#2541](https://github.com/pdcarlson/Frapp/issues/2541).
  ...
  - **What it does not cover:** only step 9 is floored. Hover (step 10) is darker than step 9 on every scale, and pressed is hover with 8% black, so a fill that clears 3:1 by a small margin leaves both under it. On `--popover`, hover measures under 3:1 for 10 of the 19 seeds (the 9 lifted ones and `#003087`, whose unlifted fill clears at 3.31:1) and pressed for 12 (those, plus `#800000` and `#4B2E2E`). Crimson's hover measures 2.47:1. Whether to floor them too is [#2586](https://github.com/pdcarlson/Frapp/issues/2586).
