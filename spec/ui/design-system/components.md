...
- **TODO-DESIGN:** the pinned house-gold tints (`#251E0E` fill, `#6B5619` border, `#F4CB63` text) have no token name — they are the house-gold instance of the accent tint family, but this card holds them regardless of the chapter seed, so they cannot be spelled `accent-3/7/11`. Nearest pattern used: the Accent badge recipe (§5), with literal hexes standing in until the roles are named.
