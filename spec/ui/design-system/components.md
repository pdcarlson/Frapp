...
- **Focus (keyboard/input):** border swaps to `accent-9` plus a 3px ring of `accent-8` at ~25% opacity. Applies to every focusable control.
  - **The border swap is the half that carries it.** `accent-8` at 25% composites to 1.18–1.31:1 against the step it sits on — 0 of the 19 seeds × 4 steps clear the 3:1 non-text floor [README.md](README.md) §6 sets — so the ring is the halo and the solid border is the signal. A control that drops the border swap has no focus indicator, whatever the ring looks like.
    - **How strong that border is depends on the chapter, but it clears 3:1 for every palette the engine derives.** Solid `accent-9` against the step behind it ranges 3.78–18.71:1 across the 19 seeds. The engine holds it there by construction ([accent-engine.md](accent-engine.md) §8, since [#2541](https://github.com/pdcarlson/Frapp/issues/2541)); before that, 9 of 19 fell under 3:1 on `--popover` ([accent-engine.md](accent-engine.md) §4 covers palettes stored before it). The guarantee is pinned in `packages/chapter-theme/src/signet.spec.ts`; `focus-contrast.spec.ts` covers only the offset recipe.
...
| Semantic | `--*-tint` (the status color @ 13% over `--card`, opaque — [foundations.md](foundations.md) §5) | none | status color | Paid / Overdue / status only — never decorative |
...
- **The Semantic kind ships three hues, and only danger needs the lift.** Measured on its own tint, which is an opaque token and so one number on every surface (#2376): success **5.31:1** and warning **5.85:1** clear §6's 4.5:1 gate unlifted, so they render in the semantic hue itself; danger is **4.22:1**, which is what `--destructive-text` (5.61:1) exists for. Info would need one too (3.65:1 as a 13% alpha tint on `--popover`; it has no tint token) and deliberately has no kind, because it has no call site. The lift applies **on the tint only** — on a plain ladder surface the solid danger measures 4.72–5.79:1 and is the correct tone; reaching for the lifted one there over-applies §1. Measured in [`apps/web/components/billing/status-contrast.spec.ts`](../../../apps/web/components/billing/status-contrast.spec.ts).
