...
- **In-bubble mention chip.** `@Name` inside a message body takes a soft amber chip on the **handle alone**: fill `--mention-chip` `#4C3A1A`, text `--mention-chip-text` `#FAA81A`, radius 5, inline padding, weight 600. The bubble around it is **never** retinted — a message that mentions you is still the sender's message, and repainting it takes the one thing the bubble's fill already says. (This was a TODO-DESIGN: the reference draws mention red only as a list badge and a notification dot, and drew nothing for the in-body case.)
  ...
  - **The fill is opaque where §5's tint recipe would use 13% alpha, and the opacity is the requirement.** A mention lands inside a bubble, and the self bubble is `--primary` — per-chapter, spanning `#006400` to `#FFFFFF` across the seeded directory alone. An alpha tint composites over that, so the same chip would measure 6.5:1 in one chapter and about 1:1 in the next, leaving the message that addresses you unreadable in exactly the chapters whose accent happens to be pale. An opaque chip carries its own ground and measures identically on every bubble in every chapter. Pinned by `apps/web/components/chat/chat-contrast.spec.ts`.
  - **Which run gets the chip is the server's answer, not the renderer's.** Mentions resolve server-side from plain `@token` text ([chat/README.md](../../behavior/chat/README.md) § Mentions); the chip wraps the spans `packages/validation`'s tokenizer reports, so the UI cannot highlight a token the API never considered — or leave one plain that it did. The composer's authored mention takes the same chip, so a handle does not change colour on send.
