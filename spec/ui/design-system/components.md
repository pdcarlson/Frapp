...
- **Editing a message happens in the bubble's place, at the bubble's width.** No board draws the editing state, so this is derived rather than transcribed. The editor replaces the bubble's *body* inside the same message block, between the same reply quote and attachment list — never in a dialog, popover or floating card: the web board draws Edit as a chip in the in-row hover cluster, and §9's web dialog is sanctioned for destructive confirmation only. **The block stops hugging while the editor is open and fills the 86% instead**, so the editor is never narrower than the bubble it replaced: at the cap it is the same width, and below the cap it opens to the width the message is about to be allowed to grow into. Holding the hug is what this rule exists to forbid, because it does not merely keep the bubble's width — a `<textarea>` in a shrink-to-fit block sizes that block from its own ~20-character default instead of from the message, so every message, long or short, got the same unreadable box (measured in Chromium: a 740px bubble became a 249px editor). The fill is scoped to the editing state, because a block that fills permanently would leave the action cluster's leftover track nothing to measure and strand the cluster at the lane edge — the rule below, undone from the other side. The editor takes the locked bubble radius and the sender's tail corner, and it grows with the draft rather than scrolling a fixed box. It paints in the **incoming** bubble's neutral card fill and hairline even on a self message: a draft is not yet a message, and a field on `accent-9` would have to re-solve a contrast pair the bubble gets from the accent engine for free.
