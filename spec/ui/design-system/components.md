...
- **The Semantic kind ships three hues, and only danger needs the lift.** Measured on its own 13% tint across the whole surface ladder: success **4.79–6.08:1** and warning **5.27–6.81:1** both clear §6's 4.5:1 gate unlifted, so they render in the semantic hue itself; danger is **3.82–4.85:1**, under the gate from `--surface-1` up, which is what `--destructive-text` (**5.07–6.45:1**) exists for. Where `color-mix` is missing the tint drops out and each label sits on the bare step, where all three still clear (foundations.md §5 has the mechanism). Info would need a lift too (3.45:1 on `--popover`) and deliberately has no kind, because it has no call site. The lift is about the tint: on a plain ladder surface the solid danger measures 4.95–5.58:1 up to `--card` and is the correct tone there, but 4.48:1 on `--popover`, so danger text in a dialog takes the lift even untinted. Measured in [`apps/web/components/billing/status-contrast.spec.ts`](../../../apps/web/components/billing/status-contrast.spec.ts) and, for both tint paths, [`apps/web/components/shared/status-tint-contrast.spec.ts`](../../../apps/web/components/shared/status-tint-contrast.spec.ts).
