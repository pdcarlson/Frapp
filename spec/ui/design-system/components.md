...
- **Focus (keyboard/input):** border swaps to `accent-9` plus a 3px ring of `accent-8` at ~25% opacity. Applies to every focusable control.
  - **The border swap is the half that carries it.** `accent-8` at 25% composites to 1.18–1.31:1 against the step it sits on — 0 of the 19 seeds × 4 steps clear the 3:1 non-text floor [README.md](README.md) §6 sets — so the ring is the halo and the solid border is the signal. A control that drops the border swap has no focus indicator, whatever the ring looks like.
    - **How strong that border is depends on the chapter, but it clears 3:1 for every palette the engine derives.** Solid `accent-9` against the step behind it ranges 3.01–18.71:1 across the 19 seeds. The engine holds it there by construction ([accent-engine.md](accent-engine.md) §8, since [#2541](https://github.com/pdcarlson/Frapp/issues/2541)); before that, 9 of 19 fell under 3:1 on `--popover`, and a chapter whose stored palette predates it paints the old border until that palette is recomputed ([accent-engine.md](accent-engine.md) §4, [#1165](https://github.com/pdcarlson/Frapp/issues/1165)). The guarantee is pinned in `packages/chapter-theme/src/signet.spec.ts`; `focus-contrast.spec.ts` covers only the offset recipe.
  ...
  - **The two recipes use different accent steps, and the reason is which half carries the indicator.** The bordered recipe dilutes its ring to 25% because the solid `accent-9` border is the signal. The offset recipe has no border to swap — that is why a control lands on it — so its ring is the *entire* indicator and must clear the 3:1 floor unaided. Measured against `--background` across all 19 seeded chapter accents:
    ...
    | --- | --- | --- |
    | `accent-9` | 3.74:1 (`#CC0000`) | yes, but only just: the engine lifts a dark fill only to its 3:1 floor, so it measures 3.01:1 on `--popover` |
...
- **No component references the chapter hex.** Every accent role resolves through the generated, gated scale ([accent-engine.md](accent-engine.md) §1).
...
- **In-bubble mention chip.** `@Name` inside a message body takes a soft amber chip on the **handle alone**: fill `--mention-chip` `#4C3A1A`, text `--mention-chip-text` `#FAA81A`, radius 5, inline padding, weight 600. The bubble around it is **never** retinted — a message that mentions you is still the sender's message, and repainting it takes the one thing the bubble's fill already says. (This was a TODO-DESIGN: the reference draws mention red only as a list badge and a notification dot, and drew nothing for the in-body case.)
  ...
  - **The fill is opaque where §5's tint recipe would use 13% alpha, and the opacity is the requirement.** A mention lands inside a bubble, and the self bubble is `--primary` — per-chapter, spanning `#006400` to `#FFFFFF` across the seeded directory alone. The opaque pair measures **5.54:1** on every one of them. The same hue as a 13% tint would instead composite over the bubble, and then its own text measures **2.40:1 at best and 1.03:1 at worst** across the seed corpus — the message that addresses you unreadable in exactly the chapters whose accent happens to be pale. Every figure here is asserted in `apps/web/components/chat/chat-contrast.spec.ts`, including the alpha version, so none of them can drift out of this paragraph silently.
  - **Which run gets the chip is the server's answer, not the renderer's — and it is answered against the server's string.** Mentions resolve server-side from plain `@token` text in the stored body ([chat/README.md](../../behavior/chat/README.md) § Mentions). The chip runs `packages/validation`'s tokenizer over **that raw body** to decide *whether* a handle was addressed, and over the rendered text only to decide *where* the chip sits. The two are different strings — CommonMark decodes character references before a renderer sees them — and the gap is forgeable: `&#64;PresidentJane` carries no literal `@`, so the API notifies nobody, while a renderer tokenizing its own decoded text would mark her as addressed. Any member can type that, in the one UI whose job is to say who was pinged. The composer's authored mention takes the same chip, so a handle does not change colour on send.
  - **Open for the Design bar:** the chip's amber sits in the same family as the pinned house gold of the ✦ Ask card below. They are separated by context and geometry today (Ask chips are 27–28px controls in a card's source row; this is an inline run inside a sentence) and by hue (`#FAA81A` against `#EFB63B`), and the Ask tokens are untouched — but if the two ever read as the same thing, this is the one to move.
- **Editing a message happens in the bubble's place, at the bubble's width.** No board draws the editing state, so this is derived rather than transcribed. The editor replaces the bubble's *body* inside the same message block, between the same reply quote and attachment list — not in a dialog, popover or floating card, because none of those is *in place*: the web board draws Edit as a chip in the in-row hover cluster ([web-framework.dc.html](../web-greenfield/reference/web-framework.dc.html) pin 12) and draws no separate edit surface anywhere. (§9 is not the argument here. It bans `window.confirm` and requires destructive confirmation to be a dialog; it does not reserve dialogs for that, and several non-destructive ones ship.)
  - **The block stops hugging while the editor is open and fills the 86% instead**, so the editor is never narrower than the bubble it replaced: at the cap it is the same width, and below the cap it opens to the width the message is about to be allowed to grow into. Holding the hug is what this rule forbids, and the reason is counterintuitive enough to write down — holding it does not preserve the bubble's width, it destroys it. A `<textarea>` makes the block's width circular, so the block falls back to sizing itself from the field's own ~20-character default instead of from the message, and every message, long or short, gets the same unreadable box. Verified once by rendering the row into Chromium on **2026-09-14** against this app's compiled Tailwind: on an 860px thread column a bubble at the cap measured 740px and its editor 249px, the 249 unchanged from a 2-character message to a 2,000-character one. Both figures scale with the thread column and neither is re-measured by anything; the invariant is "the same box at every length", not the pixel values.
  - The fill is scoped to the editing state. A block that filled permanently would look identical — every child is shrink-wrapped by the column's `items-end` — but it would leave the action cluster's leftover track nothing to measure, stranding the cluster at the lane edge on every short message: the rule below, undone from the other side.
  - The editor takes the locked bubble radius and the sender's tail corner, and paints in the **incoming** bubble's neutral card fill and hairline even on a self message — a draft is not yet a message, and a field on `accent-9` would have to re-solve a contrast pair the bubble gets from the accent engine for free. It draws the same 16/25 body type in the same text column as the bubble — to within the two hairlines a bubble does not pay — so a draft wraps where the message wrapped rather than reflowing on the way in.
  - **Height is a progressive enhancement; width is not.** Where the engine supports `field-sizing`, the field grows with the draft to the composer's cap and scrolls past it; where it does not, it keeps a fixed minimum and the member scrolls a shorter box. The cap is an absolute length, not a fraction of the viewport: the editor sits inside the timeline's scroller, which is always shorter than the viewport, so a `vh` cap can push Save and Cancel below the fold on a short window. The width has no such fallback and needs none.
  - **It does not yet chip `@mentions`**, because it is a plain field and the composer is a rich editor — so a handle *does* change colour on edit, against the rule the mention-chip bullet above sets for send. Known gap, tracked in [#2236](https://github.com/pdcarlson/Frapp/issues/2236); the stored body is unaffected either way, since both paths send plain text.

- **The per-message action cluster attaches to the message, not to the lane.** Its outer edge sits at the edge of the message block on the side away from the bubble's tail — left of a self bubble, right of an incoming one — at every message width, not only at the 86% maximum. It used to be positioned against the row, which spans the whole thread column, so on any shorter message a right-aligned self bubble got its cluster stranded against the far left of the lane with nothing under it. When a near-max-width bubble leaves no room beside it, the cluster falls back to the lane edge and overlaps the bubble rather than overflowing the pane — which is why it carries an opaque fill. It reserves no height in either case.
- **Per-message actions reach a coarse pointer by tap, not by drawing a second geometry.** The quick-reaction cluster and Reply are `:hover`/`:focus-within`-revealed, which a touch device can never trigger, and the reference draws no touch affordance for this (#1193). Mobile is not a precedent to follow here either: its reactions render always-visible, ungated (`apps/mobile/components/chat/message-bubble.tsx`'s `ReactionRow`), and it has no Reply *authoring* yet (`spec/ui/mobile/screens.md` s05) — a web-authored reply does render its quoted parent on mobile, which is the read-side half, not a Reply control — so there is no cross-platform pattern to mirror for the *cluster*, and the web treatment below is this surface's own answer, not a port of one. Tap-to-reveal: tapping a row toggles that row's cluster, and tapping a second row closes the first's — one reveal id per list, never a `useState` per row, or two rows could sit open at once. The shared hook is `apps/web/hooks/use-tap-revealed-message.ts`; its one caller today is `apps/web/components/chat/message-timeline.tsx` (it had two until #2142 deleted the Details rail's `thread-panel.tsx`), and it stays a hook precisely so the second list cannot get this subtly wrong. The toggle is a plain `onClick`, not a touch/press handler, so a scroll gesture cannot trigger it; it bails out on a click whose target is inside a `button`/`a`/form control (covers every interactive descendant — the reaction chips, the emoji-picker trigger, a card's own Vote/RSVP/checkbox controls — without each one needing its own `stopPropagation`), and it checks `window.getSelection()` scoped to the row's own subtree so finishing a text selection inside the bubble does not also flip the cluster open, and a stale selection left over from a *different* message does not block this row's tap (`apps/web/components/chat/message-item.tsx`).
...
- **The obvious fix was also wrong, and only the second relationship shows it.** A meter has two: fill-against-track and track-against-container. The chat slice shipped `bg-input`, which is a real 1.540:1 groove against `--card` — and collided with the fill at **1.017:1 under `#800000`**, because a white wash at 14% landed almost exactly where a maroon `accent-9` did. A chapter branded maroon shipped a bar whose fill was invisible against its own groove. The engine's fill floor ([accent-engine.md](accent-engine.md) §8) has since ended that collision, and the ordering held. Worst-case fill-against-track by candidate, measured after [#2541](https://github.com/pdcarlson/Frapp/issues/2541): `bg-input` 2.134, `bg-border` 2.625, `bg-popover` 3.005, accent tint 3.027, **`--background` 3.742**.
- **The bar is never the only signal.** The fill now clears [README.md](README.md) §6's 3:1 non-text floor against the track for every seed, but a bar's length cannot carry the exact figure. The count and percentage therefore render as text beside the bar, and the bar is `aria-hidden`. Removing that text on the grounds that the bar shows it is a defect.
