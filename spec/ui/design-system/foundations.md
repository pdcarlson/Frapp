...
- **Scope:** these tokens govern **Signet surfaces** — the mobile app and the web dashboard (`apps/web` ships them via `packages/theme/src/signet.css` since the #920 shell cutover), plus the landing site. The landing's token cutover has **merged** ([#2366](https://github.com/pdcarlson/Frapp/issues/2366), a slice of [#2364](https://github.com/pdcarlson/Frapp/issues/2364)): `apps/landing` imports `packages/theme/src/signet.css` and the legacy bone/bronze/ink tokens are off every shipping surface. The composition followed in slice 2 ([#2367](https://github.com/pdcarlson/Frapp/issues/2367)), so what is left of that epic is polish ([#2368](https://github.com/pdcarlson/Frapp/issues/2368)) rather than either. The two systems MUST NOT mix on one surface, and there is no longer a surface on the other one.
