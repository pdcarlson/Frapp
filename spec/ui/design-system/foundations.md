...
- On dark surfaces, semantic fills are tints, not solids: ~13% opacity of the hue as background with a lightened step of the same hue as text (see panels 4a/4d). Chip and badge recipes are specified in [`components.md`](components.md).
  - **The tint is a token, never an alpha utility** (decided 2026-09-25, #2376). `bg-success/[.13]` renders the recipe in a current browser, but Tailwind v4 compiles it to a `color-mix()` behind `@supports`, with the **solid hue** as the fallback. Below the `color-mix` floor (Chrome 111, Safari 16.2, Firefox 113) the fill therefore became the text's own colour, and every status label read 1.00:1: invisible. So the recipe lives in four CSS-only tokens instead, each a `color-mix()` of a fixed semantic with `transparent`: `--success-tint`, `--warning-tint` and `--destructive-tint` at 13%, and `--destructive-tint-hover` at 20% (the destructive button's and toast action's hover). In a current engine they are the alpha recipe exactly. Below the floor, a custom property holding a value the engine can't parse is invalid at computed-value time, so the fill falls back to **none**, and the label sits readable on its surface, with every shipped status text clear of 4.5:1 on every ladder step.
  - Two alternatives were weighed and rejected. **An opaque literal** (the recipe resolved over `--card`, as `--mention-chip` does) gives one contrast figure on every surface, but one fill can't stay distinct from every ladder step: the danger tint came out 1.06:1 against `--popover`, so a destructive button in a dialog lost its edge in every browser to fix a handful of old ones. `--mention-chip` can be a literal because it always sits on a bubble; a status tint sits anywhere. **Keeping the alpha utility and documenting the band** would leave text invisible on a stated floor rather than degrading it.
  - **Danger text on the danger tint is always `--destructive-text`.** Solid `--destructive` measures 3.82–4.85:1 on its tint, and 4.48:1 on a bare `--popover` where the tint drops out, so one rule covers both paths and every seat. `apps/web`'s `status-tint-call-sites.spec.ts` fails on a `bg-<semantic>/<alpha>` fill anywhere in the Next surfaces and on unlifted danger text over the tint, and `status-tint-contrast.spec.ts` measures the pairs on both paths. The tokens are CSS-only: `signetDarkTokens` is what `apps/mobile` reads, and React Native has no `color-mix` floor.
