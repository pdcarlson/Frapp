...
| `--success-tint` | `#253221` | The success tint fill: `--success` at 13% over `--card`, as an opaque literal (below) |
| `--warning-tint` | `#3A2F17` | The warning tint fill, same recipe |
| `--destructive-tint` | `#3D2520` | The danger tint fill, same recipe. Its text is `--destructive-text` |
| `--destructive-tint-hover` | `#4C2823` | A danger tint's hover (the destructive button and toast action): `--destructive` at 20% over `--card` |
...
- On dark surfaces, semantic fills are tints, not solids: ~13% opacity of the hue as background with a lightened step of the same hue as text (see panels 4a/4d). Chip and badge recipes are specified in [`components.md`](components.md).
  - **The tint is an opaque token, never an alpha utility** (decided 2026-09-25, #2376). `bg-success/[.13]` renders the recipe in a current browser, but Tailwind v4 compiles it to a `color-mix()` behind `@supports`, with the solid hue as the fallback. Below the `color-mix` floor (Chrome 111, Safari 16.2, Firefox 113) the fill therefore became the text's own colour, and the label read 1.00:1: invisible. So each family has a `--*-tint` token (table above): the recipe composited over `--card` and resolved to a literal, the way `--mention-chip` resolves its own. It renders the same in every engine, and its text contrast is one number on every surface: `--success` 5.31:1, `--warning` 5.85:1 and `--destructive-text` 5.61:1 on their tints, and 5.09:1 on the hover. On `--card` it matches the alpha recipe exactly; on other steps the chip is a shade off the alpha composite, which is the price of a contrast that no longer depends on the browser or the surface.
  - Two alternatives were weighed and rejected. A neutral ladder step with hue text works everywhere but stops reading as a tinted status. Keeping the alpha recipe and documenting the browser band would leave text invisible on a stated floor rather than degrading it.
  - Solid `--destructive` on its tint measures 4.22:1, so danger text on a tint is always `--destructive-text`. The four tokens are CSS-only, like the lifts below: `signetDarkTokens` is what `apps/mobile` reads, and React Native has no `color-mix` floor. `apps/web`'s `status-tint-call-sites.spec.ts` fails on a `bg-<semantic>/<alpha>` fill anywhere in the Next surfaces, and `status-tint-contrast.spec.ts` pins the pairs.
