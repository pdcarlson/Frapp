...
4. **Disable, don't hide, for recoverable states.** A subscription lapse is temporary and the user can fix it; hiding the feature makes the product look broken or missing. Hide only for permissions the user will never hold and modules the chapter has switched off, and, **in the mobile app, features the build does not include**. That last one is a build-time switch such as `EXPO_PUBLIC_ASK_ENABLED`, which no member can clear, so a control whose only function is that feature is not drawn: a control that exists only to say its feature is missing is a placeholder, not a gate, and it is the shape App Review Guideline 2.1 rejects. A control on a real task that the build can serve only part of still disables and names the other route, as the dues Pay button does. (Added 2026-09-22 with the ✦ Ask pill reversal, [#2259](https://github.com/pdcarlson/Frapp/issues/2259); the reasoning is in [`../mobile/navigation.md`](../mobile/navigation.md) § Global entries. It is mobile-only because that decision was: the web dashboard's ✦ pill still opens an "Ask isn't ready on the dashboard yet" dialog (`apps/web/components/layout/ask-pill.tsx`), and whether web follows has not been decided.) **An unresolved permission is not a held one.** Hiding is the right answer to a *denial*; a check that could not run — the offline pause §4 describes — is as recoverable as a lapsed subscription, and gets the same treatment: state it, and offer the recovery.
