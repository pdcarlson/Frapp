...
2. **Say why, and say what fixes it.** A disabled control with no explanation is its own dead end. Pair it with the reason and the recovery path — on **web**, the API's own message is a good source: "Chapter subscription is not active; complete checkout to use this feature." Per [`writing.md`](writing.md), name the blocker and the next action.
   ...
   **Not on mobile.** Relaying that sentence in the iOS app names a purchase path, which contradicts the submitted App Store Connect answers (`apps/mobile/store/README.md` — no in-app purchases, subscriptions "not offered, linked or mentioned in the app") and trades a Guideline 2.1 finding for a 3.1.1 one. Mobile surfaces recognise the refusal with `subscriptionRefusalOf` and render their own member-facing copy, which names an officer and no price, plan, purchase path or link — see `apps/mobile/lib/subscription-refusal.ts` and [`spec/behavior/onboarding.md`](../../behavior/onboarding.md) ([#2297](https://github.com/pdcarlson/Frapp/issues/2297)).
