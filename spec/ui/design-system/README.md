...
4. If no existing token role fits, extend or amend the token definitions and adopt the new role consistently — never one-off the value at the call site. Signet surfaces extend the Signet token definitions — `packages/theme/src/signet.ts`, its stylesheet `packages/theme/src/signet.css`, and the shared Tailwind preset `packages/theme/src/tailwind.config.ts`. **A role both web surfaces bind belongs in the preset; a role only one of them may use stays app-local**, in `apps/web/tailwind.config.ts` or `apps/landing/tailwind.config.ts`. Between [#2366](https://github.com/pdcarlson/Frapp/issues/2366) and [#2371](https://github.com/pdcarlson/Frapp/issues/2371) the shared half was duplicated across those two app configs instead, which is what #2371 collapsed; the app-local half is not a leftover of that and does not collapse. The landing's three marketing type roles are the case that settled it — they are declared in `apps/landing/app/globals.css` and bound as utilities in that app's config, because a 72px storefront headline one import away from every product screen is exactly the off-scale defect [`foundations.md`](foundations.md) §7 bans ([`../landing/README.md`](../landing/README.md#marketing-type-roles), and §7's amendment records the roles themselves). The per-tenant accent family is not in either file — it comes from the engine ([`accent-engine.md`](accent-engine.md)).
...
**Exempt:** every `@SubscriptionExempt()` class or route; the roster and its two rationales are in [`api-architecture.md` § Subscription enforcement (ChapterGuard)](../../../docs/guides/api-architecture.md#subscription-enforcement-chapterguard).

**Not chapter-guarded at all**, so never subscription-gated despite carrying writes: `analytics`, `notification` (except `GET /notifications` and `PATCH /notifications/:id/read`, which are chapter-guarded and route-level `@FreeTier()`), `webhook`, and `POST /chapters`, `POST /chapters/onboard`, `POST /chapters/:id/activate`. (`chapter-directory` is also un-guarded but has no non-GET route, so it contributes no write surface.) Gating these would lock a lapsed chapter out of settings and push registration it is entitled to — over-gating is a worse defect than the late 403. `POST /invites/redeem` is the exception that is gated **in the service** rather than the guard (#1546): it sits on an otherwise-guarded controller, and the chapter it writes to is `invite.chapter_id`, which `ChapterGuard` never sees — so `InviteService.redeem` applies the hard-lock rule to that chapter itself and answers 403 without consuming the token. The join screens render the server message for a 403 (`joinErrorCopy` falls through to `serverMessageOf`), so no client change was needed.
...
2. **Say why, and say what fixes it.** A disabled control with no explanation is its own dead end. Pair it with the reason and the recovery path — on **web**, the API's own message is a good source: "Chapter subscription is not active; complete checkout to use this feature." Per [`writing.md`](writing.md), name the blocker and the next action.

   **Not on mobile.** Relaying that sentence in the iOS app names a purchase path, which the submitted App Store Connect answers forbid — those answers are owned by `apps/mobile/store/README.md`; read the constraint there. Mobile recognises the refusal with `subscriptionRefusalOf` and renders its own member-facing copy instead (`apps/mobile/lib/subscription-refusal.ts`; strings in [`writing.md`](writing.md) § Subscription refused).

   **Three surfaces only, so far** — s19 new task, check-in, and study ([#2297](https://github.com/pdcarlson/Frapp/issues/2297)). The task status toggle, service hours and documents still swallow the refusal into a generic "couldn't save" with a live retry, which is the dead end #2297 removes elsewhere ([#2410](https://github.com/pdcarlson/Frapp/issues/2410)). Do not read this rule as "mobile is covered" when adding a gated write.
...
4. **Disable, don't hide, for recoverable states.** A subscription lapse is temporary and the user can fix it; hiding the feature makes the product look broken or missing. Hide only for permissions the user will never hold and modules the chapter has switched off, and, in the mobile app, two controls whose only function is something the build can't do: **the ✦ Ask pill when the build does not include Ask** ([#2259](https://github.com/pdcarlson/Frapp/issues/2259)) and **the s03 push primer card when the build cannot push** ([#2299](https://github.com/pdcarlson/Frapp/issues/2299)). Ask is a build-time switch (`EXPO_PUBLIC_ASK_ENABLED`) that no member can clear, and the pill's only function is Ask, so it is not drawn: a control that exists only to say its feature is missing is a placeholder, not a gate, and it is the shape App Review Guideline 2.1 rejects. The primer is the same shape. Its only function is "Turn on", and in a build that can't push (`isPushAvailable()` is false; [`../mobile/patterns.md`](../mobile/patterns.md#push-notifications) § Push notifications lists the causes) nothing on s03 can change that, so `shouldOfferPrimer()` omits the card instead of drawing it with a disabled CTA and an apology (`lib/notifications/primer.ts`). The reason still has a home: Settings (s16) keeps its push row in every build and states the reason there, because that row reports a status on a screen with other work, not a control that exists only to announce a gap. A control on a real task that the build can serve only part of still disables and names the other route, as the dues Pay button does. **This is not a rule for every build-time gap.** Each is decided on its own, by whether the control has any function the build can serve. (Added 2026-09-22 with the ✦ Ask pill reversal; the reasoning is in [`../mobile/navigation.md`](../mobile/navigation.md) § Global entries. It is mobile-only because that decision was: the web dashboard's ✦ pill still opens an "Ask isn't ready on the dashboard yet" dialog (`apps/web/components/layout/ask-pill.tsx`), and whether web follows has not been decided. Narrowed the same day from "features the build does not include", which the push primer then contradicted. Extended 2026-09-24 to the push primer, which had been left open in #2299 and now follows Ask.) **An unresolved permission is not a held one.** Hiding is the right answer to a *denial*; a check that could not run — the offline pause §4 describes — is as recoverable as a lapsed subscription, and gets the same treatment: state it, and offer the recovery.
