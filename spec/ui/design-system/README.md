...
4. **Disable, don't hide, for recoverable states.** A subscription lapse is temporary and the user can fix it; hiding the feature makes the product look broken or missing. Hide only for permissions the user will never hold and modules the chapter has switched off, and, **in the mobile app, the ✦ Ask pill when the build does not include Ask** ([#2259](https://github.com/pdcarlson/Frapp/issues/2259)). Ask is a build-time switch (`EXPO_PUBLIC_ASK_ENABLED`) that no member can clear, and the pill's only function is Ask, so it is not drawn: a control that exists only to say its feature is missing is a placeholder, not a gate, and it is the shape App Review Guideline 2.1 rejects. A control on a real task that the build can serve only part of still disables and names the other route, as the dues Pay button does. **This is not yet a rule for every build-time gap.** The push primer is the same shape and still takes the disable branch: when push is unavailable for the build, s03 shows the card with its CTA disabled and the reason stated (`lib/notifications/primer.ts`, `components/notifications/push-primer-card.tsx`, [`../mobile/patterns.md`](../mobile/patterns.md#push-notifications) § Push notifications). Whether it follows Ask is open and tracked separately, in [#2299](https://github.com/pdcarlson/Frapp/issues/2299). Until that lands, read this clause as the Ask pill's alone. (Added 2026-09-22 with the ✦ Ask pill reversal; the reasoning is in [`../mobile/navigation.md`](../mobile/navigation.md) § Global entries. It is mobile-only because that decision was: the web dashboard's ✦ pill still opens an "Ask isn't ready on the dashboard yet" dialog (`apps/web/components/layout/ask-pill.tsx`), and whether web follows has not been decided. Narrowed the same day from "features the build does not include", which the push primer contradicted.) **An unresolved permission is not a held one.** Hiding is the right answer to a *denial*; a check that could not run — the offline pause §4 describes — is as recoverable as a lapsed subscription, and gets the same treatment: state it, and offer the recovery.
