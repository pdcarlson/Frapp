> **TARGET SPEC (partially built).** Every route below now exists in `apps/mobile`, but most screens are stubs and several have no entry point yet — see the Status column, and note that `check-in` (from an event), `host-check-in` (role-gated), `join` and `welcome` are registered without an inbound link, pending the slices that own them — `ask` no longer is: the ✦ pill on s04 and s06 presents its sheet, and the route file survives only because the frozen tab layout registers it. Implementation is tracked in GitHub Issues — do not file spec-vs-implementation drift issues against these documents ([`README.md`](README.md)).
...
| s03 | First-run + notification primer | `(auth)/welcome.tsx` | Routed, stub — auto-joined channels not built. The primer half is **built but unhosted**: C7 shipped the card (`components/notifications/push-primer-card.tsx`) and its decision store (`lib/notifications/primer.ts`, so "Not now" sticks), because push owns the permission request; nothing in #937 owns this screen, so nothing renders them yet |
