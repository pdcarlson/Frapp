> **TARGET SPEC (foundation built, most screens live).** This tree specifies the mobile app Signet is being rebuilt into. As of S1 of #937 the **foundation layer is Signet**: `apps/mobile` runs dark-only on `getSignetTokens()` with Figtree loaded, the provider stack (gesture-handler root, safe-area, guarded keyboard, gorhom sheet host) in place, and NativeWind fully removed — and which screens are live, stubbed or unbuilt is recorded per screen in [`screens.md`](screens.md). Implementation is tracked in GitHub Issues — do not file spec-vs-implementation drift issues against these documents.
...
| One-tap response (RSVP, poll vote, reaction) | Inline optimistic tap on the card itself. MUST NOT open a sheet, modal, or confirm — the tap is the whole interaction. Optimistic/rollback rules: [`../resilience/`](../resilience/README.md). |
...
- The following do **not** run in Expo Go and MUST live behind isolation modules — a runtime execution-environment check with a graceful fallback, so importing the screen never crashes Go:
  - `@stripe/stripe-react-native` (dues payment — see [`patterns.md`](patterns.md)); its isolation module is `apps/mobile/lib/payments/stripe.ts`, behind the platform split in `stripe-module.ts` / `stripe-module.native.ts`
  - `expo-notifications` (remote push and local notifications — see [`patterns.md`](patterns.md)); its isolation module is `apps/mobile/lib/notifications/push.ts`, and it **needed the platform split too**: it links native modules at import, so `push-module.ts` carries no specifier for the package and only `push-module.native.ts` requires it. `apps/mobile/lib/notifications/push.spec.ts` pins that source shape, because the suite-wide `vi.mock` would otherwise hide a static import that crashes Go at launch
  - `react-native-keyboard-controller` (in-sheet keyboards — see [`patterns.md`](patterns.md)); its isolation module is `apps/mobile/lib/keyboard.tsx` (`KeyboardProviderGuarded` / `getKeyboardPath`). It is the **exception** that survives an ordinary lazy require, because its entry touches no native module at import — a property of that package, not a pattern to copy
...
- **A lazy `require()` is not always lazy enough.** `npx expo export --platform web` statically renders every screen in a Node environment, and under that transform Metro evaluates a required package when the module holding it loads — not when the function containing the `require` runs. For a package that touches a native module at import time (Stripe does; `react-native-keyboard-controller` does not) that aborts the export with `Invariant Violation: __fbBatchedBridgeConfig is not set`, *before* any runtime guard can run and outside the reach of a `try/catch`. The fix is a **platform split**: put the `require` in `<module>.native.ts` and ship a `<module>.ts` that returns `null`, so Metro resolves the real package only for iOS and Android. `apps/mobile/lib/payments/stripe-module.ts` is the worked example and `apps/mobile/lib/notifications/push-module.ts` is the second — two of the three guarded packages need it, so treat the split as the default and a bare lazy require as the thing that needs justifying. A `typeof import("<package>")` in a *type* position defeats the split — declare the surface structurally instead.
...
- Chapter accent colors reach screens only through the generated scale: no screen references the seed hex, though the seed's own colour can paint as a generated step ([`accent-engine.md`](../design-system/accent-engine.md) §1 owns the rule and the pipeline).
