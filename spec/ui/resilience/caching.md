...
| Chat outbound (web) | IndexedDB — `frapp-chat` | Until sent | Drafts and the outbox. **Outbound work, not a read cache**: not dropped on an identity change, because that would lose a message. It also carries no tenant scope, so on a shared browser it outlives the member who wrote it — whether to re-key or clear is open in [#2226](https://github.com/pdcarlson/Frapp/issues/2226). [`apps/web/lib/chat/offline-queue.ts`](../../../apps/web/lib/chat/offline-queue.ts). |
