...
| A `(dashboard)/error.tsx` that keeps the nav | `1k` draws the error state full-page. A second boundary inside the shell would be a second answer to one question. **Superseded by [#2175](https://github.com/pdcarlson/Frapp/issues/2175)**, which built it: the lane weighed one question (how to draw *the* error state) and #2145's `next/dynamic` splits then created a second one (a region that failed while the shell kept working). The two answers are not rivals — `app/error.tsx` still draws `1k`'s full-page crest for the pre-auth routes, and the dashboard boundary draws §10's card for a failed region. Copy for both is in [`../design-system/writing.md`](../design-system/writing.md) §7 |
