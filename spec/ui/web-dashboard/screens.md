...
The custom-role rows live in `chapter_custom_roles` and are served by dedicated endpoints (read gated by `chapter-config:view`; write by `chapter-config:manage`): `GET /custom-roles`, `POST /custom-roles`, `PATCH /custom-roles/:id`, `DELETE /custom-roles/:id`. Each write appends a `chapter_audit_log` row (mirrored to `#chapter-audit`) like every other settings save; a `core` role cannot be deleted (`403`), a duplicate `(chapter_id, key)` is rejected (`409`), and the wildcard `*` is rejected as a capability (`400`). These rows **enforce**: members are assigned custom roles from the member directory's detail sheet, and the roles' capabilities flatten into the permission set on the member's next request (bridge model, `spec/behavior/rbac.md`).
