...
one `/code-review` pass on the diff before the branch is pushed; review sub-agents inherit the session
model (Opus). There is no `claude-review-gate` required check, no `claude-review.yml` workflow, and no
...
Cursor Automations run a cloud agent in a fresh sandbox cloned from `main` on a schedule. The sandbox bootstraps from [`.cursor/environment.json`](../../.cursor/environment.json) (currently `npm install`) — read-only audits don't need Docker/Supabase. The automations are configured in the Cursor dashboard (config-as-code isn't supported yet), so the canonical prompts and every setting are version-controlled in [`docs/internal/ci-cd/CURSOR_AUTOMATIONS.md`](../../docs/internal/ci-cd/CURSOR_AUTOMATIONS.md). There are **two** automations, staggered daily, both writing to **Linear** via a `LINEAR_API_KEY` (the headless Cursor sandbox has no Linear MCP) — they **never create GitHub issues** and never edit code: the **Linear Issue Curator** ([`.cursor/skills/linear-curator.md`](../../.cursor/skills/linear-curator.md)) maintains + files `suggestion` issues into Linear Triage, and **Linear Triage** ([`.cursor/skills/linear-triage.md`](../../.cursor/skills/linear-triage.md)) prioritizes/buckets/promotes ~1h later.
