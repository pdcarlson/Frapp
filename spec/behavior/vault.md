...
- The per-chapter KMS/HSM key is **retained** for the chapter's full retention window and destroyed only when the chapter's data is purged. Both purge triggers are prospective, not live: inactivity cleanup at 2 years is a right the Terms reserve but that is **not implemented, and whose deletion strategy is not yet decided** ([`data-retention.md` § Inactive Chapter Cleanup](data-retention.md#inactive-chapter-cleanup)), and no chapter-deletion request path is specified anywhere. In practice the key is retained indefinitely today.
