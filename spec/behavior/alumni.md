...
- **Chat posting** — alumni may post only in ROLE_GATED channels that explicitly require `alumni:post` (`#alumni` in a default chapter) and in direct conversations (DM / GROUP_DM). Posting in ordinary operational channels (PUBLIC / PRIVATE) is denied. This covers **editing** as well as sending: an edit writes new member-authored content, so it clears the same gate — otherwise the rule would be bypassable by rewriting an older message. It also covers **requesting a file-upload URL**, which is the first step of a send rather than a read (#2186). Both steps enforce the rule, so an alumnus attaching a file in an operational channel is refused at the attach — already, rather than only later at the send. Read access is unchanged everywhere they can already see, and the read-only gate (`#announcements`, `#chapter-audit`) still applies on top. `GET /v1/channels` and `GET /v1/channels/:id` carry a per-channel `can_post` capability derived from this same gate (`ChannelAccessService.withPostCapability`), so web, mobile, and poll creation can disable the composer proactively instead of the member discovering the restriction from a failed send.
