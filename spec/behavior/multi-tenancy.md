...
- **Repository level** — a tenant-scope spec per repository, built on `createTenantHarness` (`apps/api/test/helpers/tenant-scope.harness.ts`). Each seeds two chapters whose rows are identical except `id` and `chapter_id`, so every predicate but the tenant one matches both and only a real tenant filter narrows the result; the harness then asserts the filter was applied, no foreign row was written, and no foreign row came back. This is the layer type-checking cannot reach: `.eq('id', chapterId)` in place of `.eq('chapter_id', chapterId)` compiles.
  ...
  Module-local workers sit in the same ledger. Scheduled-job **sweeps** (`findEventsPendingAutoAbsent`, invoice/task due windows) are cross-chapter by design — the cron has no caller chapter and pages every tenant. The compensating writes are not: `claimDispatch` and `releaseDispatch` take `chapter_id` from the sweep row (not ambient request context) and bind it on the insert and the delete. A failed send in one chapter must not drop another chapter's claim, even though the unique key on `scheduled_notification_dispatches` is the entity tuple (entity ids are globally unique UUIDs).
