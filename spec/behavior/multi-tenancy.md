...
A fourth step follows, and it deliberately lives in `FrappProvider` rather than in the helper: **when the active chapter changes, the client query cache is dropped wholesale.** Only some chapter-scoped query keys embed the chapter id, so without this the outgoing chapter's rows stay cached and render under the incoming chapter's context — a cross-chapter leak in the client, even though the API itself would reject the equivalent request. It has to run _after_ React commits the new chapter id: clearing the cache makes mounted observers refetch, and a clear that races the commit sends those refetches out under the outgoing chapter and repopulates what it just emptied. An effect keyed on the chapter id runs after commit, and covers every path that changes chapters rather than one helper.

**Mobile implements the same fourth step, from a different signal.** `apps/mobile/lib/frapp-client.tsx`'s `FrappProvider` runs the identical effect, keyed on the chapter id it reads from `useAuthSession()` — which on mobile derives from the token's `active_chapter_id` claim rather than a persisted store, so the effect covers `lib/select-chapter.ts`, the chapter picker, and a magic-link account swap alike. Two guards matter there and are not cosmetic: the effect skips when the _previous_ chapter was `null`, because `chapterId` is null until the first claim read settles and a cold start would otherwise clear its own bootstrap queries; and `auth-session.tsx` deliberately retains the last known chapter on a _failed_ claim read, so a network blip cannot present as a switch and trigger a spurious clear.

**Sign-out drops the cache too, and mobile owns that in one place.** Mobile's `QueryClient` is a module singleton (`apps/mobile/lib/query-client.ts`), so signing out clears SecureStore and React state while the cache survives in memory — and several keys are not account-scoped at all (`["settings"]`, `["user","me"]`), so they would never re-key their way out of it. On a shared device that serves the next member the previous one's rows. The clear therefore lives in `signOut` in `lib/auth-session.tsx`, on the same always-run path as the token clear, so a failed remote revoke still drops it. It is _not_ in the screens: there are three sign-out paths (`(tabs)/preferences.tsx`, `(auth)/chapter-picker.tsx`, `(auth)/join.tsx`) and each had to remember the clear for itself, which the picker's only got after the leak was found a second time. The singleton lives in its own module rather than beside `FrappProvider` because `app/_layout.tsx` nests `AuthSessionProvider` _outside_ `FrappProvider` — so `useQueryClient()` is unreachable from the auth provider, and importing the client back from `frapp-client.tsx` would form a cycle.
...
- **Repository level** — a tenant-scope spec per repository, built on `createTenantHarness` (`apps/api/test/helpers/tenant-scope.harness.ts`). Each seeds two chapters whose rows are identical except `id` and `chapter_id`, so every predicate but the tenant one matches both and only a real tenant filter narrows the result; the harness then asserts the filter was applied, no foreign row was written, and no foreign row came back. This is the layer type-checking cannot reach: `.eq('id', chapterId)` in place of `.eq('chapter_id', chapterId)` compiles.
  ...
  Module-local workers sit in the same ledger. Scheduled-job **sweeps** (`findEventsPendingAutoAbsent`, `findOpenInvoicesDueBetween`, `findIncompleteTasksDueBetween`) are cross-chapter by design — the cron has no caller chapter and pages every tenant. Each is characterised as unscoped in `scheduled-jobs.repository.spec.ts` (colliding twins in both chapters, no `chapter_id` filter). The compensating writes are not: `claimDispatch` and `releaseDispatch` take `chapter_id` from the sweep row (not ambient request context) and bind it on the insert and the delete. A failed send in one chapter must not drop another chapter's claim, even though the unique key on `scheduled_notification_dispatches` is the entity tuple (entity ids are globally unique UUIDs).
