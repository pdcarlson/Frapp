...
| Subscription lapses to `past_due`             | 3-day grace period. During grace: reads continue and free-tier writes continue, but invite *minting* is blocked (`chapter.subscription.invite_blocked`) and paid-ops writes are blocked (`chapter.subscription.write_locked`). After grace or upon `canceled`: hard lock (read-only), apart from the `@SubscriptionExempt()` billing-recovery routes. |
