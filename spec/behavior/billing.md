...
- That guard is load-bearing rather than defensive: `createCheckoutSession` rejects only `active`, so a `past_due` or `canceled` chapter does reach checkout, and because the adapter passes `customer_email` rather than the stored customer id (the gap below), Stripe mints a **fresh Customer with no trial history** on every call. Stripe therefore cannot refuse a repeat trial on its own — nothing but `grantTrial` stands between a direct API caller and unlimited free 14-day windows.
