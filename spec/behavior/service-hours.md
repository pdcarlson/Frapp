...
- Proof uses the same signed-URL flow as Backwork and chapter documents, plus a storage existence check at submission: the client requests an upload URL (`POST /service-entries/proof-upload-url`; kind `proof` in `@repo/validation` — images and PDF only, 25MB max), uploads to it, then submits the returned storage path as `proof_path`. The API rejects entries whose `proof_path` falls outside the active chapter's service-proof prefix or does not reference an actually-uploaded object — a member can never attach an arbitrary, guessed, or cross-chapter storage key. The type and size limits are enforced on the bucket itself (`allowed_mime_types` / `file_size_limit`), because a signed upload URL cannot pin a content type — the API allowlist alone would only gate URL issuance. Clients also check type and size before requesting the URL.
...
- **No self-approval:** An admin cannot approve an entry they submitted themselves — `ServiceEntryService.approve` refuses when the entry's `user_id` matches the reviewer, with 403 `Admins cannot approve their own service entries`. Approving awards SERVICE points to the submitter, so allowing it would let one member both earn and authorise a ledger write; this is [`points.md`](points.md) § Anti-Fraud's ledger-wide **No self-award** rule reaching the service path. The check runs **before** the PENDING-status check, so which refusal a caller sees never depends on the state of their own entry, and before the rate lookup, so a sub-rate entry that would award no points is refused identically. Note `service:approve` is held by no default role except President's wildcard, so a chapter on stock roles must grant it to a second role for its President's own hours to be approvable — the general fix for that is [#1340](https://github.com/pdcarlson/Frapp/issues/1340) (reassignment), not an exception here.
