...
- Clients redeem the opaque token, not the URL. Web `/join` and mobile s02 accept a pasted raw token or a full join URL (`…/join?token=…`, `frapp://join?token=…`) and extract via `extractInviteToken` in `@repo/validation` before `POST /v1/invites/redeem`. Officers copy that URL (`buildJoinUrl` / the Members invite dialog).
