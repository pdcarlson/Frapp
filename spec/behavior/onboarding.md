...
- **Redeeming is never subscription-gated.** `POST /v1/invites/redeem` carries no `ChapterGuard` at all (guards are applied per-route on `InviteController`, and `redeem` declares none) — necessarily so, since the caller is not yet a member of the chapter and has no `x-chapter-id` to check. A token minted before a lapse therefore still works after it: a member can join a `past_due` or `canceled` chapter. Only *minting* is gated.
