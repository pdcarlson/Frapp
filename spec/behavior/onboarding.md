...
- Fires on first sign-in when the authenticated user has **no chapter memberships** (`GET /v1/chapters` returns empty). It mounts as a full-screen overlay and replaces the old `chapter-bootstrap` component.
