...
- **Per-domain cap:** `SEARCH_LIMIT` in `search.service.ts`, currently **10** results per source. Anything past it is silently dropped.
- **Paging:** none. `GET /v1/search` accepts only `q` and `channelId` — there is no page or offset parameter and no `.range()` in the query path, so the cap above is the whole result set. (The spec previously claimed 20 per domain / 80 per page and a 50-result cap; none of that was ever built.)
- **Ranking:** specified as Postgres `ts_rank_cd` over the `tsvector` columns, with per-domain field weighting (e.g. Backwork title outweighs tags) set via `setweight` at index time rather than at query time. **Not implemented for any source, chat included** — `search.service.ts` applies no `ts_rank`, ordering chat by `created_at`, leaving the other three unordered, and cutting each source at `SEARCH_LIMIT`. Migration `20260829002000` records the same gap. Anything that needs relevance scores (hybrid retrieval, rerank) has to add this first.
