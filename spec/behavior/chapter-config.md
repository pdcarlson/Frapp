...
**Audit (#2575).** A confirm that changes `logo_path` writes a member-visible `chapter_audit_log` row with action `chapter_logo_updated`, and a delete that removed a logo writes `chapter_logo_removed`. Both use `target_type` `chapter` and a `diff` of `{ logo_path: { from, to } }`. Like the profile PATCH, a request that changes nothing writes no row. That covers a confirm of the path already stored, and a delete when no logo was set. A confirm of the stored path can't be a replacement, because the mint refuses to re-sign an existing key, and a removal clears `logo_path` before it deletes the object, so the stored path never names a free key. Replacing a logo with another of the same extension therefore fails today, tracked in #2592. The rows are written after the update lands, with the same #1599 residue as above: an identical retry changes nothing, so it writes nothing.
