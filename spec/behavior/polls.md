...
- **Both poll reads apply the caller's chat block list** (#2495). A poll is authored by the member who created it, so `GET /v1/polls` and `GET /v1/polls/{messageId}` mask a poll from a member the caller has blocked **in place**: the row keeps its ids, timing, expiry, every `voteCount` and the caller's own `userVotes`, and loses its question and option text (`content` becomes the block sentinel, `metadata` keeps only `choice_mode`, `expires_at` and `closed_at`, and each result's `optionText` is `null`). It is not left out of the list, because the tallies are chapter state a block counts rather than hides, and dropping the row would change the page's counts. Every row carries `sender_blocked`, which is what a client keys on; an imported poll (`sender_id: null`) is never masked; and a block list that cannot be read fails the request rather than serving the poll in the clear. The rules are [`chat/README.md` § What a block does and does not hide](chat/README.md#what-a-block-does-and-does-not-hide). The web Polls page renders a masked poll as "Poll from a member you blocked", its options as "Option 1", "Option 2" and so on with their tallies, and no way to vote; a vote the member cast before the block can still be withdrawn.
...
- **Default role mapping:** `polls:view_all` is **not** on the default Member role, and every seeded role that holds it also holds `members:view`, so the guard chain succeeds. Which seeded roles those are: [`rbac.md` § Role Lifecycle](rbac.md#role-lifecycle).
- **Backfill migrations:** chapters that predate these seeds got the grants by data migration, not by re-seeding. Which migrations, and what each inserted: [`docs/guides/database.md` § Conventions](../../docs/guides/database.md#3-conventions) (RBAC seed vs existing data).
...
- **Expiry announcement.** A scheduled sweep (`ScheduledJobsService.sweepExpiredPolls`, every 5 minutes) posts a `system_audit` message — `This poll has closed.`, as a reply to the poll (`reply_to_id`) — into the channel of every poll whose `expires_at` has passed and that was not manually closed early (a manual `close` needs no announcement; the creator already knows). Like the other sweeps it claims a `scheduled_notification_dispatches` row (`entity_type: 'POLL'`, `threshold: 'EXPIRED'`) before posting, so it fires exactly once per poll across replicas and ticks. `system_audit` messages render in the channel timeline without sending a push notification (`push-rules.ts`), matching the pattern already used for invite-acceptance and chapter-welcome notices. The notice replies rather than quoting the question because the system actor cannot be blocked: `Poll "<question>" has closed.`, the text it posted until #2495, put a blocked member's question in the blocker's timeline beside the poll the block had hidden. As a reply, both clients draw the poll as the quote, and that quote is masked like any other reply to a blocked member's message. It replies in a read-only channel too: [`chat/README.md`](chat/README.md)'s no-threaded-replies rule there gates what a member sends, not what the server writes. Two known costs: a client resolves the quote only from its loaded page of messages, so once the poll is more than a page back the notice reads "Original message not loaded" for every member ([#1571](https://github.com/pdcarlson/Frapp/issues/1571)); and notices posted before #2495 still quote the question ([#2681](https://github.com/pdcarlson/Frapp/issues/2681)). A notice is also skipped for a poll deleted after the sweep's snapshot, which would otherwise quote `[message deleted]`.
