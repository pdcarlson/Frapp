...
| Route | Scope |
| --- | --- |
| `GET /v1/channels/notification-preferences` | the caller's **effective** level per channel |
...
**The two reads answer different questions, and the difference is load-bearing.** The channel read resolves the full precedence chain through the worker's own `resolveLevel`, so it reports the level that will actually be applied — including a kind override that outranks a channel's default. It is the number a mute UI should display. Reimplementing that chain locally is what let the two disagree once before: the endpoint read only channel rows, which was correct only while nothing could write a kind row.

The kind read reports **overrides only**, with `level: null` where the member has set none. It deliberately does not fill in a default, because a kind has none to fill in: what a kind falls back to depends on the channel a message lands in, so `announcement` resolves `all` in `#announcements` and `mentions` elsewhere. Reporting a single scalar would state the wrong level for the seeded `#announcements`, and a settings screen that wrote back what it displayed would thereby *create* an override silently downgrading that channel. `DELETE` exists for the same asymmetry: a kind override outranks every channel's name-derived default, so one a member could not remove would be a permanent invisible downgrade. The channel arm needs no equivalent — writing a channel's own default back is behaviourally identical to having no row.
