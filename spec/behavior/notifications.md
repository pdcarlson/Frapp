...
- **The two surfaces differ on offline, deliberately.** Mobile has the AsyncStorage cache above and a `pending`/`retry` sync indicator, so it can accept a toggle and show that it has not landed yet. Web has neither a mutation persister nor an outbox, and the query client leaves `networkMode` at `"online"` — so an offline toggle would run `onMutate` (the switch moves), pause before the request is sent, and be lost on tab close with no error and no revert. Web therefore **refuses** the write while `OFFLINE` and toasts why, rather than accepting one it cannot keep. It refuses with `aria-disabled` rather than the `disabled` attribute: `switch.tsx` scopes every state colour to `enabled:`, so a genuinely disabled switch loses its on/off appearance entirely and drops out of the tab order — and *reading* which categories are muted is the more common offline need. The offline note says only that changes cannot be made; it deliberately does **not** claim the displayed state is what the server holds, because a toggle made just before the connection dropped can be showing an optimistic value that paused mid-flight. Queued dashboard writes generally are #1707; if that lands, this is the first surface that should stop refusing.
