...
`notifyChapter` does **not** fan out `notifyUser`. It loads the roster once, then (unless URGENT) the category preference for the whole audience in chunked `in (user_id)` reads of 100 ids — the 414 bound measured in `apps/api/src/domain/utils/chunk-ids.ts` — pages each chunk under the #686 rule, evaluates quiet hours in memory against a similarly batched `user_settings` read, inserts in-app rows with `createMany` in the same 100-row chunks, and looks up `push_tokens` once for the users who got a row. Push itself stays per recipient, because each Expo payload must stamp that user's `notificationId`. Missing preference rows stay enabled, matching `notifyUser`. Insert failures are isolated per insert chunk and logged as an aggregate count rather than swallowed; a rejected push is likewise counted, not silent. `Promise.allSettled` remains only around the Expo sends.
