...
- **A client must never "repair" a stored zone on its own verdict.** Mobile replays the stored value into a `PATCH` when quiet hours are toggled back on, so substituting on a stale-tzdata false negative would overwrite the member's zone on every device, silently and permanently. It therefore repairs only device-independent defects — not a string, blank, or over-length — and lets the server judge the rest. A legacy row holding a genuinely unresolvable zone round-trips to a `400` and surfaces as the retry state: a visible error on an already-broken row, in exchange for never corrupting a correct one.
