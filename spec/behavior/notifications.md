...
- **`announcements` is deliberately not a member switch.** Step 1 of the delivery flow runs *before* the priority check, so disabling that category suppresses URGENT chapter announcements **and** their in-app rows. Exposing the switch would let a member silently mute an emergency broadcast from a settings screen. It ships once URGENT is exempt from the preference gate.
