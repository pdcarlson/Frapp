...
- **Rate limiting:** A single admin cannot create more than N point adjustments per hour **in a given chapter**. The counter is scoped to the `(admin, chapter)` pair — `countRecentAdjustments(adminUserId, chapterId, since)` filters on both — so an officer who holds `points:adjust` in two chapters gets N in each, and a chapter's total exposure is N × the members who can reach the route — those holding `points:adjust` **or** `*` — not N. Exceeding the limit returns 429 Too Many Requests. N is `chapter_points_config.adjustment_rate_limit_per_hour`, **default 50**. This section is the canonical statement of the scoping; other docs link here rather than restating it.
