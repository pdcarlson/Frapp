...
- **No self-award:** An admin must not be able to move their own balance without a second party. Like the ceiling above, this binds the **ledger**, not one endpoint: every path where the same member both earns and authorises an award is required to refuse it. Enforcement is per-path and currently incomplete — the sub-bullets record where it holds today and where it does not.
  ...
  - Event check-in and study accrual are out of scope by construction: they award the acting member against measured state rather than an authorisation, so there is no second party to require.
  - **Service-hour approval does not yet enforce this** — [#1338](https://github.com/pdcarlson/Frapp/issues/1338). `ServiceEntryService.approve` receives the reviewer's id but never compares it to `entry.user_id`, so a member who submits their own hours can approve them. Submitting needs no special permission at all, so the approve grant is the only control on that path.
