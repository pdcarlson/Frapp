...
- **Idempotency:** `POST /v1/points/adjust` accepts a client-minted UUIDv4 `client_message_id`, and a replay carrying a key this chapter has already used **returns the original transaction instead of writing a second row**. This exists because the append-only rule above has a sharp edge: a lost response is indistinguishable from a lost write, and a retry that double-granted could not be undone — only offset by a further adjustment. The key is the dedupe key for the ledger row *and* the chat card, enforced by the partial unique index `idx_point_transactions_dedupe` on `(chapter_id, client_message_id)`. Four things follow, and the third is the one that bites:
  ...
  - **A replay fires no side effect at all** — no push notification, and no chat card. Neither is safely repeatable here: the notification is simply not idempotent, and the card only looks it. `idx_chat_messages_dedupe` is scoped `(channel_id, sender_id, client_message_id)` rather than by key alone, and the ledger row records no channel, so a replay cannot prove it names the channel the original card went to. Re-posting would therefore not deduplicate: a caller sending a byte-identical body with a different `channel_id` would get a *second* audit card for one ledger row, re-broadcasting a `FINE` to a wider audience. The consequence is that a card lost to a failed best-effort post stays lost ([#1734](https://github.com/pdcarlson/Frapp/issues/1734)) — the ledger row is the durable record, the card is not.
