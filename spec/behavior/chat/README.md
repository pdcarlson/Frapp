...
- **Presence lifecycle on mobile** — **specified, not built**: nothing binds `AppState` to presence today. As designed it would map app state to presence: backgrounded → `idle`, force-quit → `offline` (consistent with the Idle/Offline statuses tracked via Realtime Presence above).
