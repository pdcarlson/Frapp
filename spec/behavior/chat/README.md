...
- **Welcome message on onboarding submit.** A chapter created through the onboarding wizard gets a one-time welcome `system_audit` message, the first message it sees (a chapter created through `POST /v1/chapters` gets none). Its channel and text are owned by [`../onboarding.md` § First-Officer Onboarding Wizard](../onboarding.md#first-officer-onboarding-wizard). On **mobile** the officer lands in `#general`; on **web** the wizard finishes on `/billing` since #2297, so the message is waiting there rather than being landed on.
