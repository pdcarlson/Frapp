...
- **A user can hide a 1-on-1 DM from their own list** (#2303). The same route does it: on a `DM` it hides rather than leaves, and on any non-DM channel it is still rejected, since there is no exit from a chapter channel. A hide is per member and destroys nothing. The channel row, its `member_ids` and its messages are untouched, and the other member's list and thread are exactly as they were: they are not told, and nothing they can see changes. It is not a leave, because a 1:1 DM whose member list dropped to one would stop being a conversation with anyone, and archiving it would take it off the other member's list too.
  - **Where.** Mobile: a long press on a 1:1 DM row, or the row's accessibility action. Web: a Hide control on a 1:1 DM row in the rail (shown on hover and keyboard focus), and a row in the open DM's `⋯` menu. Both confirm first.
  - **What it changes.** The DM moves out of the member's channel list into a collapsed **Hidden conversations** group at its end, on mobile and web, and is marked read. While it stays hidden, `GET /v1/channels/unread` leaves it out, so it lights no badge, not even for messages from a member the hider blocked, which never bring it back (below). It stays readable by id, so a push notification, a search hit or a bookmark still opens it. `GET /v1/channels` keeps returning the row, flagged `hidden: true`, so every jump that resolves a channel against that list keeps working; each client renders it only in the Hidden conversations group, except that web keeps an open hidden DM in its usual place on the rail so the open row stays marked. Hiding the open DM on web moves the member to their default channel. A hide that fails says so at once (an alert on mobile, a toast on web).
  - **What brings it back.** Opening it again yourself: from the Hidden conversations group on either client, or the directory's Message action on web. Both go through `POST /v1/channels/dm`, which clears the hide outright. It also comes back on a new message in it, from anyone except a member the hider has blocked in this chapter, the hider's own included (sent from another device, say), the next time the list is read. A blocked member's message does not bring it back, because nothing they send reaches the blocker (§ What a block does and does not hide); an unblock therefore brings back a thread they wrote in while the block stood. A deleted message does not count. The message rule is derived when the list is read (`get_hidden_channel_ids`), not written when a message is sent, so it adds nothing to the send path and cannot race it.
  - **What it does not do.** It does not stop the other member messaging: pushes still arrive, and their next message brings the thread back. That is a block's job (§ Report and block), and hiding alone does not answer App Store Guideline 1.2's "block abusive users".
...
| The blocked member's **polls on the poll routes** (`GET /v1/polls`, `GET /v1/polls/{messageId}`) | Question and option text hidden, the poll itself kept. Each is masked in place, with its ids, timing, expiry and tallies intact and `sender_blocked` set, rather than left out of the list, since leaving it out would change the page's counts. The poll-expiry notice replies to the poll instead of quoting its question, so a new notice does not bring the question back under the system actor (#2495). Notices posted before #2495 still quote it and are not masked, since the system actor cannot be blocked: [#2681](https://github.com/pdcarlson/Frapp/issues/2681). Details: [`../polls.md`](../polls.md) |
...
- **An echoed row is never re-evaluated by the server**, because the reconnect backfill and the polling fallback read only rows after the last-seen cursor, which every echo advances. So a client remembers, for the session, which rows it already showed against a *ready* list, and which server-cleared rows it showed while the list was loading or unavailable. Server-evaluated rows are included because an UPDATE echo of a REST row (a pin, an edit, a soft delete) replaces it unevaluated. A later outage does not take any of those back; only an unevaluated row that first arrives during the outage is held (`blockClearance` in `packages/chat-core/src/blocks.ts`). A block always outranks that memory. On web, the dashboard's cached first chunk keeps each row's server verdict so a cold load can paint, and a verdict is only as old as the read that produced it, not the cache write. So the cache also keeps the ids the last ready list read named, and a cold load applies them as a floor until the live list has been read (#2688). Anyone blocked as of that read is a tombstone in the cached rows from the first paint. The residual is a block made on another device after the last ready list read web made while `/chat` was open (a read by `/profile`'s Blocked members card alone is not persisted): it shows in the cached rows until the list or a live read of the channel lands, and for the whole session with the list unreadable, the same shape as #2499.
...
- Announcement messages cannot be replied to in-thread. The rule is a property of the **channel**, not the caller: it is keyed off `is_read_only` (so it covers `#chapter-audit` and any chapter-created read-only channel, and survives a chapter renaming its announcements channel), and it holds regardless of permissions — a member with `announcements:post`, and the President's `*`, are refused a threaded reply just the same. `announcements:post` governs who may author a **top-level** announcement; no member threads one. Enforced by `allowsInThreadReplies` in `@repo/validation`, called from `ChatService.sendMessage`. **The rule gates what a member sends, not what the server writes.** `allowsInThreadReplies` has one caller, `ChatService.sendMessage`, so two server-side writers can store a reply in a read-only channel: the poll-expiry notice (`PollService.announceExpiry`), which replies to the poll it announces in every channel because the reply quote is the only thing that names the poll without re-posting its author's text under the unblockable system actor ([`../polls.md`](../polls.md), #2495); and the Discord archive importer, which carries over the reply links in history it imports into a read-only channel. Neither opens a discussion under an announcement, and every member-authored threaded reply is still refused.
...
Each user's last-read timestamp per channel is tracked in a `channel_read_receipts` table. Opening a channel stamps the cursor to server `now()`, and so does hiding a 1:1 DM (§ Direct Messages); there is no mark-read-to-a-specific-message. A DM the member has hidden has no row in `GET /v1/channels/unread` at all while it stays hidden, so it lights no badge.
