...
**Status: schema and API.** The tables exist (`20260915210000`) and the API is live: `POST|GET|PATCH /v1/chat/reports` and `GET|POST|DELETE /v1/chat/blocks` (`ChatReportController` / `ChatBlockController`, both `@SubscriptionExempt()`), and the API's read surfaces apply the caller's block list to what they serve. [`chat-read-surface-ledger.spec.ts`](../../../apps/api/src/application/services/chat-read-surface-ledger.spec.ts) is the canonical list of those surfaces. It classifies every chat route, every chat table a client can read directly, and the push fan-out, and it fails on any it does not cover. Three gaps are open there: reaction chips, which both clients read directly from `chat_message_actions` (#2494); the poll routes (#2495); and the Realtime message echo, which only a client can mask (#2315, #2313). **No client surface exists yet** — neither mobile nor web has the report affordance, the blocked-members list in Settings, the tombstone, or the client-side masking the § below requires, and nothing renders the officer queue. So the sections below are still partly a contract the remaining slices of #2257 are written against rather than a description of shipped behavior; where they describe rendering, they describe what is owed — [`AGENTS.md`](../../../AGENTS.md) § Spec vs code.
...
| Reactions the blocked member leaves, on any message | Hidden. A reaction chip is their content wherever it sits, and hiding it only on the blocker's own messages would leave the blocked member a way into every thread the blocker reads |
