...
- **The removal is an ordinary soft delete** — the same tombstone, `metadata` wipe and attachment purge as any delete (§ Edit and delete). If it fails after the claim, the claim is withdrawn — the report goes back to `open`, conditional on still carrying this request's own stamp — before the error is returned, so a failed removal does not leave an `actioned` report over content that is still there, and the report is open for a retry. A withdrawal that itself fails is logged as an error for a person to look at.
