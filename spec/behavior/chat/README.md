...
- **Only an open report in the caller's chapter.** The report is read scoped to the chapter, so another chapter's report is the same 404 as none. A report that is `reviewed`, `actioned` or `dismissed` grants nothing (409), and neither does one whose message is gone — hard-deleted (`message_id` went NULL) or already soft-deleted by its sender (409; the officer resolves the report instead).
- **The removal is an ordinary soft delete** — the same tombstone, `metadata` wipe and attachment purge as any delete (§ Edit and delete) — and the report becomes `actioned`, stamped with the officer. The message is removed first and the report resolved second, so a refused removal never leaves an `actioned` report over content that is still there.
