...
- **The removal is an ordinary soft delete** — the same tombstone, `metadata` wipe and attachment purge as any delete (§ Edit and delete). If it fails after the claim, the message is read again first, because a failure does not prove nothing was written: the tombstone can commit and only the answer be lost. While the message is still there (or the read fails too), the claim is withdrawn — the report goes back to `open`, conditional on still carrying this request's own stamp — before the error is returned, so a failed removal does not leave an `actioned` report over content that is still there, and the report is open for a retry. A withdrawal that itself fails is logged as an error for a person to look at. If the message is gone, the claim stands and the sibling sweep runs, so no report reopens over a removed message. The answer then follows the idempotency rules below: a 4xx was decided before anything was written, so someone else removed the message and the route answers 200 with `message_already_deleted: true`; any other failure may be this request's own write, so the error is returned as an unknown outcome, and a retry gets the report-level replay's 200.
