...
- **Presence lifecycle on mobile** — **specified, not built.** The only presence write anywhere is `channel.track({ userId, ts })` in `@repo/chat-core`'s realtime manager; no `idle` or `offline` status is written, and nothing binds `AppState` to presence. As designed it would map app state to presence: backgrounded → `idle`, force-quit → `offline` (consistent with the Idle/Offline statuses tracked via Realtime Presence above).
