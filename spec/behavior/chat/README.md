...
Their authorization differs (#1552). `presence:chapter:<chapterId>` is a **private** channel since 2026-09-06: `realtime.messages` RLS admits chapter members only to join and to `track()`, so an anon-key holder can neither read the roster nor publish an entry. `chat:channel:<channelId>` is still **public** — it also carries `postgres_changes` and typing broadcasts and is read by the push worker, and gating it needs a per-channel predicate (chapter membership would leak DM and role-gated presence), which is #1552's second phase. In both cases presence stays **advisory** and must never be an input to an authorization decision: even on the private topic a chapter *member* can still publish an entry naming another member, because Realtime policies see the topic and the message extension, not the presence payload. What each topic exposes is [`docs/internal/security/AUTHORIZATION_MODEL.md`](../../../docs/internal/security/AUTHORIZATION_MODEL.md) § "The policies that do exist".
