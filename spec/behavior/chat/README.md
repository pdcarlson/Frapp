...
- **An echoed row is never re-evaluated by the server**, because the reconnect backfill and the polling fallback read only rows after the last-seen cursor, which every echo advances. So a client remembers, for the session, which rows it already showed against a *ready* list, and which server-cleared rows it showed while the list was loading or unavailable. Server-evaluated rows are included because an UPDATE echo of a REST row (a pin, an edit, a soft delete) replaces it unevaluated. A later outage does not take any of those back; only an unevaluated row that first arrives during the outage is held (`blockClearance` in `packages/chat-core/src/blocks.ts`). A block always outranks that memory. One exception on web: the dashboard's cached first chunk keeps each row's server verdict so a cold load can paint, and a verdict is only as old as the read that produced it, not the cache write. The list is not persisted, so each cold load starts with it loading; a member blocked since that read shows in the cached rows until the list or a live read of the channel lands, and for the whole session with the list unreadable (#2688).
