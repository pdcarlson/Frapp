...
| The blocked member's **polls on the poll routes** (`GET /v1/polls`, `GET /v1/polls/{messageId}`) | Question and option text hidden, the poll itself kept. Each is masked in place, with its ids, timing, expiry and tallies intact and `sender_blocked` set, rather than left out of the list, since leaving it out would change the page's counts. The poll-expiry notice replies to the poll instead of quoting its question, so the question does not come back under the system actor (#2495). Details: [`../polls.md`](../polls.md) |
