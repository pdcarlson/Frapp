...
**Status: schema, API, and the mobile client.** The tables exist (`20260915210000`) and the API is live: `POST|GET|PATCH /v1/chat/reports` and `GET|POST|DELETE /v1/chat/blocks` (`ChatReportController` / `ChatBlockController`, both `@SubscriptionExempt()`), and `ChatService.getMessages` applies the caller's block list to what it serves. **The mobile app consumes the member half**: a long-press (or the "Message actions" accessibility action) on someone else's message or poll opens Report and Block; the thread tombstones a blocked member's messages and holds unmaskable rows while the list is unreadable, per § The masking contract (`apps/mobile/lib/chat/blocks.ts`); and Settings → Blocked members and the directory's member sheet can undo a block. **Still owed:** web has none of it (#2313); nothing renders the officer queue, so a filed report is readable only through `GET /v1/chat/reports`; reactions and attachment reads are not masked server-side (#2324); and whether a block hides a blocked member's words *quoted in someone else's reply* is undecided (#2312). Where the sections below describe those, they describe what is owed — [`AGENTS.md`](../../../AGENTS.md) § Spec vs code.
