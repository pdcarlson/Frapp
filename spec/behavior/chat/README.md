...
- **A user can hide a 1-on-1 DM from their own list** (#2303). The same route does it: on a `DM` it hides rather than leaves, and on any non-DM channel it is still rejected, since there is no exit from a chapter channel. A hide is per member and destroys nothing. The channel row, its `member_ids` and its messages are untouched, and the other member's list and thread are exactly as they were: they are not told, and nothing they can see changes. It is not a leave, because a 1:1 DM whose member list dropped to one would stop being a conversation with anyone, and archiving it would take it off the other member's list too.
  - **What it changes.** The DM leaves the member's channel list on mobile and web, and is marked read, so no badge is left on a row they can no longer see. It stays readable by id, so a push notification, a search hit or a bookmark still opens it. `GET /v1/channels` keeps returning the row, flagged `hidden: true`, so every jump that resolves a channel against that list keeps working; clients leave it out of the list they render (web keeps it on the rail only while it is the open channel). Hiding the open DM on web moves the member to their default channel.
  - **What brings it back.** A new message in it, from anyone except a member the hider has blocked in this chapter, the hider's own included (sent from another device, say). It reappears the next time the list is read. A blocked member's message does not bring it back, because nothing they send reaches the blocker (§ What a block does and does not hide); an unblock therefore brings back a thread they wrote in while the block stood. A deleted message does not count. Opening the DM again yourself (the directory's Message action, `POST /v1/channels/dm`) also brings it back, and clears the hide outright. The unhide is derived when the list is read (`get_hidden_channel_ids`), not written when a message is sent, so it adds nothing to the send path and cannot race it.
