...
- **Wildcard exclusion.** Custom roles can never carry `*`: create/update reject a wildcard capability with `400`, and the resolver drops `*` from custom-role capabilities defensively (pre-validation rows). Only the live President role wields the wildcard, preserving the presidency-transfer safeguard — a custom role cannot mint a second President, and `custom_role_ids` changes are exempt from (and irrelevant to) the President-role restrictions on the generic role-update endpoint.
