...
- **Wildcard exclusion.** Custom roles can never carry `*`: create/update reject a wildcard capability with `400`, and the resolver drops `*` from custom-role capabilities defensively (pre-validation rows). The same rule protects the live `roles` table: role create rejects `*`, and role update rejects *introducing* `*` to a role that does not already carry it (the seeded President keeps its wildcard and stays editable). The generic role-update endpoint refuses to add or remove **any** wildcard-carrying role — keyed on the permission, not on the seeded President row — so a legacy `*` role cannot be smuggled onto a member either. Only the presidency-transfer flow moves the wildcard.
