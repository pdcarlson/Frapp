...
- **Edge case:** If the President leaves the chapter (account deletion or manual removal by Frapp support), the system flags the chapter and prompts the next member with the highest-ranked admin role to claim the presidency. If no suitable member exists, Frapp support intervenes.
  ...
  - **`POST /v1/roles/claim-presidency`** — `RbacService.claimPresidency` re-validates `needs_president` and eligibility server-side (the status endpoint is advisory for the UI, not the enforcement point), then grants the role via the `claim_presidency` RPC (migration `20260901180000`): a compare-and-set on `needs_president` (true → false) in the same transaction as the role grant, so two eligible members racing to claim resolve to exactly one winner — the loser gets `409 Conflict`. An ineligible caller gets `403`; a chapter that doesn't currently need a President gets `400`. The RPC additionally re-verifies, atomically with the grant, that the claiming member still holds the role that made them eligible — the application-layer eligibility check and the RPC call are not the same transaction, so a concurrent `PATCH /v1/members/:id/roles` could otherwise strip that role from the claimant in between and still see the grant succeed.
