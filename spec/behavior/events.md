...
current channel — an immutable snapshot (name, when, location, point value) whose live checked-in count is
read back through the attendance query. The card offers a **Check-in** action during the event window
(start → end + grace), so the chat card is an *additional* self-service check-in surface alongside the
mobile app; the server enforces the window, the role gate, and one-check-in-per-member regardless of
surface. Recurrence, role-targeting, and the mandatory flag stay on the dashboard.
