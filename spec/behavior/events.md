...
- **Recurrence edit and cancellation scope.** `PATCH /v1/events/:id` accepts an optional `scope` in the body and `DELETE /v1/events/:id` an optional `scope` query parameter, each one of `instance` (the default) or `series`. There is deliberately no "this and past" — the rule above allows individual edits and future-only series changes, and nothing else.
  - **`instance`** touches exactly one row, which is the pre-existing behavior of both verbs and what a caller that sends no `scope` still gets.
  - **`series`** resolves to the series parent first — so the operation behaves the same whichever occurrence the client happens to hold — and then applies only to occurrences that have not yet started. **An occurrence whose `start_time` has passed is never modified and never deleted.** That boundary is evaluated server-side from `start_time`, never from a flag the caller sends, because `event_attendance.event_id` is `on delete cascade`: deleting a past occurrence would destroy the record of a meeting that happened and everyone marked at it.
  - On a `series` update, a change to `recurrence_rule`, `start_time` or `end_time` **regenerates** the future occurrences from the new rule; any other field is patched onto them in place. The comparison is by value, not by presence — a client that PATCHes the whole event back with an unchanged rule does not trigger a regeneration.
  - **Deleting a series parent with `instance` scope promotes the next surviving occurrence** into the series head and re-points its siblings at it. `events.parent_event_id` is `on delete set null`, so without that promotion cancelling the first occurrence would silently strand the rest of the series as unowned rows that no later series operation could reach.
