...
- **Runtime enforcement (wired keys).** Workflow toggles are chapter policy, not just presentation. Domain services read the merged state (seed default overlaid with the chapter's `chapter_workflows` row) via `ChapterWorkflowsService`; a read failure falls back to the seed default, so policy still applies. Wired so far:
  - **`wf_hours_receipt`** — when enabled, `POST /service-entries` rejects a submission without `proof_path` (400 with an actionable message). Approval of pre-existing proof-less entries stays at the reviewer's discretion. The web submit form marks the proof field required when the toggle is on.
  - **`wf_dues_grace`** — when enabled, `GET /invoices/overdue` counts an OPEN invoice as overdue only once `due_date` plus the grace period has passed. The period is the workflow `threshold` when the chapter set one, else `chapter_dues_config.grace_days` (Dues tab), else the seed default (7 days). Disabled → no grace (overdue the day after `due_date`).
  - The remaining catalog keys stay presentation-only until their domain surface is wired; wiring each is tracked as its own issue.
