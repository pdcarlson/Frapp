...
- **Audit-write on save:** a settings PATCH that **changes something** writes a row to `chapter_audit_log`, and so do the logo routes, custom roles and custom fields. Each audit row is created `member_visible = true` and is mirrored to the `#chapter-audit` channel via the audit bridge. Not every writer keeps to one row per *effective change*: the next two bullets say where they differ. Not every Settings write is audited: the Semester tab's rollover (`POST /v1/chapters/current/rollover`, including any New Member promotion) writes no row.
