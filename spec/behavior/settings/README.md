...
- **Analytics opt-out.** A single toggle writes the `chapters.analytics_opt_out` scalar through the config PATCH mutation (so it is audit-logged like every other config change). The switch is framed positively ("Chapter analytics" on/off) to avoid a double-negative — *checked = analytics enabled = `analytics_opt_out` false*. Default is opt-in (analytics on); onboarding discloses this. The same flag gates PostHog session replay ([`observability.md` § Privacy and replay](../observability.md#privacy-and-replay)).
...
- **Audit-write on save:** the core-profile and config PATCHes, the logo routes, custom roles (`chapter_custom_roles`) and custom fields write a row to `chapter_audit_log` when they **change something**. Each audit row is created `member_visible = true` and is mirrored to the `#chapter-audit` channel via the audit bridge. Not every writer keeps to one row per *effective change*: the next two bullets say where they differ. Not every Settings write is audited. On the Roles tab, `POST /v1/roles`, `PATCH`/`DELETE /v1/roles/:id` and `POST /v1/roles/transfer-presidency` write no row: every role create, edit or delete (including the non-system roles the Manage roles section calls custom roles), every permission-matrix toggle and a presidency transfer ([#2599](https://github.com/pdcarlson/Frapp/issues/2599)), and neither does the Semester tab's rollover (`POST /v1/chapters/current/rollover`, including any New Member promotion; [#2596](https://github.com/pdcarlson/Frapp/issues/2596)).
- **Writers differ on a no-op save.** The core-profile `PATCH /chapters/current` writes **no** row when its diff is empty. `PATCH /chapters/:id/config` returns early only when its *update payload* is empty, which is not the same test: it assigns `branding`, `vocabulary`, `enabled_modules` and `beta_config` whenever the DTO merely carries the key, so a client that re-sends an unchanged jsonb object still gets a row whose `diff` has `from` equal to `to` (#1605). The custom-role and custom-field `PATCH` routes behave like the config route: only an empty patch returns early, so re-sending an unchanged field writes an `_updated` row. The logo routes split: removing a logo that was never set writes nothing, and every confirm writes a row, `from` equal to `to` included ([`chapter-config.md` § The logo routes](../chapter-config.md#the-logo-routes) says why). Do not assume a row means something changed, and do not assume a save without changes produced no row.
- **Audit writes are not transactional with the mutation they describe** (#1599). The row is inserted after the change commits, so an insert failure surfaces as a `500` on a change that persisted. Whether an identical retry then recovers the record depends on the writer. For example, on the core-profile route and a logo removal the retry changes nothing, so nothing is written and the change stays unaudited; a retried custom-role or custom-field create or delete is refused (`409`, `404`) and writes nothing; on the config route a re-sent jsonb field, and on a logo confirm the retry itself, writes a row whose `from` equals its `to`; a retried custom-role or custom-field update writes an `_updated` row whose snapshots differ only in `updated_at`, which a trigger sets on every update. Do not build a compliance check on the assumption that every mutation has a row.
