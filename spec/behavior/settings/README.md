...
- **Audit-write on save:** the core-profile and config PATCHes, the logo routes, custom roles and custom fields write a row to `chapter_audit_log` when they **change something**. Each audit row is created `member_visible = true` and is mirrored to the `#chapter-audit` channel via the audit bridge. Not every writer keeps to one row per *effective change*: the next two bullets say where they differ. Not every Settings write is audited. The Roles tab's role and permission changes (`POST`/`PATCH`/`DELETE /v1/roles`, `POST /v1/roles/transfer-presidency`) write no row ([#2599](https://github.com/pdcarlson/Frapp/issues/2599)), and neither does the Semester tab's rollover (`POST /v1/chapters/current/rollover`, including any New Member promotion; [#2596](https://github.com/pdcarlson/Frapp/issues/2596)).
