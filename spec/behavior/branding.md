...
- On mobile the boundary is the route group: the `(tabs)` group is chapter context and carries chapter branding; the `(auth)` group sits outside it and stays Frapp-branded. The reason is the absence of a resolved chapter rather than the absence of a session: `(auth)` also holds `join` and `welcome`, which `lib/auth-gate.ts` routes *authenticated* members to, plus `chapter-picker` and `create-chapter`, which they reach deliberately rather than by the gate.
