...
- **Not yet covered: generated report exports.** Branded PDF reports are stored
  in the private `reports` bucket and are never reaped, and roster exports embed
  member names, emails, roles, and join dates. Nothing in the deletion flow
  sweeps them today, so a deleted user's details survive in any report exported
  before their request. Tracked in FRA-338; see
  [`reports.md` § Retention](reports.md#retention).
