...
- Members can still log in and view all existing data (chat history, Backwork, points, events, etc.) but cannot create new content, mint invites, or perform write operations on any `ChapterGuard`-protected route.
  - **Invite redemption honours the lock too (#1546, closed 2026-09-06).** `POST /v1/invites/redeem` carries no `ChapterGuard` — the chapter that matters is the invite's, which the guard never sees — so `InviteService.redeem` evaluates the same hard-lock rule itself (`isSubscriptionHardLocked`: `canceled`, or `past_due` past the 3-day grace window) against `invite.chapter_id` and refuses with **403** (`chapter.subscription.canceled` / `chapter.subscription.write_locked`) *before* consuming the token, so the same link works again once the chapter recovers. A `past_due` chapter inside grace still admits members, matching the guard's free-tier posture during grace. The remaining unguarded writes (`PATCH /v1/notifications/preferences`, `POST /v1/analytics/events`) are deliberately outside the lock: they are the member's own settings and telemetry, not chapter content.
...
reported content, because a report is moderation history the reported party must not be able to
erase — a member who could delete the record of a report by closing their account would have a
one-tap way to clear the queue against them. **Blocks other members placed against them are also
kept**: chat messages are preserved, so purging those rows would let a reported member clear every
block standing against him by deleting his account, and his whole history would render in the clear
again for the people who blocked him. **Reports filed *about* them are kept too**, for the same
reason and with the same consequence worth stating plainly: a report carries a snapshot of the
reported message, so a departing member's own words survive in it — including where they had
soft-deleted the message, which is precisely what the snapshot exists to defeat, and where the
...
text.
- Current-state data is deleted outright: chapter memberships (and their custom-field values), user settings, push tokens, notifications, both notification-preference tables, read cursors, chat bookmarks, chat member blocks, and study sessions (location-presence history; the points they yielded remain in the preserved point transactions). Chat blocks are purged on the **blocker side only** — the member's own safety list goes, blocks other members placed against them stay, for the reason in the bullet above. Rush ballots the member cast are purged too (#494). The `delete from` block in the `anonymize_user` RPC is authoritative — it has grown since, for chat bookmarks in `20260902160000` and for chat member blocks and rush ballots in `20260915210100` — so check it rather than this sentence before relying on the list being exhaustive.
...
- A sole President who deletes their account leaves the chapter without a President (deletion is never blocked on role). The chapter is flagged rather than stranded, and this has shipped: `AccountDeletionService` calls `RbacService.flagIfPresidentRemoved`, which sets `chapters.needs_president` and writes a `president_orphaned` audit row, and the next-highest-ranked officer claims the role through `POST /v1/roles/claim-presidency`. The flow, its eligibility walk and its safeguards are owned by [`rbac.md`](rbac.md#presidency-transfer) § Presidency Transfer — including the residue, where `resolveEligibleClaimants` finds nobody (every role above the chapter's Member role vacant, or that Member role unresolvable) and Signet support still has to step in.
...
- **Chapter activity is nearly useless as a stand-in for the login clock.** A canceled chapter is
  hard-locked read-only — `subscriptionWriteState` returns `canceled`, which outranks even the
  free-tier carve-out (`packages/validation/src/subscription.ts`). So members
  cannot write `chat_messages`, `point_transactions`, `events`, `attendance` or uploads once
  cancellation lands, and those timestamps stop moving for reasons that have nothing to do with whether
  anyone is still there. **One narrow exception, and it matters:** `ChapterGuard.enforceSubscription`
  checks `@SubscriptionExempt` *before* the `canceled` branch, and the dues-payment route
  (`apps/api/src/interface/controllers/financial-invoice.controller.ts`) is exempt precisely so it
  "must stay reachable when the chapter subscription is past_due/canceled". A canceled chapter still
  settling invoices therefore does emit timestamped writes — evidence that someone is *paying*, which
  is not the same question as whether the chapter is alive.

That is the crux rather than a detail: **cancellation removes almost every trace a living chapter would
otherwise leave, so a login record is the only general signal that could separate a dormant chapter
from one being read every week — and it is not recorded in a usable form.** Until it is, an eligibility
rule is inferring abandonment from the absence of activity the product itself forbids, with only the
exempt billing path as a partial and badly-aimed counter-signal.
...
- **Storage is outside the cascade entirely.** Buckets key on `chapters/{chapterId}/…` — some via
  `apps/api/src/domain/constants/storage.ts`, others built inline in their own services, so enumerate
  them rather than trusting one constants file — and nothing reaches those objects from a row delete.
...
- **Recipient resolution — reuse `RbacService.findPresidentRole`, do not write a new lookup.** The
  obvious `roles.system_key = 'PRESIDENT'` query is rename-proof only *forward*: that migration's
  backfill skips any chapter that had already renamed the role
  (`supabase/migrations/20260806220000_role_system_key.sql`), and dormant chapters are more likely than
  average to predate it. This is already solved — `findPresidentRole`
  (`apps/api/src/application/services/rbac.service.ts`) resolves by `is_system` plus the wildcard
  permission precisely because of that gap, and says so in its own comment.
...
- **Who is allowed to run any of this.** There is **no platform-administrator role, guard, or column
  anywhere in the API** — authorization is built around chapter-scoped membership, and the routes that
  sit outside it (the Stripe webhook, for instance) are machine callers authenticated by signature, not
  humans with cross-chapter authority. A cross-chapter sweep and an operator-triggered deletion have
  **no home in the current authorization model**, and "operator" elsewhere in this file means a human
  with service-role access, not a role the
...
- Deletion is executed **per chapter**, never as a bulk operation that can run away.
- **The audit record cannot live in `chapter_audit_log`.** That table is `on delete cascade` from
  `chapters` (`supabase/migrations/20260523130000_audit_log.sql`), so a row written to record a hard
  delete is removed by the very delete it documents. The one action most needing a durable record is
  the one the existing audit sink cannot keep, and no cross-chapter audit destination exists today.
...
Two of the missing pieces are independently useful and are being built regardless of how the open
questions resolve:

- **A cancellation timestamp on `chapters`**, set and cleared by the billing webhook, respecting the
  `last_stripe_webhook_at` ordering guard above so a retried event cannot stamp a date the billing
  layer rejected (#1559).
- **A general send method on `IEmailProvider`.** A Resend transport is already wired and
  config-selected (`apps/api/src/modules/email/email.module.ts`); what is missing is a way to send
...
- A client that needs its own pseudonymous id (e.g. to initialise a provider SDK) fetches it from `GET /v1/analytics/identity`. That response also carries the caller's **chapter-group** pseudonym when a chapter is in context (`chapter_group_id`, else `null`), so clients never hash. The identifier table is owned by [`observability.md` § Correlation schema](observability.md#correlation-schema). Chapter opt-out still suppresses **events**; identity still returns the digests so Sentry `user.id` can be set. `apps/web` and `apps/mobile` use those digests for PostHog `identify` / `group('chapter', …)` and may emit the content-free `sentry-error-correlated` marker; named product events still go through `POST /v1/analytics/events`. Replay rules stay in [`observability.md`](observability.md#privacy-and-replay). Account-deletion forget is [#709](https://github.com/pdcarlson/Frapp/issues/709) and is not complete until that automation is live.
