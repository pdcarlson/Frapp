...
- File uploads are checked for allowed MIME types and extensions via `@repo/validation` before a signed URL is issued; storage buckets enforce the same MIME list and a size cap on the upload itself. Kinds and caps: [`content-validation.md`](../../docs/internal/security/content-validation.md#1-allowed-content-types); per-bucket limits: [`spec/architecture/README.md` § 7](../architecture/README.md#7-storage-supabase-storage).
