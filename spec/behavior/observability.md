...
Caller-supplied text reaching a log message goes through `logSafe` (`apps/api/src/infrastructure/observability/log-safe.ts`) first. Records are one JSON object per line, so an unescaped newline in an attacker-chosen value forges a whole record — including a fabricated `security_event` in the stream the auth-failure spike detector reads.
