...
- **A 5xx thrown from a `catch` carries what it caught as `cause`.** A catch that turns a provider failure into a generic `ServiceUnavailableException` (or `BadGateway`, `InternalServerError`, `GatewayTimeout`) passes `{ cause: toReportableError(error) }`. The client still gets only the generic message: `AllExceptionsFilter` serializes `statusCode`, `error`, `message` and `requestId`, never the cause. Sentry's `LinkedErrors` integration, which the API keeps from the SDK's default set, ships the cause as a second exception value, and the value allowlist and free-text sweep below apply to each one. Without it, a bad Stripe price, a revoked key and an outage all arrive as one issue whose stack ends at the rethrow ([FRAPP-API-4](https://frapp-live.sentry.io/issues/FRAPP-API-4), [#2131](https://github.com/pdcarlson/Frapp/issues/2131)). `rethrown-server-error-cause.spec.ts` fails on any such throw without a `cause`.
