...
**Correction (2026-09-22):** The upload is best effort on every path, not only when the token is unset. With the token set, a `sentry-cli` inject or upload that cannot run or exits non-zero logs one `WARNING:` line (step, exit status, signal, spawn error), still strips `*.map`, and the image build succeeds: symbolication is telemetry and must not gate shipping the API. Only a failure to strip `*.map` fails the build. This was decided after the token reached the Render build and every `frapp-api-staging` build failed at `inject failed (null)`, because the script spawned a root `node_modules/.bin/sentry-cli` that the Docker deps stage never installs; it now resolves the `@sentry/cli` that `apps/api` declares (#2431).
