...
Caller-supplied text reaching a log message goes through `logSafe` (`apps/api/src/infrastructure/observability/log-safe.ts`) first. The API uses Nest's default console logger, so a record is a `[Nest] <pid> - <ts> <LEVEL> [<Context>]` prefix followed by the message — which for the request log and for `security_event` is a JSON body. Either way a record is **one line**, so an unescaped newline in an attacker-chosen value splits it and lets the caller write a line of their own choosing into the stream an incident investigation reads. A forged line is not indistinguishable from a genuine one — it carries no `[Nest]` prefix, and `security_event` is emitted at `warn` while the injectable sites log at `log` — but a stream that a caller can write arbitrary lines into is not one worth reasoning from.
