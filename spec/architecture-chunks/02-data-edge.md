...
1. `spec/redesign-context.md` — sections *Data model*, *API surface*, *System architecture for the chat hot path*, *Theming model*.
...
7. **`spec/redesign-context.md` → *Engineering principles*.** Non-negotiable for every chunk; the bullets below are this chunk's specific applications.
...
18. `spec/behavior/chapter-config.md` and `spec/behavior/chat/README.md` — chapter config endpoints, directory search shape, chat message kinds + actions.
19. `spec/product/modules.md` — module catalog (always-on vs paid).
