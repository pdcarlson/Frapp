...
- **Enforcement:** Subscription state gates **writes**, applied by `ChapterGuard` at the request boundary. Its refusals are **403** — the API returns no 402 anywhere — though `BillingService` layers its own **400** refusals on top (a duplicate checkout while a live subscription exists, or a portal session for a chapter with no Stripe customer yet). A free-tier wedge (the `@FreeTier()` routes — chat / members / **invites** among them) keeps writing while a chapter is `incomplete`, so a chapter fresh out of the onboarding wizard can invite before it has ever paid. A `past_due` chapter gets a 3-day grace window in which invite _minting_ is blocked while its other free-tier writes continue; after that window, and for `canceled`, **guarded** writes are blocked. Two things deliberately survive that lock, so "read-only" describes the guarded surface rather than the whole API: `@SubscriptionExempt()` routes, which cover **billing recovery and member safety** — the billing endpoints, so a lapsed chapter can pay its way back, and chat report/block (#2257), because App Store Guideline 1.2 has no billing exception and a member being harassed in a chapter whose card failed needs those controls exactly as much as one in a paying chapter; and the two routes carrying no `ChapterGuard` that are the member's own settings and telemetry — notification preferences, analytics events — which were never subscription-gated. Invite redemption carries no guard either but applies the lock in the service against the invite's chapter (#1546). Full matrix, including the exact codes and the `GET`/`HEAD`/`OPTIONS` definition of a read: [`docs/guides/api-architecture.md`](../../docs/guides/api-architecture.md) § Subscription enforcement (ChapterGuard).
...
- Exported PDFs use a branded template with chapter name, logo, and Frapp footer. Layout: [`../behavior/reports.md`](../behavior/reports.md) § PDF Formatting.
