...
- **Enforcement:** Subscription state gates **writes**, applied by `ChapterGuard` at the request boundary. Refusals are **403** — the API returns no 402 anywhere. A free-tier wedge (ten controllers, chat / members / **invites** among them) keeps writing while a chapter is `incomplete`, so a chapter fresh out of the onboarding wizard can invite before it has ever paid. A `past_due` chapter gets a 3-day grace window in which invite creation specifically is blocked while its other free-tier writes continue; after that window, and for `canceled`, the chapter is read-only apart from the `@SubscriptionExempt()` billing-recovery routes (Checkout, Customer Portal, dues payment-intent) — a lapsed chapter must always be able to pay its way back. Full matrix, including the exact codes and the `GET`/`HEAD`/`OPTIONS` definition of a read: [`docs/guides/api-architecture.md`](../../docs/guides/api-architecture.md) § Subscription enforcement (ChapterGuard).
