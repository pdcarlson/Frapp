...
**The wizard finishes on `/billing`, not chat (#2297).** The target used to read "to first chat message", which the wizard served literally by landing on `/chat?channel=general` — but a just-created chapter is `subscription_status 'incomplete'`, so the officer arrived somewhere every screen loads and every paid-ops *write* is refused. Checkout is the step that clears it, and #913's 14-day trial makes it free on day zero, so it is on the path rather than after it. This is still a **landing and not a gate** — nothing blocks navigating to chat, which is one click away in the nav — so the "inline nudges rather than a mandatory gate" rule above is unchanged. The paired `activation-first-chat-message` milestone in [`../behavior/observability.md`](../behavior/observability.md) is unchanged and still measures the first human message; expect it to trail signup by the time the officer spends on checkout, which is the honest reading rather than a funnel regression.
