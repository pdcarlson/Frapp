...
- **$149 per chapter / month, USD, flat.** One price for the chapter regardless of member count — no per-seat component and no quantity tiers. This is the number the public site sells (the pricing section — `id="pricing"` — in [`apps/landing/app/page.tsx`](../../apps/landing/app/page.tsx), and the page's JSON-LD offer), so it is a commercial commitment, not an internal default: moving it means moving the site in the same change.
- The flatness is load-bearing in the code, not only in the positioning. `StripeBillingService.createCheckoutSession` builds a single line item at `quantity: 1` against one `STRIPE_PRICE_ID`, so per-seat billing, quantity tiers, and any per-chapter price variation are all code changes, not a different Price object. Treat "swap the Price ID" as able to change *the amount* and nothing else.
...
The first-officer flow is school search → chapter search → archetype confirm → invite members. A new officer types something like "Sigma Phi Epsilon @ UCLA" and the wizard autofills chapter identity (Greek letters, designation, school short, founded year, default colors) from a curated Greek-life directory. Target: under 90 seconds from signup to a chapter the officer can use, for a chapter that exists in the directory. Subsequent ops setup (enabling paid modules, configuring dues, setting workflows) is optional and surfaced as inline nudges rather than a mandatory gate.

**The wizard finishes on `/billing`, not chat ([#2297](https://github.com/pdcarlson/Frapp/issues/2297)).** The target used to read "to first chat message", which the wizard served literally — but a just-created chapter is `incomplete`, so the officer arrived somewhere every screen loads and every paid-ops *write* is refused. Checkout is on the path now rather than after it. It remains a landing and not a gate, so the "inline nudges rather than a mandatory gate" rule above is unchanged. Destination, rationale and the store constraint that keeps checkout web-only are owned by [`../behavior/onboarding.md`](../behavior/onboarding.md) § First-Officer Onboarding Wizard — not restated here. The paired `activation-first-chat-message` milestone in [`../behavior/observability.md`](../behavior/observability.md) is unchanged and still measures the first human message; expect it to trail signup by the time spent on checkout, which is the honest reading rather than a funnel regression.
...
Do not duplicate brand values here. **Signet** (dark-first, warm, Figtree, house gold) is specified in [`spec/ui/brand-identity.md`](../ui/brand-identity.md); token values live in [`spec/ui/design-system/foundations.md`](../ui/design-system/foundations.md). The landing site shipped the legacy Frapp look until the token cutover ([#2366](https://github.com/pdcarlson/Frapp/issues/2366)) merged, and is now on the Signet tokens in its staged reskin ([#2364](https://github.com/pdcarlson/Frapp/issues/2364)) — see brand-identity §5; the web dashboard cut over with the #920 shell slice. Touch targets ≥ 44px: [`foundations.md`](../ui/design-system/foundations.md). Haptics on confirming actions: [`spec/ui/mobile/README.md`](../ui/mobile/README.md) (native-feel table). Chat swipe-to-reply / swipe-to-archive is **not** in the Signet inventory (s05 renders a quote for a web-authored reply; it does not author one — [`screens.md`](../ui/mobile/screens.md) s05) — do not resurrect NativeWind-era gesture copy from older positioning drafts.
