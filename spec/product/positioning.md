...
**The wizard finishes on `/billing`, not chat.** The target above used to read "to first chat message", which the wizard served literally; it no longer does. Why, and the store constraint that keeps checkout web-only, are owned by [`../behavior/onboarding.md`](../behavior/onboarding.md) § First-Officer Onboarding Wizard. It remains a landing rather than a gate, so the "inline nudges rather than a mandatory gate" rule above still holds. The paired `activation-first-chat-message` milestone in [`../behavior/observability.md`](../behavior/observability.md) is unchanged and still measures the first human message; expect it to trail signup by the time spent on checkout, which is the honest reading rather than a funnel regression.
