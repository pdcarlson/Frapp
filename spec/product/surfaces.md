...
- Pricing: **both tiers are what the page owes.** The free tier — unlimited chat, members and chapters, no card — is the wedge, and Chapter Pro is a flat monthly per-chapter plan. Amounts and what each tier gates are [`positioning.md`](positioning.md), which is canonical; the page sells them and does not set them. **As built the page shows only the single paid card** — stating the free tier is decision D3 of the reskin and lands with the page rebuild ([#2367](https://github.com/pdcarlson/Frapp/issues/2367)); the gap and why it is not fixed by editing prose are [`spec/ui/landing/README.md`](../ui/landing/README.md) § Pricing truth.
- **No unverified metrics and no testimonials.** A stats row and three testimonial quotes ship on the page today with no source behind them; the reskin deletes both ([#2367](https://github.com/pdcarlson/Frapp/issues/2367)), and neither returns. Any figure or quote added later **must** reflect verified metrics and real customers — a name, a chapter and consent — or be clearly labeled as illustrative until validated (see [`spec/ui/brand-identity.md`](../ui/brand-identity.md)).
- CTA: "Get Started" (redirects to `app.frapp.live/sign-up`) and "Log In" (redirects to `app.frapp.live/sign-in`). These must match the web app's actual auth routes — there is no `/signup` or `/login` route. An empty, whitespace, or scheme-less `NEXT_PUBLIC_APP_URL` falls back to that origin at request time rather than 500ing `/` ([#1777](https://github.com/pdcarlson/Frapp/issues/1777)); operator copy lives in [`docs/internal/environment/ENV_REFERENCE.md`](../../docs/internal/environment/ENV_REFERENCE.md).
