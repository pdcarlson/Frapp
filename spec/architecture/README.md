...
| `@repo/observability`      | Browser-safe observability policy: Sentry PII scrubbing, correlation types, sample-rate parsing in `[0,1]`, and the constants that describe the Sentry/PostHog split. Identify / groups / hex validation / Sentry correlation attach are `@repo/observability/identified-posthog` (web and mobile only). Landing uses `@repo/observability/next` for anonymous page/CTA capture. Vendor SDK init stays runtime-local to NestJS, Next.js, and React Native. Used by API + web + mobile + landing. |
