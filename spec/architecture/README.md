...
- **Security:**
  ...
  - `ChapterGuard`: Resolves the active chapter — the JWT `active_chapter_id` claim is authoritative, `x-chapter-id` is a legacy fallback that never overrides it, and a disagreement is a hard `403 chapter.context.mismatch` — then verifies the user belongs to it. Precedence is owned by [`multi-tenancy.md`](../behavior/multi-tenancy.md).
...
**chat_message_attachments** — `id`, `message_id` (FK chat_messages, ON DELETE CASCADE), `channel_id` (FK chat_channels — denormalised so the row's chapter is one hop away, exactly as `chat_messages` reaches it), `bucket`, `storage_path`, `filename`, `content_type` (nullable), `byte_size` (bigint, nullable), `width` / `height` (int, nullable), `external_url` (nullable — a source-system URL, and **always null for Discord-imported rows on both import paths**. For the *export* path the link does not exist to store: run with `--media`, DiscordChatExporter has already rewritten every URL to an export-relative path. For the *bot* path it does exist — `DiscordExportWorkerService` reads `attachment.url` straight off the Discord REST response — and is deliberately **not** persisted, because a Discord CDN link is signed and expiring, so storing one is a private-bucket bypass that outlives its own signature. Retrying a failed media fetch is "re-upload and re-run", which `discord_import_files` already expresses. The column is also a disclosure boundary, stripped by `stripAttachmentRow` on every exit from the repository), `created_at`. Unique on `(message_id, bucket, storage_path)` — **one row per object *per message***, not per object: the sigil the backfill replaced was editable text, so a member quoting another message's body produced a second message naming the same object, and a `(bucket, storage_path)` key would have silently dropped it. RLS enabled with **no policies** (default deny): the table is not a Realtime carrier and is read only by the API on the service-role key.
...
Seed materialization deep-clones the shared archetype seeds into the chapter's rows so per-chapter edits never mutate the shared reference — the rule, and why a shallow spread is not enough, are in [`engineering.md`](../engineering.md) § Seeds and shared state. **The roster is `buildChapterConfigFromArchetype` (`packages/org-archetypes/src/index.ts`), not a list in either doc:** it clones `archetype.modules`, `VOCABULARY_DEFAULTS`, `CUSTOM_FIELDS_SEED`, `WORKFLOWS_SEED` and `DUES_CONFIG_SEED`. It does **not** copy `ROLE_PACKS` at all — it stores `archetype.rolePack`, a string key — so the clone rule has simply not come up on that path yet. Anything that *does* materialize a pack still owes it a deep clone, which is `engineering.md`'s rule and names `ROLE_PACKS` explicitly. `getRolePack` returns the shared array by reference, and the freeze protecting it is one level deep: `ROLE_PACKS` and each pack array are frozen, the `RoleEntry` objects inside them are not. TypeScript's `readonly` fields are what stop a careless write today, so the runtime hazard is reachable from JavaScript or through a cast, and it would corrupt the pack for every chapter sharing that pack key.
...
- `chat_messages` gains `kind text not null default 'text'`, `payload jsonb` (inline-card data), `client_message_id text` (client-generated idempotency key), and `deleted_at timestamptz` (soft-delete; hard delete is admin-only cold path). The kind roster is owned by [`chat/README.md`](../behavior/chat/README.md) § Message Kinds and Actions, which also marks the kinds that are specified but not yet in the enum. The column carries no CHECK constraint, so adding a kind is a code change — and `CHAT_MESSAGE_KINDS` is declared in **three** places, each failing differently if missed: `apps/api/src/domain/entities/chat.entity.ts` is the live send gate (behind `@IsIn(...)` in `chat.dto.ts`, so missing it means the API rejects the send), `packages/chat-core/src/types.ts` drives `coerceKind` (missing it silently rewrites the row to `text`), and `packages/validation/src/index.ts` is the shared contract for non-Nest consumers. `imported` marks a read-only archive row brought in from another system (Discord); it is server-only, never counts toward unread, never pushes, and is excluded from the `chat_messages` SELECT policy so Supabase Realtime does not fan a bulk import out to connected clients.
...
Configurable alerts via the monitoring provider. The alert set is owned by [`observability.md`](../behavior/observability.md) § Alerting; routing and severity by [`ALERT_ROUTING.md`](../../docs/internal/ops/ALERT_ROUTING.md).

Do not restate a duration or percentage here. The ground truth is a human-configured provider dashboard, so a number written in this file is backed by nothing and cannot be checked — this section previously carried "health check fails for >1 minute", a figure that appears in no other doc and behind which no configuration was ever recorded.

Note that `ALERT_ROUTING.md` § Thresholds is not exhaustive: it holds worked thresholds for push delivery and security events only. Of the rest, error rate carries an illustrative figure in `observability.md` § Alerting (">5% 5xx in 5 minutes") and the others carry none. The two lists also do not line up item for item — `observability.md` names connection-pool exhaustion where `ALERT_ROUTING.md` § Critical alerts names database latency saturation — so reconcile against those two files rather than assuming this summary is complete.
...
- Per-channel / per-kind notification preferences (`all | mentions | off`). The default level for each channel and kind, and the precedence between them, are owned by [`notifications.md`](../behavior/notifications.md) § Chat notification preferences, which tracks `defaultLevelFor` in `apps/api/src/modules/chat-push-worker/push-rules.ts`.
...
**Consequences:** Two preference tables until consolidation. The push worker queries both arms in a single `eq(user_id).eq(chapter_id)` load per recipient and resolves precedence locally (channel-pref ▶ kind-pref ▶ channel-name default). The defaults `(announcements → all, chapter-audit → off, system_audit kind → off, otherwise mentions)` live in `apps/api/src/modules/chat-push-worker/push-rules.ts:defaultLevelFor` so the rule chain is unit-testable without DB seeds. *(Corrected 2026-09-05: `defaultLevelFor` has since grown a fifth arm, `imported → off`, which this ADR's recorded set predates. The current set is owned by [`notifications.md`](../behavior/notifications.md) § Chat notification preferences — read it there rather than from this decision record.)*
...
The Expo app opens directly into chat and holds real-time parity with web on reactions, inline cards, and presence. **Voice memos and background sync on silent push are specified but not built** — see their subsections below before planning against them. The hot-path client and realtime manager are shared across platforms as `@repo/chat-core`, with the platform-specific layers injected through its adapter ports (`KeyValueStore`, `NetworkState`, `OutboxStore`) rather than forked; the rich-message renderer registry is shared as a contract (`@repo/chat-integrations`) with framework-bound renderers per app.
...
### Background sync on silent push — specified, not built

Target state: a silent push handler wakes the app to sync new messages when the websocket isn't alive, keeping unread counts honest without a foreground session, with burst bundling and presence-aware suppression matching the web push rules (ADR-04, ADR-09).

**None of it ships today, and the client config is deliberately the other way.** `UIBackgroundModes` is not set in `apps/mobile/app.json` — that flag is what a silent push needs — and the `expo-notifications` plugin declares a single `defaultChannel` (`"default"`), so there is no per-category channel split on Android and no corresponding iOS Notification Center grouping. Both are owned by [`patterns.md`](../ui/mobile/patterns.md) § Push notifications, which states what the config sets and what it deliberately omits. Push is in any case unavailable in every build that exists today, for want of an EAS project (#938).

### Voice memos — specified, not built

Target state: the mobile composer records a voice memo, uploads it to Supabase Storage (pre-signed upload, same flow as other attachments), and sends it as `kind="audio"` with waveform metadata in `payload`; web renders the `audio` card with waveform playback from the shared renderer, so a memo recorded on mobile plays back on web. **`audio` is not yet in `CHAT_MESSAGE_KINDS`**, so none of this is reachable — see [`chat/README.md`](../behavior/chat/README.md) § Message Kinds and Actions, which owns that distinction.
