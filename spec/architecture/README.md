...
- `enabled_modules jsonb not null default '{"chat":true,"members":true,"announcements":true,"audit-log":true}'` — per-module on/off map. A module is enabled unless its key is explicitly `false`. The free set is always-on only in the UI — the Settings toggle is locked for it, while the config PATCH does not reject a disable ([`../behavior/integrations.md`](../behavior/integrations.md)). That set (`chat`, `members`, `announcements`, `audit-log`, `chapter-settings`) cannot be disabled. See [`product/modules.md`](../product/modules.md).
