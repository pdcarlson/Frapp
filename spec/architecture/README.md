...
- **A `frapp-prod` data-loss event is unrecoverable until stage 5.** `frapp-prod` is
  ...
  `ls supabase/migrations/*.sql | wc -l`. Re-read 2026-09-06: production still held 54 (newest
  `20260829002000`) while staging and the tree held 74 — the dated reads live in
  `DB_PROMOTION_RUNBOOK.md`, not here. #1620 tracks refreshing this and the matching block in
