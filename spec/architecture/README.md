...
- **pgvector vs. the PGlite migration gate — settled, no constraint on the corpus migrations.** `scripts/check-pglite-migrations.mjs` is a CI gate requiring every migration to replay under PGlite. PGlite ships pgvector as a first-party bundle (`@electric-sql/pglite/vector`, **pgvector 0.8.1** under the pinned `@electric-sql/pglite@0.4.6`), so the gate registers it on the PGlite constructor alongside `pgcrypto` and `create extension vector` replays normally. Verified: typed `vector(n)` columns, the `<=>` distance operators, and **both** ANN index access methods (`hnsw` and `ivfflat`) all work under the harness. Write the corpus migrations against pgvector as you would for hosted Postgres — no conditional extension creation, no gate carve-out, no shim.
  ...
  - Two things this decision does **not** cover. `check:migration-safety` rejects any change set touching `supabase/migrations/` without a matching update to `docs/internal/ops/DB_PROMOTION_RUNBOOK.md`, `docs/internal/ops/DB_ROLLBACK_PLAYBOOK.md`, or `spec/environments/` — budget for that doc update in the corpus migration PR. And PGlite's bundled pgvector version is independent of the hosted Supabase one; if a corpus migration ever depends on version-gated pgvector behavior, confirm the hosted version rather than inferring it from the gate passing.
