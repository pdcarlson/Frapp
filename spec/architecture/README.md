...
Do not restate a duration or percentage here. The ground truth is a human-configured provider dashboard, so a number written in this file is backed by nothing and cannot be checked — this section previously carried "health check fails for >1 minute", a threshold that appears in no other doc and behind which no configuration was ever recorded. Note that `ALERT_ROUTING.md` § Thresholds is **not** yet complete either: it covers push delivery and security events only, so error rate, API downtime, connection-pool exhaustion and Stripe webhook failures currently have a named alert and no written threshold anywhere.
...
**Consequences:** Two preference tables until consolidation. The push worker queries both arms in a single `eq(user_id).eq(chapter_id)` load per recipient and resolves precedence locally (channel-pref ▶ kind-pref ▶ channel-name default). The defaults live in `apps/api/src/modules/chat-push-worker/push-rules.ts:defaultLevelFor` so the rule chain is unit-testable without DB seeds; the current set is owned by [`notifications.md`](../behavior/notifications.md) § Chat notification preferences rather than restated here (this line previously listed four arms and `defaultLevelFor` has five — `imported → off` was missing).
