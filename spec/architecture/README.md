...
| Observability  | Sentry + PostHog + structured logging        | Contract: [`spec/behavior/observability.md`](../behavior/observability.md). Decision: [ADR-22](#adr-22-sentry-is-the-system-of-record-for-exceptions-and-traces-posthog-for-product-analytics). |
...
[ADR-22](#adr-22-sentry-is-the-system-of-record-for-exceptions-and-traces-posthog-for-product-analytics).
...
- **Vendor SDK init stays runtime-local.** Shared policy (scrubber, correlation types, safe env
  ...
  **move** `packages/validation/src/sentry-scrubbing.ts`, do not copy it. That package does
  not exist yet and is not listed under §4.
...
*read* works; *create* does not; Render unread). Product-analytics project state, recorded
here because it is the evidence this decision is not a description of the current PostHog
project:
