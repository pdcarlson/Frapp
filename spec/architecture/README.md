...
- `theme_palette_engine_version integer` — the `SIGNET_ENGINE_VERSION` that wrote `theme_palette`, always written with it (`chapterPaletteColumns`). `NULL` or lower than the running engine's means stale, and is what the sweep selects on (#1165). Never set it without the palette it describes.
