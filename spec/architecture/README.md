...
**chat_message_attachments** — `id`, `message_id` (FK chat_messages, ON DELETE CASCADE), `channel_id` (FK chat_channels — denormalised so the row's chapter is one hop away, exactly as `chat_messages` reaches it), `bucket`, `storage_path`, `filename`, `content_type` (nullable), `byte_size` (bigint, nullable), `width` / `height` (int, nullable), `external_url` (nullable — a source-system URL, and **always null for Discord-imported rows**: the importer never contacts Discord, so there is no link to keep. It is also a disclosure boundary, stripped by `stripAttachmentRow` on every exit from the repository, because a Discord CDN link is signed and time-limited and would route around the private-bucket posture entirely), `created_at`. Unique on `(bucket, storage_path)`. RLS enabled with **no policies** (default deny): the table is not a Realtime carrier and is read only by the API on the service-role key.
...
Seed materialization deep-clones the shared archetype seeds into the chapter's rows so per-chapter edits never mutate the shared reference. Which constants are cloned, and which are instead frozen and shared by reference, is owned by [`engineering.md`](../engineering.md) § Seeds and shared state; `buildChapterConfigFromArchetype` (`packages/org-archetypes/src/index.ts`) is the implementation.
...
Configurable alerts via the monitoring provider. The alert set is owned by [`observability.md`](../behavior/observability.md) § Alerting, and the thresholds that are actually configured — plus which of them are live rules rather than intended ones — by [`ALERT_ROUTING.md`](../../docs/internal/ops/ALERT_ROUTING.md) § Thresholds. Do not restate a duration or percentage here: the ground truth is a human-configured provider dashboard, so a number written in this file is backed by nothing and cannot be checked.
