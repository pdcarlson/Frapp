...
- **Amendment (2026-09-02, #472) — the moved Realtime broadcast emit is deleted; the move never brought its consumer with it.** The Consequences bullet above ("The Realtime broadcast emit previously in `chat-send` (`channel.send`) moved to `ChatService.broadcastNewMessage`") describes what #416 did and is retained as that record. What it did not say, because nobody checked at the time, is that **the client half did not move and had never existed in this form**: the emit published `new_message` to a bespoke `chapter:<channel_id>` topic, while clients join `chat:channel:<id>` (ADR-10) and read messages through `postgres_changes` on `chat_messages`. `git log --all -S"new_message" -- apps/web apps/mobile packages` returns nothing — no client has ever registered a handler for that event. So the emit was dead on **both** halves independently: wrong topic *and* no listener. It cost one HTTP broadcast POST per message and, because `removeChannel()` ran only on the success path, leaked a registered channel on the long-lived service-role client every time `send()` threw. `ChatService.broadcastNewMessage`, `realtimeTopicForChannel` and the now-unused `SUPABASE_CLIENT` injection are removed; `ChatService` no longer depends on the Supabase client at all. **This does not change message delivery**, which was already Postgres Changes and nothing else (`spec/ui/resilience.md` §3.2). The prompt was #472, which asked for the swallowed broadcast failure to be instrumented — a counter on a path that delivers to nobody could never indicate user-visible degradation, so the emit was removed rather than measured. Whether chat should have a *genuine* sub-second broadcast fast-path is a separate, open question (**#1613**); building one needs a client handler, de-duplication against the Postgres Changes echo of the same row, and an amendment to ADR-02, which scopes Broadcast in chat to presence and typing.
