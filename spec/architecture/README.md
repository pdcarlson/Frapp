...
- **Amendment (2026-08-19) — `web-tests` becomes a required check, and the "skip→success wrapper" premise was wrong.** `web-tests` is the **only** suite covering `packages/hooks`, `packages/ui` and `packages/chat-core`; the consolidation work now beginning edits `packages/hooks` directly, so leaving it advisory meant a broken shared hook could merge green. It is added to `CI_CHECKS` in [`scripts/configure-branch-protection.mjs`](../../scripts/configure-branch-protection.mjs).
  ...
  One real caveat from the same table, and it is why this is safe rather than merely lucky: a job skipped because a `needs:` parent **failed** "may not block merging". `web-tests` needs `packages-build` and `changes`; `packages-build` is itself a required check, so a red parent already blocks the PR under its own name and the skip cannot become a hole. Adding `always()` would be wrong here — it would defeat the `cancel-in-progress` saving this ADR exists to bank.
