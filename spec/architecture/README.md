...
| `@repo/formatting`        | Shared date/time/duration display helpers. `parseInstant` is the primitive under all of them — "parse this timestamp, or treat it as absent", returning a `Date` or `null` — and is exported so no surface hand-rolls `new Date(v)` + a `NaN` guard again. It is `new Date(v)`, so it carries the bare-date defect exactly as `formatLocaleDate` does: a bare `date` column still takes a cluster parser, never this. `apps/web/lib/date-call-sites.spec.ts` watches it for that. Generic locale formatters (`formatClock`, `formatLocaleDateTime`, `formatLocaleDate`) plus three **protected clusters** that must stay distinct: stopwatch padding (`formatPaddedStopwatch` / `formatTimer`), bare-date timezone parsing (`parseBareDateLocalMidnight` / `parseBareDateUtcNoon` / `parseInstantOrBareUtcNoon` for a column of mixed shape, plus the cluster's own formatter `formatBareDate` — a `date` column takes that, never `formatLocaleDate`), and minute-duration rounding (`formatMinutesExact` / `formatMinutesRounded`), plus `formatBytes`. Calendar-day arithmetic (`dayDelta`) is here too rather than per-screen — it reads each instant's *local* Y/M/D, which is what every "due tomorrow" and TODAY/EARLIER split wants, so a bare `date` column reaches it through a cluster parser and never through `new Date`. Used by API + web + mobile. |
