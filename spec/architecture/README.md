...
| `@repo/formatting`        | Shared date/time/duration display helpers. `parseInstant` is the primitive under every member that parses a **string** — the three locale formatters and both bare-date parsers — and returns a `Date` or `null`, i.e. "parse this timestamp, or treat it as absent". (The members that take a number or an already-built `Date` — the stopwatch pair, the minute pair, `formatBytes`, `dayDelta` — never reach it.) It is exported so a surface reaches for it instead of hand-rolling `new Date(v)` + a `NaN` guard; only `apps/web` has a gate enforcing that (`apps/web/lib/date-call-sites.spec.ts`), so on mobile and the API it is a convention. It *is* `new Date(v)`, so it carries the bare-date defect exactly as `formatLocaleDate` does: a bare `date` column still takes `formatBareDate`, never this. Generic locale formatters (`formatClock`, `formatLocaleDateTime`, `formatLocaleDate`) plus three **protected clusters** that must stay distinct: stopwatch padding (`formatPaddedStopwatch` / `formatTimer`), bare-date timezone parsing (`parseBareDateLocalMidnight` / `parseBareDateUtcNoon` / `parseInstantOrBareUtcNoon` for a column of mixed shape, plus the cluster's own formatter `formatBareDate` — a `date` column takes that, never `formatLocaleDate`), and minute-duration rounding (`formatMinutesExact` / `formatMinutesRounded`), plus `formatBytes`. Calendar-day arithmetic (`dayDelta`) is here too rather than per-screen — it reads each instant's *local* Y/M/D, which is what every "due tomorrow" and TODAY/EARLIER split wants, so a bare `date` column reaches it through a cluster parser and never through `new Date`. Used by API + web + mobile. |
...
**channel_read_receipts** — `id`, `channel_id` (FK chat_channels), `user_id` (FK users), `last_read_at` (timestamp), `hidden_at` (nullable timestamp: when this member hid a 1:1 DM from their own list, [`chat/README.md` § Direct Messages](../behavior/chat/README.md#direct-messages)), `updated_at`. Unique on (channel_id, user_id).
...
- `theme_palette jsonb not null default '{}'` — the derived accent role map, regenerated server-side whenever the accent changes, and by the hourly stale-palette sweep whenever the engine that wrote it is behind (see *Theming Model* below). Unconstrained jsonb; every write replaces the whole map.
- `theme_palette_engine_version integer` — the `SIGNET_ENGINE_VERSION` that wrote `theme_palette`. Every palette write sets it in the same statement (`chapterPaletteColumns`). The config PATCH also clears it to `NULL` in its branding write, ahead of its recompute ([`accent-engine.md`](../ui/design-system/accent-engine.md) §6 Persistence). `NULL` or lower than the running engine's means stale, and is what the sweep selects on (#1165). Clearing it is always safe, since it only re-queues the row. Never set it to a version without the palette that version wrote.
...
- **Reactions:** Clients hold one global subscription to inserts and deletes on `chat_message_actions` (`packages/chat-core/src/realtime-manager.ts`). New reactions are pushed in real time. Inserts are delivered only for messages the member can read, and not at all for a reaction by a member the subscriber has blocked in that message's chapter, because Realtime applies the table's SELECT policy to them ([`AUTHORIZATION_MODEL.md`](../../docs/internal/security/AUTHORIZATION_MODEL.md) § The policies that do exist). Deletes are not RLS-filtered, so every subscriber receives the bare id of every removal and ignores ids it doesn't hold (`20260803150000_chat_message_actions_membership_rls.sql`).
...
- **Retrieval is hybrid, not vector-only:** a dense vector search for semantic similarity, a sparse keyword search for exact terms (a bylaw article number, a dollar amount, a member's name), and a reranking pass over the merged candidate set. Dense-only retrieval reliably misses exact-match queries, which are common in this corpus.
  - **The sparse half now exists for global search, and the corpus can build on it.** `apps/api/src/application/services/search.service.ts` runs indexed Postgres full-text search on every source; how (columns, indexes, parse mode, migrations) is [`spec/behavior/search.md`](../behavior/search.md#global-search) (the Implementation bullet), not restated here. What remains corpus-specific is chunk-level indexing, **relevance scoring on the sparse side** — `search.service.ts` ranks nothing today, ordering chat by `created_at` and cutting at a flat limit — and rerank. The sparse *index* exists; the sparse *scoring* hybrid retrieval needs to merge candidate sets does not.
...
The cutover migrated no stored data, because nothing needed to: the column is unconstrained jsonb and both clients read by allow-list. Rows written before it keep holding the eight dead keys until the #1165 sweep recomputes them, and since every writer replaces the whole map, that recompute drops the keys. That happens within an hour of each environment's first deploy of #1165, so on production only once **Deploy production** has shipped it. The allow-list guards stay, because they are what keeps any non-role key in the column off `:root`.
