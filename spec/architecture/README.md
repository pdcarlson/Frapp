...
- **Amendment (2026-09-02) — lever (A)'s cache key and build step are now one composite action, and there are seven consumers, not six.** The decision text above says `packages-build` writes the cache and *"the six downstream jobs restore it read-only."* It is **seven** — `lint-and-typecheck`, `api-tests`, `web-tests`, `api-contract-check`, `dependency-cruiser`, `mobile-validate` and `web-responsive-floor`; the last was added after this ADR was written and nothing caught the count, which is the drift this amendment's other half exists to stop. All eight call sites (one producer, seven consumers) now `uses: ./.github/actions/turbo-packages-build`, the repo's first composite action, per stage 4 of the CI/CD redesign (#1382). The producer passes `save: "true"` and gets `actions/cache`, whose post-job hook writes `.turbo` back; consumers get `actions/cache/restore`, which has no post hook and so cannot race the producer for the same key — the split the eight hand-written blocks already made, now stated once. **The cache key itself is unchanged**, so existing cache entries stay warm across this change. What changed is that the key had been repeated verbatim eight times: a producer and a consumer drifting onto different keys would have turned every downstream job into a cold rebuild while failing silently, because a cache miss is not an error. `clean-checkout-typecheck` and `web-production-build` deliberately do **not** use the action, and their in-file comments say so.
