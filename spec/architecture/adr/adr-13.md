...
- **Branch protection and repository-level Actions secrets are unaffected** — both are available on private repos with Pro. The deploy pipeline resolves runtime secrets from Infisical at workflow time and uses only repo-level bootstrap secrets, so it keeps working.
  - **Correction (2026-09-23).** Repository-level bootstrap secrets were the hole #2518 closes. Whatever the visibility, a repository secret is readable from any branch, because a same-repository pull request or a push to any branch runs that branch's own workflow definitions. The decision since: every GitHub secret belongs to an environment restricted to `main`. The workflows follow it, and since #2583 (2026-09-24) the settings do too. Canonical: [`docs/internal/ci-cd/AGENT_INFRA.md`](../../../docs/internal/ci-cd/AGENT_INFRA.md) § GitHub environments and bootstrap secrets.
