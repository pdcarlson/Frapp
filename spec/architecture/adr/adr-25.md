  ...
  3. **Server.**
     - *Code:*
       ...
       - The conformance naming checks in `scripts/ci/staging-conformance.mjs`, which `production-auth-conformance.mjs` reuses:
         ...
         - the `leftoverFrappMailerSubjectKeys` guard. It fails any `mailer_subjects_*` containing "Frapp" before the subject is compared, so it inverts to catch leftover Signet, with its lock `signet-mailer-subjects.test.mjs`. *2026-09-24: done in step 3 ([#2578](https://github.com/pdcarlson/Frapp/issues/2578)): the guard is now `leftoverSignetMailerSubjectKeys` and its lock `frapp-mailer-subjects.test.mjs`.*
     ...
     - *Consoles (owner):*
       ...
       - *2026-09-24: step 3 moved `ENV_REFERENCE.md` to the Frapp value, and sets an order: `RESEND_FROM_EMAIL` just before the deploy that restarts the API (the API reads it at boot), then the Auth consoles, staging once the merge has deployed there and production after Deploy production of the merge commit, so production's sign-in mail and invite mail change name together. Merge once that day's scheduled Staging and Production Auth conformance runs have both been queued, and deploy production the same day, so both land before the next scheduled conformance run. The steps are in [`supabase.md` § ADR-25 step 3](../../../docs/internal/ops/deployment/supabase.md#adr-25-step-3-the-sender-becomes-frapp).*
