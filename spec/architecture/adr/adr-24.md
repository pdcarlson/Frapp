...
1. **Alerts reach no one reliably.** Every watchdog upserts an issue labelled `routine-state` with no
   assignee or @mention (`scripts/ci/lib/alert-issue.mjs`), and `/next`, issue-triage and `/needs-me`
