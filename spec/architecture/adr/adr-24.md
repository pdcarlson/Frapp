...
| 2 | **The pager is Sentry → a private Discord channel** with phone notifications, **email** as the second path, and every alert issue **assigned to the owner** and labelled `incident` instead of `routine-state`. Agents triage and report on incident issues but don't change provider state from them. A test firing before the beta proves the path. Decided, not built yet: #2505 wires it. *(Corrected 2026-09-24: the issue half is built. Every watchdog files and reopens its alert as `incident`, assigned to the owner, from one place in `scripts/ci/lib/alert-issue.mjs`. The existing alert issues gained `incident` ahead of the merge and lose `routine-state` after it; #2505 records the migration. The Discord pager, the email path and the test firing aren't built yet.)* *Round one chose "the Sentry mobile app"; replaced.* | No current first-party Sentry phone app was found (digest 10). Round one recommended it without checking. Discord is already in the product's world. Assignment is a participating notification, which reaches the owner under every repo-watch setting except Ignore. An unassigned issue reaches them only if their setting covers new issues (All Activity, or Custom with Issues). The test firing confirms the owner's setting. The agent rule guards against an alert's suggested fix being wrong, as #1564's was. |
