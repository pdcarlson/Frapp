 ...
 * that still references one of them through USER_REF_FKS_SQL afterwards lies outside it: in
 * another chapter (the App Review login founding or joining one, say, or rows it wrote there
 * before leaving), or in no chapter at all, like a directory request. Deleting the account
 * would delete that row, fail on it, or null its reference, so this refuses instead.
 * demo-seed.sql carries the same block for its own namespace, and a test pins the two
 * together.
 */
...
export function renderRemoveSql({ namespace }) {
  ...
  return (
    ...
    `-- This touches the database only. Then run \`seed-demo.mjs storage --namespace ${namespace} --remove\`; add\n` +
    `-- \`auth --namespace ${namespace} --remove\` only to delete the login for good, not before a re-seed, which\n` +
    `-- links the same login again. The full commands: docs/guides/demo-data.md, To remove a demo chapter.\n` +
    `BEGIN;\n` +
    `DELETE FROM chapters WHERE id = '${chapterId}';\n` +
    accountGuardSql(userIdLike) +
 ...
 * Every bucket is listed and checked (assertUnderPrefix) before the first delete, so a stray
 * path in any of them stops the run with nothing removed. `listObjects` is there for tests.
 */
export async function removePlaceholders({
  supabaseUrl,
  serviceKey,
  namespace,
  fetchImpl = fetch,
  ...
  for (const bucket of names) {
    ...
    listed.push({ bucket, paths });
  }
  const results = [];
  for (const { bucket, paths } of listed) {
...
export async function main(argv, env = process.env, io = { out: process.stdout, err: process.stderr }, fetchImpl = fetch) {
  ...
  if (options.command === "storage") {
    ...
    for (const { bucket, count } of results) {
      // --remove walks every bucket; the empty ones are not news.
      if (options.remove && count === 0) continue;
      say(`storage: ${options.remove ? "deleted" : "uploaded"} ${count} object(s) in ${bucket}`);
    }
    if (options.remove) {
      const total = results.reduce((sum, r) => sum + r.count, 0);
      say(`storage: ${total} object(s) removed under chapters/${demoIds(namespace).chapterId}/ across ${results.length} bucket(s)`);
    }
  }
}
