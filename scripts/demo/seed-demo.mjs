...
export function renderRemoveSql({ namespace }) {
  ...
  return (
    ...
    `DO $remove_guard$\n` +
    `BEGIN\n` +
    `  IF EXISTS (SELECT 1 FROM members WHERE user_id::text LIKE '${userIdLike}' AND chapter_id <> '${chapterId}') THEN\n` +
    `    RAISE EXCEPTION 'a demo account is also a member of another chapter; refusing to delete it, which would cascade through that membership';\n` +
    `  END IF;\n` +
    `END $remove_guard$;\n` +
 ...
 * the database, so it works before or after `sql --remove`.
 */
