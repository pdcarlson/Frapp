...
export function renderRemoveSql({ namespace }) {
  ...
  return (
    ...
    `-- This touches the database only. Then run \`seed-demo.mjs storage --remove\`; add \`auth --remove\` only to\n` +
    `-- delete the login for good, not before a re-seed, which links the same login again.\n` +
    `BEGIN;\n` +
  ...
  if (!zoned) {
    fail(
      `no upcoming event with a check-in zone (${upcoming.length} upcoming in all): the seed is stale. ` +
        ...
        "(docs/guides/demo-data.md, Re-seed before every submission, has the full commands)",
    );
  }
  pass(
