...
# Clear the Supabase CLI's ephemeral staging dir before bringup. `supabase start`
# stages per-container secret files under supabase/.temp/start-secrets/ and removes
# the previous set on the next start. A build snapshot captures that directory
# (it is gitignored and untracked, and a fresh checkout preserves it), and the boot
# that restores the snapshot can run under a different uid than the one that wrote it,
# so the next `supabase start` dies with `EACCES ... rm .../start-secrets/...` and
# fails the whole bringup. The dir is pure ephemeral CLI state (regenerated by
# `supabase start`; DB data lives in Docker volumes, not here), so sudo-removing it
# per boot is safe and makes bringup robust to any snapshot-carried staging state.
# sudo because that stale state may be owned by another uid; guarded so a bad ROOT
