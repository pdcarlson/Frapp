...
// `backup-production-storage` is missing, hung more than 3h, or its last
// success is older than 36h. A latest run that is in flight (under 3h),
// cancelled, failed or skipped greens the run only while an earlier success
// within 36h backs it, and never closes an open alert: only the latest run's
// own success within 36h is recovery. Unreadable Actions responses are FAIL,
// not pass. The rules are shared with the dump watch:
