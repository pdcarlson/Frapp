...
// #1840 skipped every pulls-404. That is wrong when the number is a real PR
// the token cannot see (private-repo existence-hiding). A 404 still throws
// unless GET /issues/N confirms a bare issue (no `pull_request` key). The
// operator escape is an explicit `bump` on the Release dispatch.
//
 ...
 * a scope. Skipping a squash trailer that named an issue is #1839: that 404
 * is not an auth failure, and throwing it aborts the tag after production is
 * already live. A 404 we cannot classify as a bare issue still fails closed.
 * The operator escape is `bump=minor` (or patch/major).
 */
