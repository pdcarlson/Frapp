...
echo ""
echo "Before npm run dev:* (first time on this machine):"
echo "  npx infisical login"
echo "  See docs/internal/SECRETS_MANAGEMENT.md — Infisical 'local' env must be populated."
echo ""
echo "Run apps (Infisical injects secrets from repo root):"
echo "  One terminal (API + web + landing + docs, prefixed logs):"
...
echo "    npm run dev:docs      -> http://localhost:3005"
echo ""
echo "Without Infisical: use .env.local per app + npm run start:dev -w apps/api (etc.)."
echo "  See docs/internal/LOCAL_DEV.md"
echo ""
