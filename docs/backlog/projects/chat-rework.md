...
| 09 — Members directory + custom fields rendering | [#437](https://github.com/pdcarlson/Frapp/issues/437) | shipped | this PR | spec-compliant directory (role/cohort/status filters, table+card toggle, sortable, 25/page, points, wired bulk role assign) + **custom-field rendering** vertical slice: new `member_custom_field_values` store (migration `20260531120000`), read-only `GET /custom-fields`, `GET /members/:id` returns **server-side visibility-filtered** values, detail-sheet renders them. Deferred → #579/#580/#581/#582. Canon → `spec/behavior/members.md`, `spec/behavior/rbac.md` (visibility tiers), `spec/ui/web-dashboard/` |
