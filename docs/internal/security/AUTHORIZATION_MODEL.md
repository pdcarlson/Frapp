...
| `audit-log` | `GET` | A+C+P `chapter_config:view` | A — chapter is the subject; read-only, `chapter_audit_log`'s RLS carries no SELECT policy so only this service-role-backed query can ever return rows |
