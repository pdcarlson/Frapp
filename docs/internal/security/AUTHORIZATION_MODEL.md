...
| `backwork` | `POST /upload-url`, `POST /`, `GET /`, `GET /:id`, `GET /departments`, `GET /professors`, `PATCH`\\|`DELETE /departments/:id`, `POST /departments/:id/merge`, `PATCH`\\|`DELETE /professors/:id`, `POST /professors/:id/merge`, `DELETE /:id` | A+C+P, `backwork:upload` **or** `backwork:admin` at class level — so reads are gated too. The class gate is ANDed with each handler's own, never replaced: the two uploads (`POST /upload-url`, `POST /`) add `backwork:upload`, so `backwork:admin` alone cannot upload; every taxonomy write and `DELETE /:id` adds `backwork:admin`, so `backwork:upload` alone cannot rename, merge, or delete | B — `resourceRepo.findById(id, chapterId)` (`backwork.service.ts:192,213`); the taxonomy routes resolve on `(id, chapterId)` in `backwork.service.ts` and 404 across chapters |
...
| `chapters/current` | `GET /current`; `PATCH /current`, `POST /current/logo-url`, `POST`\|`DELETE /current/logo` | A+C+P `members:view` (read); `chapter-config:view` **and** `chapter-config:manage` for the four writes (`CHAPTER_PROFILE_PERMISSIONS`, `@repo/validation`, #2575) | A — the guard-resolved chapter; the `GET`, `PATCH` and both `/current/logo` responses are projected through `toChapterMemberView` (#930) |
...
| `members` | `GET`, `GET /search`, `GET /:id`, `PATCH /:id/roles`, `PATCH /me/onboarding`, `PATCH /me/ops-nudges/dismiss`, `DELETE /:id` | A+C+P `members:view`, `roles:manage`, `members:remove` | **C** — `memberRepo.findById(id)` then `member.chapter_id !== chapterId → 403` (`member.service.ts:112,205,218`). The two `me/*` writes are the exception: they carry no handler-level permission, so they run on the class-level `members:view`, and their row proof is `@CurrentMember()` rather than an id comparison — `ChapterGuard.findMembership` resolves `request.member` filtered by **both** `user_id` and `chapter_id`, so no caller-supplied id reaches the write. |
...
| `POST /invites/redeem` | A | Redeems by opaque invite code; the code *is* the capability. Chapter comes from the invite row, not the caller — which is also why the subscription hard lock is evaluated in `InviteService.redeem` against `invite.chapter_id` (#1546) rather than by `ChapterGuard`, which never sees that chapter |
...
**10 in `public` + 1 in `realtime` = 11** at that read. Migration `20260906203000` (#1552)
...
| Table | Policy | Effect |
| --- | --- | --- |
| `chat_message_actions` | `_select` | `auth.role() = 'authenticated' AND can_read_chat_message(message_id) AND NOT (starts_with(action_type, 'reaction:') AND chat_viewer_has_blocked(user_id, message_id))`. The first two conjuncts are a per-row channel-membership check via a `SECURITY DEFINER` function mirroring `canAccessChannel`. The third, since `20260924170000` (#2494), withholds a blocked member's reactions from the member who blocked them. `chat_viewer_has_blocked` is `SECURITY DEFINER` too and takes no blocker parameter: it resolves the viewer from `auth.uid()`, so it only answers about the caller's own block list, and the blocked member's reads don't change. It answers false for a message the caller can't read (`can_read_chat_channel`), so over RPC it can't be used to test whether a message exists. Votes and other action types are unaffected. RLS is the only gate here: both clients hydrate reactions with a direct select (each `use-chat-channel.ts`), and live ones come over the Realtime subscription in `packages/chat-core/src/realtime-manager.ts` |
...
| `realtime.messages` | `realtime_messages_scoped_select`, `realtime_messages_scoped_insert` | **SELECT** authorises five private topics by prefix — the three change-ping topics (`notif:` / `events:` / `attendance:`) and, since 2026-09-06 (#1552, migration `20260906203000`), the two presence topics: the Directory's `presence:chapter:<chapterId>` behind chapter membership (`realtime_can_read_chapter_scope`, as `events:` already was) and chat's `chat:channel:<channelId>` behind **`can_read_chat_channel`** — the channel half of `can_read_chat_message`, extracted so the message RLS and the realtime authorisation share one definition of "may read this channel" (PUBLIC to the chapter, PRIVATE/DM/GROUP_DM to `member_ids`, ROLE_GATED by permission, a ROLE_GATED channel with no requirement to nobody). Chapter membership alone would have exposed DM presence chapter-wide. **INSERT** authorises `extension = 'presence'` on `presence:chapter:` and `presence` **or** `broadcast` on `chat:channel:` (typing is a client broadcast), each behind the same predicate as its SELECT arm; the change-ping topics are receive-only. Both `case`s end in `else false`: any other topic is denied on a private channel and must ship a branch here before it can go private. **Presence stays advisory even now:** the policies see the topic and the message extension, not the presence payload, so a member who may read a channel can still publish an entry naming another member of it. What is closed is the anon-key outsider, in both directions, on both topics |
...
| Table | Coverage |
| --- | --- |
| `chat_message_actions` | membership/tenancy matrix (read path) + the block clause: a blocker reads none of a blocked member's reactions in that chapter, and keeps their votes, their reactions in another chapter, and other members' reactions; the blocked member's reads are unchanged (#2494) |
