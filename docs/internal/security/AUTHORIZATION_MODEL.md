...
| `chapters/current` | `GET /current`; `PATCH /current`, `POST /current/logo-url`, `POST`\|`DELETE /current/logo` | A+C+P `members:view` (read); `chapter-config:manage` for the four writes (`CHAPTER_PROFILE_PERMISSION`, `@repo/validation`, #2575) | A — the guard-resolved chapter; the `GET` and `PATCH` responses are projected through `toChapterMemberView` (#930) |
