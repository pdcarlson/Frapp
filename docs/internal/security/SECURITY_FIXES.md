...
The policy is additionally scoped `TO authenticated`. That is load-bearing rather than cosmetic: `execute` on the helper is revoked from `anon`, and Postgres resolves a function's EXECUTE ACL when it initializes the expression — before any `AND` short-circuit can spare it — so an `anon` read would raise `42501 permission denied for function` instead of returning zero rows. The role clause, not qual ordering, keeps `anon` out of the function. (The migration emits the clause through `format()` guarded on `pg_roles`, because bare-Postgres substrates such as the PGlite CI harness have no `authenticated` role.)
...
Note that the CI harness does **not** cover that path: PGlite runs as a single superuser with no `authenticated` role, so it cannot `SET ROLE` and exercise the policy black-box. Regression coverage in `scripts/check-pglite-migrations.mjs` therefore splits in two — shape assertions on the applied catalog (exactly **one** permissive SELECT policy, since Postgres OR-s permissive policies together and a second one would silently re-open this exact leak; `SECURITY DEFINER` with `search_path` pinned to `public`; EXECUTE revoked from `public`; replica identity still default), plus a seeded functional tier that drives `can_read_chat_message` directly across 15 scenarios (own-chapter, cross-chapter, `PRIVATE` in/out, `DM` in/out, `GROUP_DM` in/out, `ROLE_GATED` with/without permission, `*` wildcard, empty requirement, a chapter-B role id held by a chapter-A member, an uppercase stored role id, and null `auth.uid()`). End-to-end enforcement as a real JWT-bearing role remains deferred to the NestJS tier (#423).
