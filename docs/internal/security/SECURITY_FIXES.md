...
- **Root `overrides`** (in `package.json`) force patched versions of transitive packages without requiring upstream releases. The override pattern is the canonical lever for transitive CVEs in this monorepo — extend it rather than patching individual workspaces. The `@xmldom/xmldom` override uses an unbounded floor (`>=0.8.13`) so consumers that declared a higher major retain it — today `plist@3.1.1`, which declares `@xmldom/xmldom@^0.9.10`, resolves to the hoisted `0.9.x`. **`@expo/plist@0.8.1`, which declares `@xmldom/xmldom@^0.8.8`, must not**: it is pinned back to `^0.8.15` by a nested override (`"@expo/plist": { "@xmldom/xmldom": "^0.8.15" }`), because `@expo/plist@0.8.1` calls `DOMParser.parseFromString(xml)` with no MIME type and xmldom `0.9` throws on that (`the provided mimeType "undefined" is not valid`). With a single hoisted `0.9.10` — the state this paragraph described until 2026-09-06 — `expo prebuild` failed on the iOS `Info.plist` mod (`withIosInfoPlistBaseMod`), and EAS runs that same prebuild from this lockfile, so **no EAS iOS build could have succeeded**. `0.8.15` is inside the advisory floor; `npm audit` reports nothing against it. Two npm 10 traps when touching this: `npm install` does not re-resolve an already-locked package when an override changes (it reports the edge `invalid` and leaves it), and deleting the lock entry by hand makes `npm install` drop the package from the tree entirely. `npm update @xmldom/xmldom --package-lock-only` is the invocation that re-resolves it correctly. (The list read `jsdom@29`, `expo-server-sdk@5`, `plist@3` until #1419; neither of the first two reaches `@xmldom/xmldom` in the tree at all any more — the `jsdom@29` was the orphaned root copy #1419 pruned, whose story is told in [`ci-cd/AGENT_INFRA.md`](../ci-cd/AGENT_INFRA.md#a-group-regeneration-can-drop-jsdom-and-vitest-resolves-it-from-the-root).) The `undici` override is bounded — `>=7.29.0 <8.0.0` — because undici `8.x` raises its engine to `node>=22.19.0`, which the `node:20-alpine` Docker base `apps/api` used at the time did not satisfy. npm only warns on `EBADENGINE` unless `engine-strict` is set, which this repo has never set, so the mismatch alone does not fail `npm ci`. PR #307 (squash-merged as `b2899a89`) first attributed an `npm ci` crash in `api-docker-build`'s prod-deps stage, "Exit handler never called!", also reproduced in a local `node:20-alpine` container, to undici 8, and added this cap. Its later diagnostic commits traced the CI failure to a missing `apps/api/node_modules` breaking a Docker `COPY`, caused by the PR's other overrides, and the cap stayed in; no run after that fix tried undici 8 without it. What lifting the cap now involves (root `jsdom` 30 declares `undici ^8.9.0` and is held at 7.x by this override) is on #2360. **Only lift the `<8.0.0` cap after** the repo's `engines.node` is bumped to `>=22.19.0`, the `apps/api` Dockerfile base image is moved off `node:20-alpine`, and CI's `setup-node` matrix is updated to match. **All three conditions were met on 2026-09-17** (`engines.node` had been raised to `>=24.9.0`, the Dockerfile was `node:24-alpine` in all four stages, every CI job pinned `node-version: 24`), so this is no longer a blocker — the cap is simply un-lifted, tracked in #2360. Lifting it is a deliberate dependency change with its own verification, not a side effect of the Node move, which is why it did not ride along. The floor was raised from `>=6.24.0` in #699 — see [undici floor](#undici-floor-raised-off-the-vulnerable-7x-band-699) below. Note the floor is only a *lower* bound on an override npm always resolves to the max of, so it does not by itself pick the version; it exists to stop a future resolution from sliding back under the advisory. undici `7.29.0` declares `engines.node >=20.18.1`, which every real runtime here satisfies (Docker `node:24-alpine`, all CI jobs on `node-version: 24`, root `engines.node`, which requires at least Node 24).
...
- ~~**`@infisical/cli`** kept pinned at `0.43.40` (matches main). Newer `0.43.80+` declares `tar ^7.5.13` natively but breaks the `apps/api` Docker build during the preinstall (`tar.x` extraction in `node:20-alpine` fails consistently). The two resulting high advisories (`@infisical/cli` and its nested `tar`) are accepted as **dev-only install-time exceptions**~~ — **superseded (#831/#618 sweep): the pin is now `0.43.121` and the exception is lifted.** Re-tested at `0.43.121`, whose preinstall extracts with node-tar `7.5.22`; a full Dockerfile deps-stage `npm ci` inside `node:20-alpine` completes with a working binary, so the tar advisories are fixed rather than excepted — see "npm audit sweep + CI gate" below. (`@infisical/cli` is a root `optionalDependencies` entry used only by `npm run dev:*` scripts. Its postinstall fetches the binary from GitHub Releases; a 5xx there must not fail `npm ci` — CI injects Infisical via `.github/actions/infisical-secrets`, not this binary, and the `apps/api` Dockerfile `prod-deps` stage already excludes it. Observed as a required-check skip cascade on #1938 / #1939.)
...
undici reaches the *production* image (`apps/api` → `expo-server-sdk` → `undici`), not just test tooling, so this floor is load-bearing for the deployed artifact. The `<8.0.0` ceiling is retained, though its original `node:20-alpine` engine reason lapsed with the Node 24 move recorded above — see #2360.
...
**Amendment (2026-09-08):** `multer@2.2.0` is in range for three new HIGH GHSAs (`GHSA-wc9g-mqfw-jrwm`, `GHSA-qfvm-cv95-jqjf`, `GHSA-535w-7cp7-47q4`), all fixed in `2.3.0`. `@nestjs/platform-express` still pins `multer@2.2.0` exactly at the repo's `^11.2.1` and at npm latest `12.0.1`, so a Nest bump cannot clear them. Root override `"multer": ">=2.3.0 <3.0.0"` plus `npm update multer --package-lock-only` forces the patched copy. **Amendment (2026-09-17):** that range was never the effective override. It was added alongside an existing `"multer": "2.3.0"` key in the same `overrides` block, and duplicate JSON keys are last-wins, so npm always read the exact `2.3.0`. The duplicate was collapsed to the single effective entry while resolving the Node 24 bump; resolution never changed (`multer@2.3.0` before and after), but the **remedy above does not work as written**: under an exact override, `npm update multer --package-lock-only` is a no-op. To clear the next multer advisory, edit the override itself. Whether it should go back to being a range is #2359. The API has no `FileInterceptor` / multer upload routes; GHSA-535w's extra `limits.fieldArrayIndexLimit` is unused until one lands.
...
- **The React pin moves with the SDK.** SDK 57 requires React `19.2.3` exactly. Because the root `overrides` entry is global, mobile cannot take it while the override holds the old version — so every pin site moves in one commit. The pin stays *exact*. [`AGENTS.md`](../../../AGENTS.md) § gotchas names the sites; do not restate the count here, and re-derive it with `git ls-files '*package.json' | xargs grep -ln '"react": "19'` rather than trusting a copy.
...
- Parses `npm audit --json` at the **advisory** level (GHSA ids, case-normalized), not the package level, and **fails on any high/critical advisory absent from `scripts/npm-audit-allowlist.json`**. Moderate/low are reported, never blocking. A gated-severity advisory whose GHSA id cannot be parsed is a hard failure, not a skip. The invocation pins dev/optional inclusion and npm's own fetch budget so an `.npmrc` cannot move either — the flags and the reasons for each are in the script's header, not restated here. Locally, `ci:local-gate` passes `--soft-network` so an offline dev is warned rather than blocked when the registry is unreachable (same convention as the secret scan's `--soft-missing`); CI never softens.
...
- Current allowlist: read `scripts/npm-audit-allowlist.json`. Each entry carries its own reason, tracking issue and expiry; a copy of the roster here goes stale the first time one is added or cleared, silently.
...
The `TO authenticated` gap is **closed**: the harness now creates the `authenticated` role before applying migrations and grants it to `rls_probe`, so the ~18 migrations that wrap policies and grants in `if exists (select 1 from pg_roles where rolname = 'authenticated')` actually execute, and the `TO` clause applies to the probe. That was not cosmetic — before it, a `create policy … to authenticated using (true)` written in that guarded idiom (the repo's dominant one) left the entire harness green while handing every signed-in client every row.

Two gaps remain, both promotion-time checks rather than CI ones (see `DB_PROMOTION_RUNBOOK.md`): PGlite has no `anon` **role**, so the EXECUTE assertion checks only the PUBLIC bit while hosted Supabase grants `anon` directly, and a policy whose `TO` clause names `anon` still does not apply to the probe — note the black-box tier *does* stub `auth.role()` to `'anon'`, so predicates that **test** the role are exercised for both values, but the grant-level `TO anon` targeting is not. Full end-to-end enforcement under a real JWT also stays deferred to the NestJS tier: `rls_probe`'s `auth.*` values are stubbed rather than minted, so claims beyond `sub`/`role` are not exercised.
