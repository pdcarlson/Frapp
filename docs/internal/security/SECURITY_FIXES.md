...
Two gaps remain, both promotion-time checks rather than CI ones (see `DB_PROMOTION_RUNBOOK.md`): PGlite has no `anon` role, so the EXECUTE assertion checks only the PUBLIC bit while hosted Supabase grants `anon` directly; and it has no `authenticated` role, so the `TO authenticated` clause is never exercised. Full end-to-end enforcement under a real JWT stays deferred to the NestJS tier. (The harness does now read tables black-box as a non-owner `rls_probe` role it creates itself — see ADR-12 — but that role is not `authenticated`, and its `auth.*` values are stubbed rather than minted.)
