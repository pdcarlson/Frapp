...
| Autofix on PR create | **Off** for all four | A routine that repairs its own red CI keeps pushing unattended. Curator, Triage and PR Follow-ups have it off; Docs Upkeep was created with it **on** by default and should be turned off to match — its skill already says a red CI on a sweep is a run-report finding, not a reason to push again. |
