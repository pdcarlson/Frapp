...
| ------------ | ----------------- | ----------------------------------- |
| `staging`    | None; deployment branches `main` only ([#2583](https://github.com/pdcarlson/Frapp/issues/2583)) | Staging deploys (`main`), plus the two staging checks (`staging-conformance.yml`, `verify-deployments.yml`) |
...
| 13:15 | `production-backup-freshness.yml` | Nightly `db-backup.yml` `backup-production` last succeeded within 36h. Missing, hung more than 3h, or unreadable Actions responses are FAIL. A latest run that is in flight (under 3h), failed or skipped is pass only while an earlier success within 36h backs it (the job stays green; an open alert stays open). Does not name `environment: production` or `environment: production-backup`. Does not change the dump cron |
| 14:00 | `production-backup-storage-freshness.yml` | Nightly `db-backup.yml` `backup-production-storage` last succeeded within 36h. Missing, hung more than 3h, or unreadable Actions responses are FAIL. A latest run that is in flight (under 3h), failed or skipped is pass only while an earlier success within 36h backs it (the job stays green; an open alert stays open). Does not name `environment: production` or `environment: production-backup`. Does not change the dump cron |
