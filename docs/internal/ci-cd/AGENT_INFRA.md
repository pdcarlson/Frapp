...
Additional repo-level secrets: `RENDER_API_KEY`, `VERCEL_API_KEY`. Read-only consumers: `production-guardrails.yml` (both deploy keys), `verify-deployments.yml` and `staging-conformance.yml` (`RENDER_API_KEY`). `deploy-production.yml` uses the same two keys to **create** deploys — a Render deploy by `commitId` and a Vercel deployment with `target: production`. They still never carry runtime values.

Every workflow that needs runtime secrets (including `SUPABASE_ACCESS_TOKEN`) pulls them from Infisical at job time ([`SECRETS_MANAGEMENT.md` § GitHub Actions is not a sync](../environment/SECRETS_MANAGEMENT.md#github-actions-is-not-a-sync)). No GitHub environment-scoped runtime secrets are required beyond the Infisical bootstrap listed above.
