...
| Composite actions   | `.github/actions/<name>/action.yml` — shared step sequences called as `uses: ./.github/actions/<name>`. Requires an `actions/checkout` earlier in the job. Currently **three**. (1) **`turbo-packages-build`** — the ADR-15 Lever A turbo cache restore + `packages/*` build, used by all 8 jobs that need prebuilt packages; its cache key is single-sourced there and **no workflow may spell it out again** — `scripts/ci/__tests__/turbo-packages-build-action.test.mjs` enforces that, and also that `clean-checkout-typecheck` and `web-production-build` never acquire the action. (2) **`infisical-secrets`** — the credential preflight + `Infisical/secrets-action` injection, all 11 sites across 6 workflows. (3) **`supabase-cli`** — the single Supabase CLI version pin, all 4 sites; it takes **no inputs**, so the pin cannot be overridden per call site. `scripts/ci/__tests__/infisical-secrets-action.test.mjs` enforces (2) and (3). Add `.github/actions/**` to any `dorny/paths-filter` list that gates a job using one, or a PR touching only the action skips that job. |
