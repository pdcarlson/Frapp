...
| Production uptime | `.github/workflows/production-uptime.yml` — **scheduled** every 15 minutes + `workflow_dispatch`. GETs `https://api.frapp.live/health/ready` (never `/health`, which always 2xxes while the process is up). Raises one `routine-state` alert on failure and closes it on recovery. Does **not** name `environment: production` — a `schedule:` job that did would suspend on the required-reviewer gate ([#1435](https://github.com/[REDACTED]/Frapp/issues/1435)). Not a substitute for a Sentry 60s monitor (still human; quota). Logic in `scripts/ci/production-uptime.mjs`. **Not** a required check. |
...
| PR base sync        | `.github/workflows/pr-base-sync.yml` — `push` to `main`: sweeps open PRs targeting it (cap 20, logged); behind + clean PRs are auto-updated via the update-branch API **only when the base-sync GitHub App token mints** (default-token pushes trigger no CI). Conflicts and per-PR update failures upsert one `<!-- frapp-base-sync -->` wake comment telling the watching agent to merge `main` itself; a missing or rejected token is repo-wide, so it raises **one** `routine-state` alert issue instead of the same comment on every PR. Logic in `scripts/ci/pr-base-sync.mjs` (tests: `scripts/ci/__tests__/pr-base-sync.test.mjs`). **Not** a required check. See [`pr-babysitting.md` → Base-branch sync](pr-babysitting.md#base-branch-sync-scriptscipr-base-syncmjs). |
| Codex review        | `.github/workflows/codex-review.yml` — `pull_request` (opened / synchronize / reopened / ready_for_review) on PRs into `main`: installs `@openai/codex@0.155.0` and runs the CLI's `codex review` subcommand against a BYOK OpenRouter provider, then upserts **one plain** `<!-- frapp-codex-review -->` PR comment. **Advisory: blocks nothing, and must stay out of `scripts/ci/lib/required-checks.mjs`.** Never posts a GitHub *review* event — a write-access `CHANGES_REQUESTED` blocks squash and no agent can clear it ([#1875](https://github.com/pdcarlson/Frapp/pull/1875)) — so the permission is `issues: write`, not `pull-requests: write`. Skips drafts, fork PRs (no secrets on `pull_request`) and docs-only PRs (measured across the whole PR diff, so a PR that ever touched code keeps being reviewed; re-derived 2026-09-18 at 8.8–12% of recent commits, **not** the spike's unsourced "~22%"). Checks out `head.sha`, never `refs/pull/N/merge`. Purges every `AGENTS*.md` and restores only the merge base's, because `codex review` reads project instruction files natively — otherwise a PR rewrites the rules it is reviewed under. The CLI call is wrapped in `timeout 900`: an unreachable provider makes it **retry, not fail**. Reviewer trouble never reds the run — `scripts/ci/codex-review.mjs` always exits 0 and raises one `routine-state` alert issue instead (tests: `scripts/ci/__tests__/codex-review.test.mjs`). **Not** a required check. See [`AI_CODE_REVIEW_RUNBOOK.md` → Advisory CI review](AI_CODE_REVIEW_RUNBOOK.md#advisory-ci-review-codex-reviewyml). |
...
| `production-backup` | **No required reviewers, on purpose.** Referenced by `db-backup.yml` since 2026-09-06 (#1435). GitHub's documented behaviour is to create an environment the first time a job *runs* against a name that does not exist, with no protection rules — so it appears after the first scheduled run, not on merge, and had not been observed via `GET /repos/…/environments` when this row was written (reconfirmed 2026-09-07 with the same `GET`: the name is still absent). If it is ever created by hand, it must be created **without** reviewers; a required-reviewer rule here silently suspends every nightly production dump. Once it exists it **should** get Deployment branches → Selected → `main` (the [`db-backup.yml`](../../../.github/workflows/db-backup.yml) header); that is a branch filter, not a reviewer gate | The nightly **read-only** production dump and Storage mirror. A `schedule:` job naming `production` would suspend on the reviewer gate every night; this environment exists so the backup needs no human at 02:30 while keeping the deploy gate untouched. It holds no secrets of its own — the job injects Infisical `prod` (source) and `staging` (offsite bucket), and each backup action asserts the injected project ref against `.github/environments.json` before linking |
...
incident. Remaining production-parity work on those dashboards stays on #1384.
...
15-minute readiness probe does not talk to the Management API, so it does not join that stagger:
...
| every 15 min | `production-uptime.yml` | Live `GET https://api.frapp.live/health/ready` — HTTP 200 with JSON `status: "ok"`. Does not name `environment: production` |
