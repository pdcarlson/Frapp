...
| Codex review        | `.github/workflows/codex-review.yml` — `pull_request` into `main`: runs the Codex CLI's `codex review` subcommand against a BYOK provider, posts findings as inline comments on one **`COMMENT`** review and upserts **one summary** PR comment. **Advisory: blocks nothing, and must stay out of `scripts/ci/lib/required-checks.mjs`.** Never posts `CHANGES_REQUESTED` or `APPROVED` — the first blocks squash and no agent can clear it ([#1875](https://github.com/pdcarlson/Frapp/pull/1875)), the second would satisfy a human-review requirement nothing human looked at. Logic in `scripts/ci/codex-review.mjs` (tests: `scripts/ci/__tests__/codex-review.test.mjs`, which also asserts the workflow's own invariants). **Not** a required check. The CLI pin, the config gotchas, the instruction-file purge, the liveness verdicts and the docs-only gate are **not restated here** — [`AI_CODE_REVIEW_RUNBOOK.md` → Advisory CI review](AI_CODE_REVIEW_RUNBOOK.md#advisory-ci-review-codex-reviewyml) is their one home, and the decision and its evidence are in ADR-14's three 2026-09-18 amendments (decision, implementation, commenting). Its alert issue is rostered in [`ALERT_ROUTING.md`](../ops/ALERT_ROUTING.md). |
