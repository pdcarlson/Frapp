...
`frapp-staging`. Scope is **staging only** — SMTP, Infisical syncs, and the
sign-in probe are not yet true of production (SMTP is still the hosted 2/hour
cap, [#1824](https://github.com/[REDACTED]/Frapp/issues/1824)). Production Auth
hook + redirect allow list (and `ACTIVE_HEALTHY`) are a sibling,
`production-auth-conformance.yml`, with its own alert title so a recovered
staging cannot close a live production incident. Remaining production-parity
work on those dashboards stays on #1384.
...
| 07:30 | `staging-conformance.yml` | Project health, auth hook, redirect allow list, Auth SMTP (Resend + ≥300/hour), Magic Link template (`token_hash` on the app host), Infisical syncs, and a live sign-in probe against `frapp-staging` |
| 07:45 | `production-auth-conformance.yml` | Project health, auth hook, and redirect allow list on `frapp-prod` (Site URL pinned to `https://app.frapp.live`). Does not name `environment: production`. Does **not** assert production SMTP |
