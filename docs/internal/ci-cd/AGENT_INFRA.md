...
| 07:30 | `staging-conformance.yml` | Project health, auth hook, redirect allow list, Auth SMTP (Resend + ≥300/hour), Magic Link template (`token_hash` on the app host), Render `healthCheckPath` `/health`, Infisical syncs, and a live sign-in probe against `frapp-staging` |
