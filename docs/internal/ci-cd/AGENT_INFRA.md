...
| `scripts/ci/lib/providers.mjs` | `fetchJson`, `fetchRenderDeploys`, `fetchVercelDeployments` | `fetchJson` is the shared ok-check-throw-json wrapper (was three near-identical copies, #1351); the other two list Render deploys / Vercel deployments through it. |
