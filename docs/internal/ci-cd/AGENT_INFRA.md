...
| AI code review      | **Local pre-push gate**, not CI — Cursor Cloud: [`.cursor/hooks.json`](../../../.cursor/hooks.json) `beforeShellExecution` with `failClosed: true` (Cursor defaults fail-open) wrapping [`.claude/hooks/pre-push-review-gate.sh`](../../../.claude/hooks/pre-push-review-gate.sh). Claude Code: the same inner hook via `.claude/settings.json` `PreToolUse`. A push is blocked until `.cache/diff-review/<SHA>` exists — `/diff-review` writes the marker; `/code-review` is richer but only conditionally model-invocable and does not write the marker (ADR-14 2026-06-04 amendment; the `claude-review.yml` CI workflow was removed). See `AI_CODE_REVIEW_RUNBOOK.md` |
...
| `hooks` | PreToolUse + SessionStart | Wires [`pre-push-review-gate.sh`](../../../.claude/hooks/pre-push-review-gate.sh) (Bash matcher — the Claude Code pre-PR review gate) and [`session-start.sh`](../../../.claude/hooks/session-start.sh) (cloud-sandbox bringup). Cursor Cloud's gate is [`.cursor/hooks.json`](../../../.cursor/hooks.json) (`failClosed: true`); it reuses this inner hook. A second PreToolUse hook (`linear-autoallow.sh`, PR #676) auto-approved Linear's write tools; it was deleted with the Linear retirement — see "Applied permission allows" below. Details: [`AI_CODE_REVIEW_RUNBOOK.md`](AI_CODE_REVIEW_RUNBOOK.md) and [`AGENTS.md`](../../../AGENTS.md) § Cursor Cloud specific instructions / § Claude Code web sandbox. |
