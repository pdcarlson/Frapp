...
| every 15 min (scheduled; runs far less often, see [ADR-24](../../../spec/architecture/adr/adr-24.md)) | `production-uptime.yml` | Live `GET https://api.frapp.live/health/ready` — HTTP 200 with JSON `status: "ok"`. Does not name `environment: production` |
