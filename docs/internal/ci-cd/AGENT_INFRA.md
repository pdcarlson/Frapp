...
| Codex review        | `.github/workflows/codex-review.yml` — `pull_request` into `main`: runs the Codex CLI's `codex review` subcommand against a BYOK provider and upserts **one plain** PR comment. **Advisory: blocks nothing, and must stay out of `scripts/ci/lib/required-checks.mjs`.** Never posts a GitHub *review* event — a write-access `CHANGES_REQUESTED` blocks squash and no agent can clear it ([#1875](https://github.com/pdcarlson/Frapp/pull/1875)). Logic in `scripts/ci/codex-review.mjs` (tests: `scripts/ci/__tests__/codex-review.test.mjs`, which also asserts the workflow's own invariants). **Not** a required check. The CLI pin, the config gotchas, the instruction-file purge, the liveness verdicts and the docs-only gate are **not restated here** — [`AI_CODE_REVIEW_RUNBOOK.md` → Advisory CI review](AI_CODE_REVIEW_RUNBOOK.md#advisory-ci-review-codex-reviewyml) is their one home, and the decision and its evidence are in ADR-14's two 2026-09-18 amendments. Its alert issue is rostered in [`ALERT_ROUTING.md`](../ops/ALERT_ROUTING.md). |
