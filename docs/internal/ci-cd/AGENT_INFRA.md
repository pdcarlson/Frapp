...
| 13:15 | `production-backup-freshness.yml` | Nightly `db-backup.yml` `backup-production` succeeded within 36h. Missing, failed, skipped, hung more than 3h, or unreadable Actions responses are FAIL. In-flight under 3h is pass. Does not name `environment: production` or `environment: production-backup`. Does not change the dump cron |
