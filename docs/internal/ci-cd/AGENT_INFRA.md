...
**Every secret is an environment secret, and no repository secret exists.** For a same-repository pull request, and for a push or a dispatch on any branch, GitHub runs the workflow definitions from that branch. So a repository secret is readable by anyone who can push a branch: they edit a workflow, or add one. Agent sessions push branches, and they read public issue text. The boundary that holds is an environment whose **deployment branches** rule admits `main` only:
