...
and skip-until-on SMTP are a sibling, `production-auth-conformance.yml`,
...
| 07:45 | `production-auth-conformance.yml` | Project health, auth hook, redirect allow list, and Auth SMTP on `frapp-prod` (Site URL pinned to `https://app.frapp.live`). Empty SMTP is SKIPPED (hosted 2/hour cap); SMTP on requires `no-reply@mail.frapp.live` at ≥300/hour. Does not name `environment: production` |
