...
| `permissions.allow` | `["Workflow"]` | Auto-approves launches of the multi-agent **Workflow** tool (bare tool name = allow all invocations). Lets `/next ultracode` and other opted-in turns orchestrate fan-outs without a permission prompt breaking autonomy. Allows nothing else — MCP and shell tools keep their normal prompting. |
...
- **Supabase MCP write tools (`create_branch`, `apply_migration`, `delete_branch`) and most read tools (`list_branches`, `get_project`, `get_cost`):** not granted by `.claude/settings.json` (its only allow rule is `Workflow`), so they prompt — and unattended sandboxes cannot approve the prompt. `list_projects` has been observed to go through. Do not assume any MCP tool works until you've tried it.
