...
The Vercel deployer used to be the second example here. Since [#1578](https://github.com/pdcarlson/Frapp/issues/1578)
it creates deployments through the Vercel CLI rather than by POSTing a `gitSource`, so `deploy-vercel.mjs`
only ever reaches this module with `GET`s — the deployment lookup and the poll. One instance of the hazard
is gone; the rule is not softened, because Render still has it and any future create would inherit it.
