...
| `staging` | `INFISICAL_MACHINE_IDENTITY_ID`, `INFISICAL_CLIENT_SECRET`, `RENDER_API_KEY`, `VERCEL_API_KEY`; optional `STAGING_SMOKE_USER_EMAIL`, `STAGING_SMOKE_USER_PASSWORD` | `deploy-api.yml`, `deploy-vercel-staging.yml`, `db-backup.yml` (staging jobs), `staging-conformance.yml` |
...
Read-only consumers of the provider keys: `production-guardrails.yml` (`RENDER_API_KEY` and `VERCEL_API_KEY`) and `staging-conformance.yml` (`RENDER_API_KEY`). `deploy-production.yml` uses the same two keys to **create** deploys: a Render deploy by `commitId`, and a Vercel deployment with `target: production`. `deploy-api.yml` creates the staging Render deploy by `commitId` the same way, and `deploy-vercel-staging.yml` the staging Vercel deployments. They never carry runtime values. Those runtime values (including `SUPABASE_ACCESS_TOKEN`) come from Infisical at job time ([`SECRETS_MANAGEMENT.md` § GitHub Actions is not a sync](../environment/SECRETS_MANAGEMENT.md#github-actions-is-not-a-sync)), and the Infisical pair above is the only way in. `INFISICAL_PROJECT_ID` and `OPENROUTER_API_KEY` had no consumer and were deleted with #2583 ([#1587](https://github.com/pdcarlson/Frapp/issues/1587), [#2447](https://github.com/pdcarlson/Frapp/issues/2447); the OpenRouter key was also revoked at the provider).
...
`always()`, so it sees the whole run's shape. (Each workflow sets its own condition.) Per run it
does two things:
...
`deploy-vercel-staging.yml`, which shipped in #1578 with no alerting at all. From #2431 to #2505 it
also served `verify-deployments.yml`, a push-triggered observer of the staging API's Render deploy;
since #2505 `deploy-api.yml` creates and polls that deploy itself, so a failed build fails
`deploy-staging` and raises the Deploy API alert. Which workflow a run is
...
| 13:15 | `production-backup-freshness.yml` | Nightly `db-backup.yml` `backup-production` has a success within 36h. What it asserts: the Production backup freshness row in [§ CI/CD summary](#cicd-summary) |
| 14:00 | `production-backup-storage-freshness.yml` | Nightly `db-backup.yml` `backup-production-storage` has a success within 36h. What it asserts: the Production backup storage freshness row in [§ CI/CD summary](#cicd-summary) |
