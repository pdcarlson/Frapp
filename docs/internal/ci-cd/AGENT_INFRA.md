...
| Docs                | `.github/workflows/docs.yml` — one job, `env-slugs`: every Infisical environment slug named in the files `scripts/check-env-slugs.mjs` scans is one that exists. Despite the workflow's name it is **not** a documentation gate, and it is **not** a required check; the four docs gates that used to run here were deleted, and nothing replaced them in CI. Markdown links are a separate workflow (`links.yml`, job `link-check`), also not required. See [`DOCS_CI.md`](DOCS_CI.md) |
