...
**If** the environment's network allowlist carries the **live staging egress** lines, the remaining reach is narrower than this section assumes: live Realtime / Presence and RLS-as-enforced-by-GoTrue can be exercised against hosted `frapp-staging` from a sandbox, provided the environment carries those lines *and* a staging smoke credential is available. Probe first — `curl -sS -o /dev/null -w '%{http_code}\n' https://api-staging.frapp.live/health` — and read [`.claude/skills/live-verification/SKILL.md`](../../../.claude/skills/live-verification/SKILL.md) before touching the deployed environment. Push fanout against real APNS/FCM remains unreachable and is not proposed for the allowlist.
