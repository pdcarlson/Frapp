...
GitHub Issues (canonical: planning, status, Triage intake)
   ▲ Cursor Cloud (primary interactive /next path) via the GitHub MCP
   ▲ Cursor Automations (intended scheduled path — **not live** until observed; see [`ROUTINES.md`](ROUTINES.md) and #2024/#2027)
   ▲ Claude Code (web) fallback via the GitHub MCP
   ▲ Claude Code Routines (current observed scheduled runtime until Automations observed)
...
- **Work is closed by the PR that does it** (`Fixes #N` in the PR **body** — native GitHub
  ...
  *title*, so the body is load-bearing). GitHub matches the closing keywords (`close` /
  ...
  also honours `Fixes` **only on merge into the default branch** (#962) — that is issue-close
