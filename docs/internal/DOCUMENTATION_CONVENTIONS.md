...
- **Shipped migrations are the exception: they keep the doc paths they shipped with.** Don't edit a comment in `supabase/migrations/` to follow a moved doc, and don't leave a redirect stub at the old path for it. Shipped migration files are treated as immutable, and a comment-only edit is still a migration change: `check:migration-safety` demands a runbook edit for it. A reader recovers the new home from git: `git show -M --name-status $(git log -1 --format=%h -- <old path>)` prints the commit that moved or deleted the file, and where it went. A cloud session's clone is shallow, so run `git fetch --unshallow` first if that prints nothing.
