...
- **Do not verify intent against code.** Code states current behavior, not intended behavior ([`AGENTS.md` § Spec vs code](../../AGENTS.md#spec-vs-code)), and a rationale is the record rather than a claim about the world that code can settle. Code moving on is not evidence the rationale was wrong, so never "correct" a record to match today's code. Correct it when the record itself is wrong, and date the correction so a reader can tell it from the original.
...
- **A doc move leaves shipped migrations alone.** A comment in `supabase/migrations/` that cites a moved or deleted doc keeps the path it shipped with: the move doesn't edit the migration, and the old path gets no redirect stub. Dozens of shipped migrations cite doc paths, and every edit under `supabase/migrations/` is a migration change that `check:migration-safety` makes carry a runbook edit, so a doc PR would turn into a migration PR. A reader recovers the new home from git: `git log -1 --format=%h -- <old path>` names the commit that moved or deleted the file, and `git show -M --name-status <that commit>` shows where it went. If the first command prints nothing, the clone is too shallow to reach that commit (a cloud session's clone is shallow): run `git fetch --unshallow` and retry.
