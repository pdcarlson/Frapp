...
- **Shipped migrations are the exception: they keep the doc paths they shipped with.** A shipped migration is never edited ([`database.md` § Conventions](../guides/database.md#3-conventions)), not even to follow a moved doc, and the old path gets no redirect stub either. A reader recovers the new home from git: `git log -1 --format=%h -- <old path>` names the commit that moved or deleted the file, and `git show -M --name-status <that commit>` shows where it went. If the first command prints nothing, the clone is too shallow to reach that commit (a cloud session's clone is shallow): run `git fetch --unshallow` and retry.
