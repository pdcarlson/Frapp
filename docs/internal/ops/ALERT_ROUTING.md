...
| --- | --- | --- | --- |
| *Deploy API is failing — pushes are not reaching the environment* | `deploy-outcome` job, `deploy-api.yml` | the last `Deploy API` run that tried to deploy did not succeed: a staging migration failed, Render did not take the commit's deploy `live` (`build_failed` / `update_failed` / `pre_deploy_failed`, superseded, or not finished in 20 minutes), or `frapp-api-staging` did not serve that commit as ready. The run log says which. A run whose deploy plan is `stale` (a re-run of an old run) never raises or closes it | a later run for `main`'s tip deploys successfully, or finds staging up to date and verified |
