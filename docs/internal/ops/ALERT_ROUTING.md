...
- **Staging alerts:** today the staging [watchdogs](#automated-github-issue-alerts) (deploy outcomes and staging conformance) open `incident` issues, as above. Staging isn't automatically low priority: each alert's priority is on its roster row, and the two that watch the staging API deploy are **P1**, because a production deploy of that commit builds the same image. Migration drift watches staging and production together. A daily Sentry digest is planned in #2505; none exists yet.
