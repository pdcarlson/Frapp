...
| *Nightly production dump is stale or failed — recoverability is unproven* | `production-backup-freshness.yml` (daily 13:15 UTC) | the newest `db-backup.yml` run's `backup-production` job failed, is missing or hung more than 3h, or no success of it completed within 36h, or the Actions GET was unreadable. **P1.** A cancelled, skipped or in-flight newest run is not this alert while an earlier success within 36h backs it (#2332). Rules: [`backup-job-freshness.mjs`](../../../scripts/ci/lib/backup-job-freshness.mjs). Does not name `environment: production` or `environment: production-backup` (#1435). The hosted restore leftover stays on its own issue (1861); the reviewer watch stays on its own issue (1956) | a later run's own `backup-production` job succeeds within 36h (a pass resting on an earlier run does not close it) |
| *Nightly production Storage mirror is stale or failed — recoverability is unproven* | `production-backup-storage-freshness.yml` (daily 14:00 UTC) | the newest `db-backup.yml` run's `backup-production-storage` job failed, is missing or hung more than 3h, or no success of it completed within 36h, or the Actions GET was unreadable. **P1.** A cancelled, skipped or in-flight newest run is not this alert while an earlier success within 36h backs it (#2332). Rules: [`backup-job-freshness.mjs`](../../../scripts/ci/lib/backup-job-freshness.mjs). Does not name `environment: production` or `environment: production-backup` (#1435). The Postgres dump watch stays on its own issue (1963); the hosted restore leftover stays on its own issue (1861) | a later run's own `backup-production-storage` job succeeds within 36h (a pass resting on an earlier run does not close it) |
