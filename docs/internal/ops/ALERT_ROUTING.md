...
| *Render staging deploy is failing — frapp-api-staging is not confirmed live on main* | `deploy-outcome` job, `verify-deployments.yml` (every push to `main`) | the last `Verify deployments` run to reach a verdict did not find the pushed commit's `frapp-api-staging` deploy live on Render: it ended in `build_failed` / `update_failed` / `pre_deploy_failed`, no deploy appeared for the commit, none finished in time, or Render refused or kept failing its reads. The run log says which, and [`ci-cd.md` § Deploy verification](deployment/ci-cd.md#deploy-verification-observer-workflow) has the exact rules. **P1.** `Deploy API` can't see this: it watches the *trigger*, going green once Render accepts its deploy hook and the old instance still answers `/health/ready`. This alert watches the *outcome* ([#2431](https://github.com/pdcarlson/Frapp/issues/2431)). A superseded deploy (`canceled` / `deactivated`) is no verdict and leaves the alert as it is. So does a cancelled run, and so does any run for a commit `main` has moved past | the run for `main`'s newest commit finds its deploy `live`: the next push, or re-running that run once the cause is fixed (the commit redeployed from the Render dashboard, a rotated `RENDER_API_KEY`) |
...
| *Nightly production dump is stale or failed — recoverability is unproven* | `production-backup-freshness.yml` (daily 13:15 UTC) | the `db-backup.yml` `backup-production` job is missing from a run that ran, hung more than 3h, or has no success that completed within 36h, or the Actions GET was unreadable. **P1.** A latest run that is in flight, cancelled, failed or skipped is not this alert while an earlier run's success within 36h backs it (that run shows a `::warning::`), so one failed night alerts about a day late (#2332). Rules: `scripts/ci/lib/backup-job-freshness.mjs`. Does not name `environment: production` or `environment: production-backup` (#1435). The hosted restore leftover stays on its own issue (1861); the reviewer watch stays on its own issue (1956) | a later run's own `backup-production` job succeeds within 36h (a pass resting on an earlier run does not close it) |
| *Nightly production Storage mirror is stale or failed — recoverability is unproven* | `production-backup-storage-freshness.yml` (daily 14:00 UTC) | the `db-backup.yml` `backup-production-storage` job is missing from a run that ran, hung more than 3h, or has no success that completed within 36h, or the Actions GET was unreadable. **P1.** A latest run that is in flight, cancelled, failed or skipped is not this alert while an earlier run's success within 36h backs it (that run shows a `::warning::`), so one failed night alerts about a day late (#2332). Rules: `scripts/ci/lib/backup-job-freshness.mjs`. Does not name `environment: production` or `environment: production-backup` (#1435). The Postgres dump watch stays on its own issue (1963); the hosted restore leftover stays on its own issue (1861) | a later run's own `backup-production-storage` job succeeds within 36h (a pass resting on an earlier run does not close it) |
