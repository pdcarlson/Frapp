...
(07:45) owns Auth hook + redirect allow list + skip-until-on SMTP + skip-until-SMTP-on Magic Link on `frapp-prod`; `production-release-pin.yml` (08:00) owns the three production hosts sharing a peeled `vX.Y.Z`; `production-backup-storage-freshness.yml` (13:30) owns that the latest `backup-production-storage` mirror succeeded within 36h. One real drift still raises exactly
