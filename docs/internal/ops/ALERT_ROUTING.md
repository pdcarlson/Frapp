...
| *Advisory codex review is not producing reviews* | `codex-review.yml` (every non-draft, same-repo PR into `main`) | the advisory CI reviewer did not produce a usable review. The verdict in the body says which: `missing-credential` (no `OPENROUTER_API_KEY` secret — the likeliest first failure), `config-error`, `reviewer-did-not-run` (a step before the CLI failed), `reviewer-failed` (any other non-zero exit; `124` is the 900s timeout, which is what an unreachable provider looks like since it retries), `empty-output`, `contract-violation` (the model ignored the output schema, which is prompt-enforced only) `render-mismatch` (it returned findings this script could not parse — real findings are being dropped) or `diff-not-inspected` (it raised nothing and said it could not read the diff — the runner's sandbox failed, so the review did not happen; fix the runner, not the repo). **P3, and never blocking:** the reviewer is advisory, so the workflow stays green and this issue is the only signal; the merge-quality gate is the local `.githooks/pre-push` + `/diff-review` path. Note that `clean-unverified` and `findings-unverified` deliberately neither raise nor clear this — both mean the detector could not see, and letting either clear would erase the signal | a later run produces **contract-confirmed** findings or a contract-confirmed clean review |
