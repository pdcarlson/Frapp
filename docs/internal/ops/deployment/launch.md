...
- [x] In-repo uptime: `.github/workflows/production-uptime.yml` (see [`AGENT_INFRA.md`](../../ci-cd/AGENT_INFRA.md) § Scheduled conformance). A Sentry 60 s monitor replaces it under #2505. The free plan allows one monitor, so ask before creating it; the Sentry MCP exposes `create_uptime_monitor` (not yet exercised). GitHub cron starts after this file is on `main`
