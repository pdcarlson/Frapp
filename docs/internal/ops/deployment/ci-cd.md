...
- **Staging API** (`deploy-api.yml`, `deploy-staging`; [#2505](https://github.com/pdcarlson/Frapp/issues/2505)): after CI and `migrate-staging`, [`deploy-render-commit.mjs`](../../../../scripts/ci/deploy-render-commit.mjs) creates a Render deploy for the commit CI verified and polls that deploy id. `live` passes; `build_failed` / `update_failed` / `pre_deploy_failed`, a superseded `canceled` / `deactivated`, a refused read, or no terminal state within 20 minutes fails. Then [`verify-served-commit.mjs`](../../../../scripts/ci/verify-served-commit.mjs) polls `API_HEALTHCHECK_URL` + `/ready` until it answers 2xx **and** its `commit` is that SHA, for up to 5 minutes. Render's `live` only means the new instance passed `GET /health`, and a bare 2xx can come from the old instance while the new one switches in. A missing `API_HEALTHCHECK_URL` fails the job. Staging deploys serialize on the `deploy-api-staging` concurrency group, so one run's deploy never cancels another's. A failure fails the job, and `deploy-outcome` raises the Deploy API alert.
