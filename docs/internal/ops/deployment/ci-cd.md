...
**What this gate can't see:** Render's own build of the pushed commit. Polling the Render [Deploys API](https://render.com/docs/deploys) for that commit's deploy happens after the push instead, in the observer workflow below, which alerts when the deploy fails. It can't gate a merge, because Render deploys after it; building the same Dockerfile here is what can.
...
- **Render** (`verify-render-api`): fails on `build_failed` / `update_failed` / `pre_deploy_failed`, on "no deploy created for this SHA" within `RENDER_NO_DEPLOY_GRACE_MS` (autoDeploy-wiring red flag), on no terminal state within `RENDER_OVERALL_TIMEOUT_MS`, and on a Render read that is refused (401/403/404) or fails on `RENDER_MAX_CONSECUTIVE_READ_ERRORS` polls in a row. Any other failed read is re-asked on the next poll, because a failure raises a P1 alert; a 429, a 5xx, a network-level failure or a 15s per-attempt timeout is also retried inside the read by `lib/http.mjs`'s `resilientFetch`. The constants are in [`verify-render-deploy.mjs`](../../../../scripts/ci/verify-render-deploy.mjs). Treats `canceled` / `deactivated` as neutral (superseded).
...
The workflow is advisory (not a required check), and a red check on a `main` commit notifies nobody. So its `deploy-outcome` job raises an alert issue when `verify-render-api` fails, and closes it when a run confirms a deploy `live` ([`ALERT_ROUTING.md`](../ALERT_ROUTING.md#automated-github-issue-alerts), [#2431](https://github.com/pdcarlson/Frapp/issues/2431)). Only the run for `main`'s current tip does either: a verdict is about one commit, so a re-run of an old run, or a slow one finishing after a newer one, is logged and changes nothing. The failure message in the run log names the commit SHA and the last observed state; open the Render dashboard to read the full deploy log.
...
Two GitHub secrets bootstrap the Infisical connection: `INFISICAL_MACHINE_IDENTITY_ID` and `INFISICAL_CLIENT_SECRET`. Like every GitHub secret here they belong in environments restricted to `main`, never in repository scope, because a repository secret is readable from any branch ([#2518](https://github.com/pdcarlson/Frapp/issues/2518)). The deploy-time values themselves come from Infisical at job time ([`SECRETS_MANAGEMENT.md` § GitHub Actions is not a sync](../../environment/SECRETS_MANAGEMENT.md#github-actions-is-not-a-sync)). Which environment holds which secret, including the provider API tokens the deploy and observer workflows use: [`AGENT_INFRA.md` § GitHub environments and bootstrap secrets](../../ci-cd/AGENT_INFRA.md#github-environments-and-bootstrap-secrets).
