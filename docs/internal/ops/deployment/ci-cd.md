...
- **Render** (`verify-render-api`): fails on `build_failed` / `update_failed` / `pre_deploy_failed`, on "no deploy created for this SHA" within `RENDER_NO_DEPLOY_GRACE_MS` (autoDeploy-wiring red flag), on no terminal state within `RENDER_OVERALL_TIMEOUT_MS`, and on a Render read that is refused (401/403/404) or fails on `RENDER_MAX_CONSECUTIVE_READ_ERRORS` polls in a row. Any other failed read is retried, first by `lib/http.mjs`'s `resilientFetch` and then on the next poll, because a failure raises a P1 alert. The constants are in [`verify-render-deploy.mjs`](../../../../scripts/ci/verify-render-deploy.mjs). Treats `canceled` / `deactivated` as neutral (superseded).
