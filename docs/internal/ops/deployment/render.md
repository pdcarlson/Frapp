...
**The stale-palette sweep takes no claim either, for a different reason: its write is compare-and-set.** Every replica reads the chapters whose `theme_palette_engine_version` is `NULL` or behind the running engine and recomputes each one, but the `UPDATE` matches only while the row is still stale **and** its `branding.colors.accent` is the seed the palette was derived from. The replica that loses finds the row current and writes nothing, and an officer's accent save that lands between the read and the write is never overwritten. A second replica costs duplicated CPU (about 20 ms of palette derivation per stale chapter), not a duplicated side effect. In steady state every row is current and each tick is one query that returns nothing. The ticks that matter are the first after a deploy that bumps `SIGNET_ENGINE_VERSION` (or, once, the first after #1165 shipped, when every row started `NULL`): one `palette sweep: recomputed X/N …` log line per replica that found stale rows, where a replica that lost the race to another reports its rows as changed during the sweep. A `failed` count in that line names rows that stay stale and are retried every hour.
