...
for ENV in preview production; do
  ...
  # secret key. Name kept for history; see ENV_REFERENCE.md § Mobile.
  eas env:set --environment $ENV --scope project --visibility plaintext \
...
Why production refuses the legacy anon key: [`ENV_REFERENCE.md`](../../environment/ENV_REFERENCE.md) § Mobile.
