...
> the per-project dates and freeze points, each breakage in full, and the amendments recording what
> each issue changed. Read the detail there, not here.
>
...
> has unit coverage but has never run against the live projects. Prefer running it with
> **`dry_run_only`** first, and watch step 6 rather than walking away from it.
