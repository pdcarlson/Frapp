...
| `pglite-migrations`  | Every migration applied from empty to PGlite, with the RLS posture asserted on the result (`scripts/check-pglite-migrations.mjs`). Added to the roster by [#2538](https://github.com/pdcarlson/Frapp/issues/2538) and **not yet applied to live protection**: until the apply, `validate-deploy-sha` already requires it for production deploys while a PR can still merge past a red run |
