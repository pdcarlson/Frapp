...
| `changes`            | Path filter deciding whether `web-tests`, `web-responsive-floor` and `landing-fold` run; required only because they need it |
...
| `landing-fold` | The landing's fold holds the reskin boards' geometry at 1440x900 and 390x844 ([`fold.spec.ts`](../../../apps/landing/tests/visual/fold.spec.ts)). Playwright, same no-baseline lane as the row above; serves a production build, because `next dev` never arms the page's reveals — added by [#2368](https://github.com/pdcarlson/Frapp/issues/2368) and **not yet applied to live protection** |
...
**Not required on branches (informational):** `pglite-migrations` is advisory, as is `duplicate-detection` ([why, in `QUALITY_GATES.md`](../ci-cd/QUALITY_GATES.md#the-gates-and-why-each-has-the-posture-it-does)). Both are intentionally omitted from [`scripts/ci/lib/required-checks.mjs`](../../../scripts/ci/lib/required-checks.mjs).
