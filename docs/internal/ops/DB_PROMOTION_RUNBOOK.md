...
- **If the old and new API overlapped across the top of the hour**: Render keeps the old instance serving until the new one is live ([ADR-24](../../../spec/architecture/adr/adr-24.md)). If the new instance's first `palette sweep: recomputed …` line is timestamped before the deploy went live, an accent save the old, pre-#1165 instance served in between wrote the old engine's palette and left the sweep's stamp current, so no later tick re-derives it. Once the deploy is live, re-queue every row: `update public.chapters set theme_palette_engine_version = null;`. The next tick recomputes them all again, which is harmless. Only this first deploy has the gap: from #1165 on, an older instance stamps its own lower version, and the sweep picks its writes up.
