...
* **Checks**: After `db push`:
  * `select polname, pg_get_expr(polqual, polrelid) from pg_policy p join pg_class c on c.oid = p.polrelid where c.relname = 'chat_message_actions' and polcmd = 'r';` returns **exactly one** row, whose expression contains `can_read_chat_message(message_id)` joined by `AND` (a second permissive SELECT policy would OR together and silently re-open the leak).
