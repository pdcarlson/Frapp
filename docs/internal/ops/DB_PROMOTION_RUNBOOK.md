...
- **If the deploy was rolling out at the top of the hour**: Render's deploys overlap the old and new instances briefly ([ADR-24](../../../spec/architecture/adr/adr-24.md)), and the new instance registers its cron at startup, before it goes live. So its first sweep can run while the old, pre-#1165 instance is still serving. A palette write the old instance served after the sweep had written that row (an accent save, a config PATCH carrying colours, or `POST /v1/chapters/:id/theme-palette`) left the old engine's palette under the sweep's current stamp, and no later tick re-derives it. Render's events show when the deploy started and when it went live, but no event marks the old instance's exit after its drain. So if a top of the hour fell between the deploy's start and a few minutes after it went live, re-queue every row once the old instance is gone: `update public.chapters set theme_palette_engine_version = null;`. The next tick recomputes them all again, which is harmless, so re-queue whenever you're unsure. Don't judge by the `palette sweep: recomputed …` line: it is logged when the whole sweep finishes, not when it starts writing. Only this first deploy has the gap: from #1165 on, an older instance stamps its own lower version, and the sweep picks its writes up.
