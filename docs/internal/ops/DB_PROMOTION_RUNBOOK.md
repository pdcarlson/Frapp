...
* **Purpose**: Data-only backfill so existing chapters match new seeds: Treasurer gains `polls:view_all` where missing; Vice President and Secretary system rows are inserted with **both** `members:view` and `polls:view_all` (both are required on insert — `PollController`'s class-level guard needs `members:view`), and `display_order` is shifted for chapters that lacked VP. The convention, and why the `20260417150000` repair below still exists, are owned by [`database.md`](../../guides/database.md) § 3. Conventions.
