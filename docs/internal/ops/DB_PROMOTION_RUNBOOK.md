...
* **Checks**: After `db push`:
  ...
  * `select prosecdef, proconfig from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'can_read_chat_message';` returns `prosecdef = true` and `proconfig = {search_path=public}`. **On a fully-migrated database the expected value is `{"search_path=public, pg_temp"}` instead**: `20260827190000_secdef_search_path_pg_temp.sql` later re-pins this function (and six others) to `public, pg_temp`, so `{search_path=public}` is the correct answer only at this point in the sequence. Getting the longer value here is not a failure. **That migration has no promotion entry of its own in this runbook** — no Shape, Locks, Checks or Rollback — which is tracked as [#1702](https://github.com/pdcarlson/Frapp/issues/1702); until it lands, an operator promoting in sequence has no post-promotion check that all seven functions took the hardened `search_path`.
...
* **Purpose**: Data-only backfill so existing chapters match new seeds: Treasurer gains `polls:view_all` where missing; Vice President and Secretary system rows are inserted with **both** `members:view` and `polls:view_all` (both are required on insert — `PollController` **and** `PointsController` each carry a class-level `members:view` guard, so dropping it costs VP/Secretary the whole `/points` surface as well as polls), and `display_order` is shifted for chapters that lacked VP. The convention, and why the `20260417150000` repair below still exists, are owned by [`database.md`](../../guides/database.md) § 3. Conventions.
