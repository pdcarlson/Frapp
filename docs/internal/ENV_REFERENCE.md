...
| `NEXT_PUBLIC_API_URL` | `lib/providers/frapp-client-provider.tsx`, `lib/providers/network-provider.tsx` (same value; the web app strips a trailing `/v1` before calling openapi-fetch so `/v1` paths are not doubled) | ✅ |
