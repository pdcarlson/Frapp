...
1. `docs/internal/redesign/README.md` and `docs/internal/redesign/master-plan.md` (whole thing).
...
4. **Update spec docs.**
   - `spec/ui-brand-identity.md`: new palette + typography + radii. Note that chapter theming overlays on top (described in `master-plan.md`).
   - `spec/ui-web-dashboard.md`: sidebar lockup, BETA badge spec, default route change.
   - `spec/product.md`: add the "chat is the spine" positioning statement (one short section pointing at the master plan).
...
- Open a PR titled `Chunk 01 — Foundation: theme + shell + design reference`. PR body: link to `docs/internal/redesign/chunks/01-foundation.md` and tick the verification checklist.
- Update `docs/internal/redesign/STATUS.md` row 01 to `in review` with PR link. Move to `shipped` on merge.
