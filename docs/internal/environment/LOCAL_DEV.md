...
> Cloud VM bringup (Cursor `start` / Claude SessionStart) lives in
> [`CLOUD_SANDBOX.md`](./CLOUD_SANDBOX.md). This file is the **laptop/local** path.
...
Cursor Cloud `start` is `scripts/cursor-cloud-up.sh` (via `scripts/cursor-agent-start.sh`);
wait on `.cloud-sandbox-up.done` / `.cloud-sandbox-up.failed`. Claude SessionStart is the
fallback and shares that bringup. Full config: [`CLOUD_SANDBOX.md`](./CLOUD_SANDBOX.md).
...
- [`CLOUD_SANDBOX.md`](./CLOUD_SANDBOX.md) — Cursor Cloud `start` and Claude SessionStart fallback
