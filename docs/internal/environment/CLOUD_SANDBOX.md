...
lock under the same guard ([#2547](https://github.com/pdcarlson/Frapp/issues/2547)): it refuses while
another bringup from this boot is running, and replaces a lock whose bringup is dead or from an
earlier boot, so no remedy needs the lock removed first. It also relaunches when the lock carries a
