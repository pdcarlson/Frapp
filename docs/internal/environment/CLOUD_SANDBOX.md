...
still starting, since another session start may have taken it a moment ago, and concurrent
session starts make this decision one at a time, under an `flock` on
`/tmp/cloud-sandbox-up.lock.guard`. Run by hand, `bash scripts/cloud-sandbox-up.sh` takes the same
lock under the same guard and judges it by the same rule, `bringup_lock_live` in
...
this boot is running or starting, and replaces a lock whose bringup is dead or from an earlier
boot, so no remedy needs the lock removed first. A hand run writes `/tmp/cloud-sandbox-up.log` as
well as the terminal, so the log a session start points at is always the running bringup's. When
the refusal names a bringup that is alive but hung (its log has stopped advancing), `kill` the pid
it names, then run bringup again. It also relaunches when the lock carries a
