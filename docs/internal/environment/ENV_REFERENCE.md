...
| Variable                      | `dev`           | `staging`                                                                                                                                                                          | `prod`                                                                                                             |
| ----------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `BACKUP_S3_ENDPOINT`          | _(leave empty)_ | `https://<ACCOUNT_ID>.r2.cloudflarestorage.com` — Cloudflare dashboard → R2 Object Storage → Account Details → S3 API. No trailing slash, no bucket name.                            | _(same value as staging — not yet set here; the production backup jobs inject `staging` first to read it, see [`DB_ROLLBACK_PLAYBOOK.md`](../ops/DB_ROLLBACK_PLAYBOOK.md#backups-what-exists) § Backups: what exists. Setting it here makes that injection redundant)_ |
| `BACKUP_S3_BUCKET`            | _(leave empty)_ | The R2 backup bucket's name (Cloudflare dashboard → R2 Object Storage → the private bucket created for #1287). Just the name, no `s3://`.                                            | _(same bucket — since 2026-09-06 the `db-offsite-backup` action takes the prefix from its `environment` input, so production dumps land under `production/` and Storage under `storage-production/` in this same bucket. Not yet set in `prod`; read from `staging` as above)_ |
