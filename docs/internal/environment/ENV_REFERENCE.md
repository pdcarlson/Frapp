...
| `SENTRY_AUTH_TOKEN`                     | `next.config.js` (`lib/sentry/build-config.js`)                                                       | ❌ — optional; when absent, source-map **upload** is skipped but debug IDs are still injected. Upload targets project `frapp-landing`. |
