...
| `RESEND_FROM_EMAIL` | _(leave empty → default `Signet <invites@mail.frapp.live>`)_                          | `Signet <invites@mail.staging.frapp.live>` (verified `mail.staging.frapp.live`) | _(leave empty → default `Signet <invites@mail.frapp.live>`)_                                                                                                                                          |
...
| `BACKUP_S3_BUCKET`            | _(leave empty)_ | The R2 backup bucket's name (Cloudflare dashboard → R2 Object Storage → the private bucket created for #1287). Just the name, no `s3://`.                 | _(**not yet set**; a separate production bucket is the target, per the endpoint row. Today the `db-offsite-backup` action takes the prefix from its `environment` input, so production dumps land under `production/` and Storage under `storage-production/` in the shared staging bucket)_                                                                                                                                                                                           |
...
| `SENTRY_AUTH_TOKEN`           | `apps/api/Dockerfile` builder (optional; **build-time only**). After `nest build`, `upload-sentry-sourcemaps.js` injects debug IDs and uploads `apps/api/dist` to Sentry org `frapp-live` / project `frapp-api` with `--release` = `RENDER_GIT_COMMIT`. **Unset → skip upload, still strip `*.map` from the image.** Same Infisical name as web/landing; path `/` is what the Render syncs read. Do not `ENV` it in the Dockerfile. Not read at runtime. | ❌       |
...
| `RESEND_FROM_EMAIL`           | `modules/email/email.module.ts` (from-address override; default `Signet <invites@mail.frapp.live>`)                                                                                                                                                                                                                                                                                                                                                      | ❌       |
...
| `NEXT_PUBLIC_API_URL`                   | `lib/providers/frapp-client-provider.tsx` (SDK base URL), `lib/providers/network-provider.tsx` (health poll — `/health` is the one route outside `/v1`). Unset becomes `http://localhost:3001` in `FrappProvider`. A Vercel Production build (`VERCEL_ENV=production`) fails at `next.config.js` unless the origin is `https://api.frapp.live` (callout under `APP_URL`) | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `SENTRY_AUTH_TOKEN`                     | `next.config.js` (`@repo/observability/next/sentry-build-config.js`), and `apps/mobile`'s `@sentry/react-native/expo` config plugin (both build-time only, never bundled)                                                                                                                                                                                                | ❌ — optional; when absent, source-map **upload** is skipped but debug IDs are still injected (`sourcemaps.disable` is always false). Only needed in the deploy environment for readable stack traces. **The mobile half is a second, separate place to set it:** the Expo plugin resolves it from the environment at EAS build time (the generated `sentry.properties` carries no token, only `# Using SENTRY_AUTH_TOKEN environment variable`), and there is no Infisical→EAS sync, so it does not arrive on its own. Unset, a mobile build still succeeds and every `frapp-mobile` stack trace arrives minified. |
...
DSN names (different projects: `frapp-web`, `frapp-mobile`, `frapp-landing`) and the PostHog
key/host pair on mobile (EAS dashboard; there is no Infisical→EAS sync). No secret belongs
