...
>
> **Staging (and local) must use that same Signet account's test keys**, not a second test-mode Stripe account. `STRIPE_PRICE_ID` above lives on Signet. A sibling sandbox (`acct_1U932aKySXtAfQjP`, "Signet sandbox") can hold customers and still have **zero Prices** — checkout then 503s with Stripe `resource_missing` on the price. Do not copy that account's `STRIPE_SECRET_KEY` into Infisical.
>
...
| `NEXT_PUBLIC_SENTRY_RELEASE`           | `lib/sentry/options.ts` — **derived, never configured**                                                                                                 | 🚫 — **do not set this anywhere, including Infisical.** `next.config.js` maps it from Vercel's `VERCEL_GIT_COMMIT_SHA`. Empty when that SHA is unset (local/CI). Same reason as `NEXT_PUBLIC_SENTRY_ENVIRONMENT`: a hand-set copy would silently disagree with the deployment. |
...
| `NEXT_PUBLIC_SENTRY_RELEASE`             | `lib/sentry/options.ts` — **derived, never configured**                                               | 🚫 — **do not set this anywhere.** `next.config.js` maps it from `VERCEL_GIT_COMMIT_SHA`. |
...
| `NEXT_PUBLIC_SENTRY_RELEASE`                                                 | Sentry `release` (git SHA). **Derived** from `VERCEL_GIT_COMMIT_SHA` in web and landing `next.config.js`; do not set in Infisical. Empty when that SHA is unset. Mobile's release is `bundleId@version+nativeBuildNumber` from expo-application, and the git SHA is `extra.gitSha` metadata, not an `EXPO_PUBLIC_` variable |
