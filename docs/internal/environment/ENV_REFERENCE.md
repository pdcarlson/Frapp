...
| `SENTRY_TRACES_SAMPLE_RATE`   | `infrastructure/observability/sentry-options.ts` (parsed by `@repo/observability` `parseSampleRate`: finite `[0, 1]`, default `0.1`; set-but-unusable values fall back and warn at boot — #2040)                                                                                                          | ❌       |
...
| `NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE` | `lib/sentry/options.ts` (parsed by `@repo/observability` `parseSampleRate`: finite `[0, 1]`, default `0.1`)                                              | ❌ — optional; same default and parser as the API's `SENTRY_TRACES_SAMPLE_RATE`. Set-but-unusable values fall back to `0.1` and warn at boot rather than yielding `NaN`.                                                                                                                                                                   |
