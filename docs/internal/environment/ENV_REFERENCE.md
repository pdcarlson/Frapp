...
> **Which provider was selected is visible in the boot log.** `selectAnalyticsProvider()` logs its choice under the `AnalyticsProvider` context every time: the PostHog host when a key is set, a plain `log` line when neither the key nor the salt is set (the intended local/CI state), and a **warning** when `ANALYTICS_HMAC_SALT` is set but `POSTHOG_API_KEY` is not — the case where events are pseudonymized and then silently discarded. Named product events still post to `POST /v1/analytics/events`. `apps/web` also loads PostHog JS with the write-only `phc_` token (`NEXT_PUBLIC_POSTHOG_KEY` = `${POSTHOG_API_KEY}`) and `apps/mobile` loads PostHog RN with `EXPO_PUBLIC_POSTHOG_KEY` (EAS dashboard — there is no Infisical→EAS sync) for identify / groups / flags / replay-gates / the correlation marker — that token is public by design, like a Sentry DSN. The salt and any personal API key stay out of the bundle.
...
| `NEXT_PUBLIC_POSTHOG_KEY`            | `${POSTHOG_API_KEY}`                                    | apps/web           |
| `NEXT_PUBLIC_POSTHOG_HOST`           | `${POSTHOG_HOST}`                                       | apps/web           |
...
The prefixed set is deliberately small. It is **exactly these twenty-four variables** — enumerated by name
...
| `NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE`                                     | Web tracing sample rate (finite `[0, 1]`, default `0.1`, parsed by `@repo/observability`). **Web only** — `apps/mobile` hardcodes `DEFAULT_TRACES_SAMPLE_RATE` in `lib/sentry/options.ts` rather than reading a variable, so there is no `EXPO_PUBLIC_` twin. Carries no credential |
| `NEXT_PUBLIC_SENTRY_RELEASE`                                                 | Web Sentry `release` (git SHA). **Derived** from `VERCEL_GIT_COMMIT_SHA` in `next.config.js`; do not set in Infisical. Empty when that SHA is unset. **Web only** — mobile's release is `bundleId@version+nativeBuildNumber` from expo-application, and the git SHA is `extra.gitSha` metadata, not an `EXPO_PUBLIC_` variable |
| `NEXT_PUBLIC_POSTHOG_KEY` · `EXPO_PUBLIC_POSTHOG_KEY`                   | Write-only PostHog `phc_…` project token. Public by design (ingest, not read), like a Sentry DSN. Unset → the SDK is never constructed. The web half is Infisical `${POSTHOG_API_KEY}` synced to Vercel; the mobile half is entered in the EAS dashboard, because no Infisical→EAS sync exists |
| `NEXT_PUBLIC_POSTHOG_HOST` · `EXPO_PUBLIC_POSTHOG_HOST`                 | PostHog ingest host. Defaults to `https://us.i.posthog.com`. The web half is Infisical `${POSTHOG_HOST}`; the mobile half is EAS-optional |
...
Twenty-four variables, enumerated above. Each `NEXT_PUBLIC_`/`EXPO_PUBLIC_` pair that
shares a canonical is an Infisical reference to the same value — exceptions are the Sentry
DSN pair (different projects) and the PostHog key/host pair on mobile (EAS dashboard; there
is no Infisical→EAS sync). No secret belongs in this set; `ANALYTICS_HMAC_SALT` is the
