...
| `SENTRY_TRACES_SAMPLE_RATE`   | `infrastructure/observability/sentry-options.ts` (default: `0.1`; **current parse** is `Number(env ?? '0.1')`, so a malformed value yields `NaN` and the SDK treats tracing as on. Intended clamp: [`observability.md` § Sampling](../../../spec/behavior/observability.md#sampling). Closed #904 covered mobile only.) | ❌       |
...
| `NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE` | `lib/sentry/options.ts` (default: `0.1`)                                                                                                                | ❌ — optional; same default as the API's `SENTRY_TRACES_SAMPLE_RATE`. **Current parse** still yields `NaN` on a malformed value (SDK treats tracing as on). Intended clamp: [`observability.md` § Sampling](../../../spec/behavior/observability.md#sampling). Closed #904 covered mobile only.                                                     |
