...
The prefixed set is deliberately small. It is **exactly these twenty-two variables** — enumerated by name
...
| `NEXT_PUBLIC_SENTRY_RELEASE`                                                 | Web Sentry `release` (git SHA). **Derived** from `VERCEL_GIT_COMMIT_SHA` in `next.config.js`; do not set in Infisical. Empty when that SHA is unset. **Web only** — no `EXPO_PUBLIC_` twin in this slice |
| `NEXT_PUBLIC_POSTHOG_KEY`                                                   | Write-only PostHog `phc_…` project token. Infisical `${POSTHOG_API_KEY}`. Public by design (ingest, not read), like a Sentry DSN. Unset → `posthog.init` is never called. **Web only** in this slice (landing WS6, mobile WS7) |
| `NEXT_PUBLIC_POSTHOG_HOST`                                                  | PostHog ingest host. Infisical `${POSTHOG_HOST}`; defaults to `https://us.i.posthog.com`. **Web only** in this slice |
...
Twenty-two variables over **six** canonical values plus ten direct-set or derived entries; each
