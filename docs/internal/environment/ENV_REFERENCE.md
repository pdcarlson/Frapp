...
> **Which provider was selected is visible in the boot log.** `selectAnalyticsProvider()` logs its choice under the `AnalyticsProvider` context every time: the PostHog host when a key is set, a plain `log` line when neither the key nor the salt is set (the intended local/CI state), and a **warning** when `ANALYTICS_HMAC_SALT` is set but `POSTHOG_API_KEY` is not — the case where events are pseudonymized and then silently discarded. Named product events still post to `POST /v1/analytics/events`. `apps/web` and `apps/landing` also load PostHog JS with the write-only `phc_` token (`NEXT_PUBLIC_POSTHOG_KEY` = `${POSTHOG_API_KEY}`) — that token is public by design, like a Sentry DSN. Web uses it for identify / groups / flags / replay-gates / the correlation marker. Landing uses it for anonymous page/CTA analytics only (no identity, no alias, no chapter group). The salt and any personal API key stay out of every bundle.
...
The prefixed set is deliberately small. It is **exactly these twenty-three variables** — enumerated by name
...
| `NEXT_PUBLIC_SENTRY_RELEASE`                                                 | Sentry `release` (git SHA). **Derived** from `VERCEL_GIT_COMMIT_SHA` in web and landing `next.config.js`; do not set in Infisical. Empty when that SHA is unset. No `EXPO_PUBLIC_` twin |
| `NEXT_PUBLIC_POSTHOG_KEY`                                                   | Write-only PostHog `phc_…` project token. Infisical `${POSTHOG_API_KEY}`. Public by design (ingest, not read), like a Sentry DSN. Unset → `posthog.init` is never called. **Web and landing** (landing is anonymous page/CTA only). No `EXPO_PUBLIC_` twin in this slice (WS7) |
| `NEXT_PUBLIC_POSTHOG_HOST`                                                  | PostHog ingest host. Infisical `${POSTHOG_HOST}`; defaults to `https://us.i.posthog.com`. **Web and landing** |
...
Twenty-three variables over **six** canonical values plus eleven direct-set or derived entries; each
