...
Ensure pull requests targeting `main` and `production` always receive a CodeRabbit review request.
...
2. **Workflow fallback trigger**: `.github/workflows/trigger-coderabbit-review.yml`
   - Runs on `pull_request_target` for `main` and `production`.
...
2. Verify CodeRabbit UI repository settings:
   - Auto review enabled.
   - Base branch settings include `main` / `production` if required by plan.
