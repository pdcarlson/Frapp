...
- Added comprehensive unit test coverage for the `UserController` endpoints (`getMe`, `updateMe`, `requestAvatarUploadUrl`) and verified correct application of guards (`SupabaseAuthGuard`, `ChapterGuard`, `PermissionsGuard` with `members:view`) and interceptors (`AuthSyncInterceptor`). User routes now align with other chapter-scoped controllers: clients must send `x-chapter-id` so `ChapterGuard` can resolve membership before `PermissionsGuard` runs. OpenAPI (`apps/api/openapi.json`) and the generated API SDK types document the `chapter-id` security requirement and required `x-chapter-id` header on those operations.
