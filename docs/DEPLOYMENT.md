...
| `NEXT_PUBLIC_API_URL`           | `https://api.frapp.live` (no `/v1` suffix; legacy `.../v1` is stripped in the client) | `https://api-staging.frapp.live` |
...
→ Expo Go requires the API to be network-accessible. Use the deployed staging URL, not `localhost`. For local dev, use your machine's LAN IP (e.g., `http://192.168.1.x:3001` as `EXPO_PUBLIC_API_URL`; paths include `/v1/...` from the OpenAPI contract).
