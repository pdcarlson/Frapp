...
- **Do not stack a throttle on a route that already has a domain-level limit.** `POST /v1/points/adjust` already carries a domain-level adjustments/hour limit inside `PointsService` (owned by [`spec/behavior/points.md`](../../spec/behavior/points.md) § Anti-Fraud, which states its exact scoping and default) (see [`spec/behavior/points.md`](../../spec/behavior/points.md) § Anti-Fraud for the default); a second control with different semantics on the same route makes the effective limit unreadable.
