...
The global `ValidationPipe` runs `whitelist: true` + `forbidNonWhitelisted: true`, so an unexpected property is rejected with a 400 rather than silently dropped. It is registered by `configureApp` (`apps/api/src/bootstrap.ts`), not in `main.ts`, and its flags live in `VALIDATION_PIPE_OPTIONS` (`apps/api/src/interface/pipes/validation-pipe.options.ts`) — every boot path, tests included, goes through `configureApp` so none of them can drift from it ([`testing.md`](./testing.md)). Two conventions keep that baseline honest:
