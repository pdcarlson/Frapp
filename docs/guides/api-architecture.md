...
When a service loads the **same shape** of related rows for many parent records (for example, the author avatar path behind each message on a chat page), prefer **one batched repository query** (for example `SupabaseChatMessageRepository.findAuthorAvatarPaths`, which does `.in('id', messageIds)` on PostgREST) and group results in memory. Issuing one query per parent—even via `Promise.allSettled`—still creates N+1 HTTP calls to PostgREST and can exhaust connection pools under large limits. Batch-then-group remains the right default wherever no aggregate exists.
