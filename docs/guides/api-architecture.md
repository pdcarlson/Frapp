...
Zod schemas in `packages/validation` are shared with the web and mobile forms for UX. They are **not** enforcement — a curl request never runs them — so any rule that matters server-side must also exist on the DTO. The same package also owns the three shared **client gates** — `can` / `canAll` / `canAny` (`src/permissions.ts`), `isModuleEnabled` (`src/index.ts`), and `subscriptionWriteState` (`src/subscription.ts`) — so a surface cannot drift from the server's permission, module, or subscription rules. The gates are UX only; the API remains the trust boundary.
