...
The two halves need two different tests, because the ordering is **unreachable over HTTP**: whitelisting rejects a colliding key before the controller runs, so an end-to-end request cannot tell `{ ...dto, chapter_id }` from `{ chapter_id, ...dto }`. `apps/api/test/mass-assignment.e2e-spec.ts` covers the whitelisting half (and imports `VALIDATION_PIPE_OPTIONS` from `apps/api/src/interface/pipes/validation-pipe.options.ts`, the same object `configureApp()` uses, so loosening a flag there fails that suite rather than a local copy of it). `apps/api/src/interface/controllers/write-payload-ordering.spec.ts` covers the ordering half by calling the controller methods directly with a DTO that carries the hostile key — the shape a future DTO change would produce. Both were verified to fail when their respective regression is reintroduced.

Zod schemas in `packages/validation` are shared with the web and mobile forms for UX. They are **not** enforcement — a curl request never runs them — so any rule that matters server-side must also exist on the DTO. The package is on Zod 4; `z.record` takes a key schema and a value schema (`z.record(z.string(), z.boolean())`). The one-argument form was Zod 3 and does not type-check. The same package also owns the four shared **client gates** — `can` / `canAll` / `canAny` (`src/permissions.ts`), `isModuleEnabled` (`src/index.ts`), `subscriptionWriteState` (`src/subscription.ts`), and `isAnalyticsOptedOut` (`src/analytics-opt-out.ts`) — so a surface cannot drift from the server's permission, module, subscription, or analytics-opt-out rules. The gates are UX only; the API remains the trust boundary.
...
Both hard locks are scoped to the **guarded surface**. Two sets of routes survive them by design: `@SubscriptionExempt()` ones (below) — billing recovery, so a lapsed chapter can still pay its way back, **and member safety**, so chat report/block keep working while it is locked; and routes that declare no `ChapterGuard` at all — `PATCH /v1/notifications/preferences`, `POST /v1/analytics/events` — which are the member's own settings and telemetry and were never subscription-gated. `POST /v1/invites/redeem` also declares no guard (the chapter that matters is the invite's, which the guard never sees) but applies the same hard-lock rule inside `InviteService.redeem` against `invite.chapter_id`, via `isSubscriptionHardLocked` in `#domain/constants/subscription-grace` (#1546).
...
- **Do not stack a throttle on a route that already has a domain-level limit.** `POST /v1/points/adjust` already carries a domain-level adjustments/hour limit inside `PointsService` (owned by [`spec/behavior/points.md`](../../spec/behavior/points.md) § Anti-Fraud, which states its exact scoping and default); a second control with different semantics on the same route makes the effective limit unreadable.
...
When a service loads the **same shape** of related rows for many parent records (for example, the author avatar path behind each message on a chat page), prefer **one batched repository query** and group results in memory. Issuing one query per parent—even via `Promise.allSettled`—still creates N+1 HTTP calls to PostgREST and can exhaust connection pools under large limits. Batch-then-group remains the right default wherever no aggregate exists.

`SupabaseChatMessageRepository.findAuthorAvatarPaths` is the shape to copy, and **the scoping predicate is part of the pattern, not incidental to that method**: it filters `.eq('channel_id', channelId)` *and then* `.in('id', messageIds)`. The API holds the `service_role` key and bypasses RLS, so an id list is never on its own a tenant boundary — a batched read with only `.in('id', ids)` returns whatever those ids name, in any chapter. Carry the parent scope into every batched query, exactly as the per-row query it replaced did.

**Better still, when the answer is an aggregate, do not transfer the rows at all.** Poll tallies are the worked example, and deliberately no longer an example of the paragraph above: no code path batch-loads `poll_votes` rows any more, because both read paths call `GROUP BY` RPCs and receive one row per option instead of one row per vote. The paged row read this section used to describe (`findByMessage` / `findByMessages`) was deleted with its last caller.

- `PollService.listPolls` calls `IPollVoteRepository.aggregateOptionTotalsByMessages` (RPC `get_poll_vote_option_totals`) once per request, plus `findUserVotesByMessagesForUser` (RPC `get_poll_user_votes_for_messages`) when the caller opted into `userVotes`. Only the fan-out of those returned totals into a `Map` happens in memory.
- `PollService.getPoll` calls the same totals RPC with a single message id. It previously read every `poll_votes` row for the poll and filtered the array once per option, which cost O(votes) on the wire and O(options × votes) to scan.
...
Where an aggregate RPC is not available and you do have to batch-load the rows, group them in **one pass** into a `Map` keyed by parent id instead of running a nested filter per parent per field — the nested form scales with parents × fields × rows.
