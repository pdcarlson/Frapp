...
- **Do not stack a throttle on a route that already has a domain-level limit.** `POST /v1/points/adjust` enforces a per-chapter adjustments/hour limit inside `PointsService`, read from `chapter_points_config.adjustment_rate_limit_per_hour` (see [`spec/behavior/points.md`](../../spec/behavior/points.md) § Rate limiting for the default); a second control with different semantics on the same route makes the effective limit unreadable.
