...
The global `ValidationPipe` runs `whitelist: true` + `forbidNonWhitelisted: true`, so an unexpected property is rejected with a 400 rather than silently dropped. It is registered by `configureApp` (`apps/api/src/bootstrap.ts`), not in `main.ts`, and its flags live in `VALIDATION_PIPE_OPTIONS` (`apps/api/src/interface/pipes/validation-pipe.options.ts`) — every **serving** boot path goes through `configureApp`, tests included, so none of them can drift from it ([`testing.md`](./testing.md)). The one deliberate exception is `apps/api/src/export-openapi.ts`, which builds an app purely to emit the contract and never serves a request, so it does not call `configureApp`. Two conventions keep that baseline honest:
