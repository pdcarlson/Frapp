...
When a service loads the **same shape** of related rows for many parent records (for example, the author avatar path behind each message on a chat page), prefer **one batched repository query** (for example `SupabaseChatMessageRepository.findAuthorAvatarPaths`, which does `.in('id', messageIds)` on PostgREST) and group results in memory. Issuing one query per parent—even via `Promise.allSettled`—still creates N+1 HTTP calls to PostgREST and can exhaust connection pools under large limits.
...
Both paths fan those rows out through one private `groupTotalsByMessage` helper rather than each writing its own grouping. That is not only deduplication: it forces both to key on the `message_id` **the database returned**. A `uuid` is 128 bits, so Postgres accepts any case on the way in and always renders it canonically lower-case on the way out — a route parameter can therefore match every row inside the query and still fail `===` against every row that comes back, and the detail view renders a real poll as every option at zero. Look tallies up by `message.id`, never by the id the caller sent.
...
The two paths differ deliberately on failure. `listPolls` swallows an aggregate failure, returns the list with zero tallies (degraded response), and logs it with Nest's `Logger` so operators can diagnose storage or connectivity issues. `getPoll` lets it propagate: on a single-poll detail view, every option rendered at zero is indistinguishable from a real result, so a degraded response there would be a silently wrong one.
