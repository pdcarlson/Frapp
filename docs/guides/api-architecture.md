...
**Better still, when the answer is an aggregate, do not transfer the rows at all.** Poll tallies are the worked example: both read paths call `GROUP BY` RPCs and receive one row per option instead of one row per vote.
