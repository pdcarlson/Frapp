...
When a service loads the **same shape** of related rows for many parent records (for example, all `poll_votes` rows for each poll on a chapter list), prefer **one batched repository query** (for example `.in('message_id', ids)` on PostgREST) and group results in memory. Issuing one query per parent—even via `Promise.allSettled`—still creates N+1 HTTP calls to PostgREST and can exhaust connection pools under large limits. **`PollService.listPolls`** went one step further and no longer reads the rows at all: it tallies in Postgres via `aggregateOptionTotalsByMessages` and, for `userVotes`, `findUserVotesByMessagesForUser` — both RPC-backed, so vote rows never cross the wire. Each is guarded independently: if either throws, the handler still returns the poll list (with zero tallies, or without `userVotes`) but logs the failure with Nest’s `Logger` so operators can diagnose storage or connectivity issues. Batch-then-group in memory, as described above, remains the right default wherever no aggregate exists.

`SupabasePollVoteRepository.findByMessages` — now reached only through the single-message `findByMessage`, and deliberately off `IPollVoteRepository` — paginates with `.range()` (1000-row pages) so heavy chapters do not hit PostgREST default row caps and return incomplete vote tallies. Pages are ordered by primary key `id` so offset pagination stays deterministic when rows change between requests (avoids skipped or duplicated rows affecting tallies).
