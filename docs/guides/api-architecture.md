...
- **Do not stack a throttle on a route that already has a domain-level limit.** `POST /v1/points/adjust` enforces a per-admin adjustments/hour limit inside `PointsService` — `countRecentAdjustments(adminUserId, chapterId, …)`, so the ceiling is chapter-configured but counted per admin — read from `chapter_points_config.adjustment_rate_limit_per_hour` (see [`spec/behavior/points.md`](../../spec/behavior/points.md) § Anti-Fraud for the default); a second control with different semantics on the same route makes the effective limit unreadable.
