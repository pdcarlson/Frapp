...
> **Note:** Signet icons and the marketing lockup are synced from `packages/brand-assets/` into each Next app on **`next build`** (`prebuild`). After editing the vector master `packages/brand-assets/assets/signet-emblem-B.svg`, run `npm run rasterize:brand-assets` then `npm run sync:brand-assets` from the repo root (or build once). See `spec/ui/assets.md`.
