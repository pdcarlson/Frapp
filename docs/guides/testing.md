...
`scripts/ci/lib/required-checks.mjs`), so all three gate PRs to `main` without a
...
dependency is one published `"type": "module"` with no CJS build.

**Most of this section is now history — read the flag subsection below first.** Jest loads ESM-only
packages natively here, so the failure described next is no longer the default outcome. It is kept
because the *diagnosis* is unchanged and the failure still occurs for packages `require(esm)` cannot
serve, and because the trap it describes is the reason the flag is not optional.

Historically, Jest resolved through its own module registry rather than Node's loader and, by
default, transformed nothing under `node_modules`. Every suite that transitively imported such a
package failed with `SyntaxError: Unexpected token 'export'` before a single test ran.

The trap was that **nothing else went red**. Node 20.19+ and 22.12+ can `require()` an ES module,
and TypeScript allows the import under `module: nodenext`, so `check-types`, `nest build`, and
`api-docker-build` all stayed green — the API really would boot. `api-tests` was the only check that
saw it, and it reports suites that *failed to run*, not tests that failed. Dependabot's
...
`jest` invocation in `apps/api/package.json`. Remove it from one and that script alone starts
...
the two suites importing `@Cron` could not load. With the flag the whole unit suite passes unchanged
...
The script names are not restated here on purpose. `apps/api/package.json` is the roster, and
[`scripts/ci/__tests__/api-jest-esm-flag.test.mjs`](../../scripts/ci/__tests__/api-jest-esm-flag.test.mjs)
asserts every `jest` invocation in it carries the flag — so a dropped prefix fails a named test
rather than one unrelated suite. That guard runs under `node --test`, not Jest, deliberately: a
check on Jest's own launch flags must not depend on Jest launching.
...
- **`apps/api/src/application/services/chat.service.spec.ts`** — pins the wiring of `sendMessage` (idempotent insert on `client_message_id`, dedup-as-success on the partial unique index) and `recordMessageAction` (atomic dedup on the `chat_message_actions` unique index, vote-action UPSERT per ADR-07, no false-positive dedup on non-23505 errors). Also owns § "mint and send agree", the table asserting that `requestChatUploadUrl` and `sendMessage` reach the same authorization verdict for every channel shape — the guard against the two drifting apart (#2186). That is a service-level parity check, not a restatement of the predicate matrix below, and production code cites it by name.
...
- **Pin the row shape to the canonical entity**, e.g.
  ...
  otherwise goes silent in production while `tsc` stays quiet.
  **Know what that pin does and does not buy.** It is enforced by ts-jest when the suite runs
  (`npm run test -w apps/api`, which CI runs) and **not** by `npm run check-types`, whose
  `tsconfig.build.json` excludes `**/*spec.ts`. And the entity is hand-authored rather than
  generated by `supabase gen types`, so it catches a TypeScript-side divergence but cannot by itself
  catch a migration that renames a column without touching the entity. Closing that second gap needs
  schema-derived types, which this repo does not have.
- **Assert a positive outcome, not just an absent one.** `handleMessage` wraps its body in a
  try/catch that logs and swallows, so `expect(notifyUser).not.toHaveBeenCalled()` is satisfied
  identically by "the rule correctly skipped" and by "the worker threw". Pair every such negative
  with a case on a channel whose default level delivers, where a landed push proves execution
  actually reached the code under test.
- **Check that the fixture can express the failure.** Preference-mute cases must run on a channel
  whose *default* level would otherwise deliver (`announcements` → `all`); on a `mentions`-default
...
| Mutation | Tests that fail |
| --- | --- |
| Subscribe to the wrong table | 13 of 15 |
| Subscribe to the wrong schema | 13 of 15 |
| Skip `filterCanReadChannel` (push to every chapter member) | 5 |
| Stop excluding `row.sender_id` from candidates | 6 |
| `row.mentions ?? []` → `row.mentions as string[]` | 2 |
| Fail **open** when the permission lookup throws | 1 |
| Rethrow instead of returning `[userId, null]` on a lookup failure | 1 |
| `resolveLevel` stops honouring channel-scoped preferences | 1 |
| Mention no longer overrides the level in `decidePush` | 1 |
| Never call `.subscribe()` on the registered channel | 1 |
| Drop the `removeChannel` teardown | 1 |

The first two rows are what prove the payload path is load-bearing rather than decorative: a spec
that called the handler method directly would still pass with the subscription pointed at nothing.
The two that survive them are the subscription-contract cases, which assert the join and the
teardown rather than a delivered row.

**Every row here was earned twice.** Six of these mutations left an earlier draft of this spec fully
green, and that draft looked thorough — it had the right cases with the wrong assertions. The
recurring shapes, all worth checking in any spec of this kind:

- A negative assertion (`not.toHaveBeenCalled()`) that a swallowed exception satisfies just as well
