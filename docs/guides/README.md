...
**Maintenance:** `useCurrentChapter` in `@repo/hooks` keys the React Query cache by `chapterId` (`["chapters","current", chapterId]`) so switching the active chapter refetches; pass `chapterId` from the chapter store at call sites. `AttendanceService.markAutoAbsent` uses `AttendanceRepository.createMany` (Supabase bulk `insert`) for absence rows; see [`api-architecture.md`](api-architecture.md) and [`../performance/attendance.md`](../performance/attendance.md). The dashboard ⌘K palette’s backwork search hits route `/backwork` (same as primary nav), not `/home`.
