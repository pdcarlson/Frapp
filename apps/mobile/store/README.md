...
> That narrows #2257 to client work but does not close it. The iOS binary's half is built
> as of 2026-09-22 — report a message, block and unblock a member, the tombstone and
> Settings → Blocked members (`spec/behavior/chat/README.md` § Report and block) — but it
> ships only with a binary built after that change. Nothing anywhere *reads*
...
| # | Risk |
| --- | --- |
| [#2257](https://github.com/pdcarlson/Frapp/issues/2257) | Guideline 1.2 — member-level report and block are built on mobile (2026-09-22); the officer queue that reads reports is not built anywhere |
...
| [#2257](https://github.com/pdcarlson/Frapp/issues/2257) | Guideline 1.2 (restated as a blocker, not a risk): API and production DB ship report/block and the mobile client consumes both (2026-09-22, in binaries built after that change), but **nothing reads a filed report** — the officer review surface is still owed, and so is web (#2313) | blocker |
