...
| [#2308](https://github.com/pdcarlson/Frapp/issues/2308) / [#2309](https://github.com/pdcarlson/Frapp/issues/2309) | No App Review chapter or login exists in `frapp-prod` yet, so the reviewer cannot sign in. The seed is ready: `scripts/demo/seed-demo.mjs --reviewer` targets a hosted project and was proven on staging (#2308). Applying it to production is the owner's step, [`demo-data.md` § Production (App Review)](../../../docs/guides/demo-data.md#production-app-review) | hard gate |
...
- The account signs in as the president of a fictional demo chapter, so every screen has content. Worth trying: Chat (the chapter's channels, the officer composer in #announcements, and a direct message from the chapter's treasurer), Events, Tasks (including New task), and under More: Documents (open one), Study hours, Directory, Service hours, Dues, and Profile for points. Setting a chapter up is done on the web dashboard, not in this app: roles and permissions, dues and who has paid, service-hour approvals, channels, points, and study zones.
  ...
  > The web-only list is § Description's own sentence plus study zones. **Host check-in
  > is deliberately left out.** Its QR code needs `EVENT_CHECK_IN_TOKEN_SECRET` on the
