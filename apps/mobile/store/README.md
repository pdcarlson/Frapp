...
> affect a reviewer, who signs in with the seeded account's email and password and is
...
| ~~[#2261](https://github.com/pdcarlson/Frapp/issues/2261)~~ | Fixed in the repo 2026-09-23 with #2302: the Terms say 18 or older and carry the zero-tolerance clause Guideline 1.2 expects. The server refuses to add anyone to a chapter until they accept (join, onboarding, `POST /v1/chapters`). A member from before #2302 is asked by the app's Terms prompt, which is a client gate, not a server one: [`spec/behavior/legal.md`](../../../spec/behavior/legal.md#acceptance-record) § Acceptance record says what it can't block. Live only after the next Deploy production (landing, API and web) and in a binary built after it merged |
...
| [#2308](https://github.com/pdcarlson/Frapp/issues/2308) / [#2309](https://github.com/pdcarlson/Frapp/issues/2309) | No App Review chapter or login exists in `frapp-prod` yet, so the reviewer cannot sign in. The seed is ready: `scripts/demo/seed-demo.mjs --reviewer` targets a hosted project, and its `auth` → `sql` → `storage` → `verify` path passed end to end on staging on 2026-09-23 at `69e0021` (output in [#2537](https://github.com/pdcarlson/Frapp/pull/2537); staging was cleaned afterwards). Changes made after that run, which reached `main` in the follow-up PR that links back to #2537 rather than in #2537 itself, were verified against the local stack, unit tests and the `pglite-migrations` check only: `sql` (a guard that runs between its deletes, and a refusal to adopt a login that other rows reference), `storage` (`--remove` clears the chapter's folder in every bucket, and refuses while the chapter exists) and `verify` (a stricter staleness check). Applying it to production is the owner's step, [`demo-data.md` § Production (App Review)](../../../docs/guides/demo-data.md#production-app-review) | hard gate |
| [#2257](https://github.com/pdcarlson/Frapp/issues/2257) | Guideline 1.2 (restated as a blocker, not a risk): API and production DB ship report/block. The mobile client consumes both, but only in a binary built from `main` after the #2257 member-side change merged; the web dashboard's officer queue reads filed reports and acts on them (#2311), once that is deployed. Web members now get the same block list and can unblock (#2313), but still cannot report or block from web (#2687). Stays a blocker until the submitted build and the deploy are confirmed | blocker |
...
trusting either place. The last row is the inverse: a state to
...
| Location confirms "inside a chapter study zone" | Seeded: the chapter carries study zones at fixed coordinates on its campus in Akron, Ohio, and the reviewer has past sessions in them (the required `pglite-migrations` CI check asserts both). A reviewer anywhere else is told the device is outside a zone when starting one, which is the check working; § Review notes says so, so it does not read as broken. Zone creation is web-dashboard-only, so it cannot be done from the app being reviewed |
...
- The account signs in as the president of a fictional demo chapter. On first sign-in the app asks the account to agree to the Terms of Service and Privacy Policy, which forbid objectionable content and abusive users; tick the box and continue. Worth trying: Chat (the chapter's channels, the officer composer in #announcements, and a direct message from the chapter's treasurer), Events, Tasks (including New task), and under More: Documents (open one), Directory, Service hours, Study hours, and Profile for points. Study hours lists past sessions; a new one starts only inside one of the chapter's study zones, and the demo chapter's zones are on its campus in Ohio, so elsewhere the app will say the device is outside a zone. Setting a chapter up is done on the web dashboard, not in this app: roles and permissions, dues and who has paid, service-hour approvals, channels, points, and study zones.
  > **Submitter note, not for the paste (2026-09-23, #2308).** Every screen named above
  > was checked against the binary (tabs in `app/(tabs)/_layout.tsx`, More in
  > `more.tsx`, the announcements composer and New task per § Description's verdicts)
  > and against the `--reviewer` seed: each one has the reviewer's own rows, since
  > Service hours, Study hours and Dues show the viewer's history only. **Dues is left
  > out** because the seed gives the reviewer no invoices on purpose (§ Seed the
  > reviewer's chapter), so it reads "No dues yet". The web-only list is § Description's
  > own sentence plus study zones. **Host check-in is deliberately left out too.** Its QR code needs `EVENT_CHECK_IN_TOKEN_SECRET` on the
  > production API; unset, the screen says the feature is not configured
  > ([`ENV_REFERENCE.md`](../../../docs/internal/environment/ENV_REFERENCE.md)). Staging
