...
export default function StudyScreen() {
  ...
  // and production tokens still lack the claim (empty prod until first onboard,
  // or the rollback playbook disabling the hook — not the hook being off), so a
  // strict read would hide the screen from everyone. The writes are gated
  ...
  function renderBody() {
    ...
    // and `chapterId` is still null for every production member (empty prod
    // until first onboard, or the playbook has disabled the hook), so any such
