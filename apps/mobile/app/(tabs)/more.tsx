 ...
 * why that would be an outage rather than a feature while tokens omit the
 * claim (empty prod / playbook, not a disabled hook) — so it needs a door.
 *
 ...
 * !!chapterId`, and production tokens still
 * lack an `active_chapter_id` claim (empty prod until first onboard, or the
 * rollback playbook disabling the hook — not the hook being off) — so in that
 ...
 * but it does mean the section only appears once a token carries the claim
 * (a local stack with a membership, or prod after first onboard).
 *
