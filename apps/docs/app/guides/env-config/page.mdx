...
All staging and production secrets are centrally managed in **Infisical** and automatically synced to deployment providers (Vercel, Render, EAS).

<Callout type="info">
  For the complete list of every environment variable, per app, per environment,
  see <a href="https://github.com/pdcarlson/Frapp/blob/preview/docs/internal/ENV_REFERENCE.md"><code>docs/internal/ENV_REFERENCE.md</code></a>.
</Callout>
...
- **Provider-native syncs** — Infisical pushes secrets to Vercel, Render, and GitHub Actions automatically.
...
Then create `.env.local` files for each app using the values from the command output. See `docs/internal/ENV_REFERENCE.md` for the exact variables each app needs.
