...
<Callout type="info">
  The canonical description of the data model is in{" "}
  <code>spec/architecture.md</code> Section 5. Always update the spec before
  changing the schema.
</Callout>
...
<Callout type="warning">
  Never disable RLS in production. Local testing may temporarily relax policies,
  but staging and prod must always run with RLS enabled.
</Callout>
