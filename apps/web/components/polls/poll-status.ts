 ...
 * poll DTO carries `isExpired: boolean`, derived from `expires_at` by
 * `isPollClosed` in `@repo/validation`, and `spec/behavior/polls.md` describes
 * the state as a lock rather than a field. So this is §5's other branch, "a
