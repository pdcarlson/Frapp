 ...
 *
 * Content only: this is the "Saved" panel of the channel overflow menu
 * (`channel-menu.tsx`), which owns the one Popover, its trigger, its width and
 * its dismissal. Nothing here opens or closes anything.
 *
 * **Chapter-wide, not channel-scoped** — that is the one structural difference
 * from the sibling `PinsPanel` and it is what the spec asks for: "Bookmarked
              ...
              <li key={bookmark.id} className="relative">
                ...
                */}
                {available ? (
                  <button
                    type="button"
                    // Dismissing on jump is the host menu's job now — it closes
                    // the popover when this fires, for the reason the pins
                    // panel records: a 320px panel left open over the pane it
                ...
                {onRemove ? (
                  <Button
                    ...
                    // The deleted `thread-panel.tsx` used exactly this for the same
