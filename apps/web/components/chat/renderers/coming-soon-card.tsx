 ...
 * no card yet — `dues`, per the dispatcher in `./index.tsx`. The docstring used
 * to list `hours` too, and `/hours` now has `HoursCard`. It also used to list
 * `event`, `task` and `points`, and the copy still said "coming in Chunk 10";
 * those shipped their own renderers long ago, and chunks are not a unit
 * anything else in the repo still counts in.
 *
