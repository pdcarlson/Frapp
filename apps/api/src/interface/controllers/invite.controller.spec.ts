...
describe('InviteController', () => {
  ...
  describe('create', () => {
    it('should call inviteService.create with correct arguments', async () => {
      ...
      jest.spyOn(inviteService, 'create').mockResolvedValue(expectedResult as any);

      const result = await controller.create(chapterId, userId, dto);

      expect(inviteService.create).toHaveBeenCalledWith(chapterId, userId, dto.role);
  ...
  describe('createBatch', () => {
    it('should call inviteService.createBatch with correct arguments', async () => {
      ...
      const expectedResult = [{ token: 'invite-token-1' }, { token: 'invite-token-2' }];

      jest.spyOn(inviteService, 'createBatch').mockResolvedValue(expectedResult as any);

      const result = await controller.createBatch(chapterId, userId, dto);

      expect(inviteService.createBatch).toHaveBeenCalledWith(chapterId, userId, dto.role, dto.count);
      expect(result).toEqual(expectedResult);
    });
  });
  ...
  describe('redeem', () => {
    it('should call inviteService.redeem with correct arguments', async () => {
      ...
      jest.spyOn(inviteService, 'redeem').mockResolvedValue(expectedResult as any);
  ...
  describe('list', () => {
    it('should call inviteService.findByChapter with correct arguments', async () => {
      ...
      jest.spyOn(inviteService, 'findByChapter').mockResolvedValue(expectedResult as any);
