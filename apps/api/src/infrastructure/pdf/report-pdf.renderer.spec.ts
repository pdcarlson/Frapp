...
/** WinAnsi hex of the footer brand as pdf-lib encodes Helvetica `drawText`. */
const GENERATED_BY_SIGNET_HEX = '47656E657261746564206279205369676E6574';
const GENERATED_BY_FRAPP_HEX = '47656E657261746564206279204672617070';

/**
 * Inflate a page's content stream(s). Footer `drawText` is a PDFHexString
 * inside a FlateDecode stream, so a latin1 scan of the saved bytes cannot
 * see "Generated by Signet".
 */
function decodedPageContent(pdf: PDFDocument, pageIndex = 0): string {
  ...
  if (contents instanceof PDFRawStream) {
    ...
    for (let index = 0; index < contents.size(); index += 1) {
      const resolved = contents.lookup(index);
...
describe('ReportPdfRenderer', () => {
  ...
  it('names Signet as producer, creator, and visible footer brand', async () => {
    const bytes = await renderer.render(doc());
    // load() defaults updateMetadata: true, which overwrites Producer to
    // pdf-lib's own string before getProducer can read what we saved.
    const parsed = await PDFDocument.load(bytes, { updateMetadata: false });
    const content = decodedPageContent(parsed);

    expect(parsed.getProducer()).toBe('Signet');
    expect(parsed.getCreator()).toBe('Signet');
    expect(content).toContain(GENERATED_BY_SIGNET_HEX);
    expect(content).not.toContain(GENERATED_BY_FRAPP_HEX);
  });
