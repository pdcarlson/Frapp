...
describe('ReportPdfRenderer', () => {
  ...
  it('names Frapp as producer, creator, and visible footer brand', async () => {
    const bytes = await renderer.render(doc());
    ...
    expect(parsed.getProducer()).toBe('Frapp');
    expect(parsed.getCreator()).toBe('Frapp');
    expect(content).toContain(GENERATED_BY_FRAPP_HEX);
    expect(content).not.toContain(GENERATED_BY_SIGNET_HEX);
  });
