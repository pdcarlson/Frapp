...
export class ReportPdfRenderer implements IReportPdfRenderer {
  ...
  async render(document: ReportPdfDocument): Promise<Uint8Array> {
    ...
    const pageCount = Math.max(1, Math.ceil(document.rows.length / rowsPerPage));
  ...
  private drawHeader(
    ...
    page.drawText(
      fitText(toWinAnsi(document.branding.chapterName), fonts.bold, 15, textWidth),
      { x: textLeft, y: top - 13, size: 15, font: fonts.bold, color: INK },
    );
    page.drawText(
      fitText(toWinAnsi(document.branding.university), fonts.regular, 9.5, textWidth),
      { x: textLeft, y: top - 26, size: 9.5, font: fonts.regular, color: MUTED },
    );
    page.drawText(
    ...
    if (document.subtitle) {
      page.drawText(
        ...
        { x: textLeft, y: top - 58, size: 9, font: fonts.regular, color: MUTED },
      );
    }
  ...
  private drawFooter(
    ...
    page.drawText(
      toWinAnsi(`Generated by Frapp - ${document.generatedAt}`),
