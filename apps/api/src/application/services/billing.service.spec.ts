...
describe('BillingService', () => {
  ...
  describe('handleWebhookEvent', () => {
    ...
    it('should handle notification failure gracefully when president not found', async () => {
      const event: WebhookEvent = {
        id: 'evt_sub_update_no_pres',
        type: 'customer.subscription.updated',
        created: Date.now(),
        data: {
          object: {
      ...
      // No members with president role
      mockMemberRepo.findByChapter.mockResolvedValue([]);
    ...
    it('should map stripe incomplete_expired status to canceled', async () => {
      const event: WebhookEvent = {
      ...
      mockChapterRepo.findBySubscriptionId.mockResolvedValue(incompleteChapter);
      mockChapterRepo.update.mockResolvedValue({
        ...incompleteChapter,
        subscription_status: 'canceled',
      });
    ...
    it('should handle unknown stripe status gracefully', async () => {
      const event: WebhookEvent = {
    ...
    it('should handle invoice.paid when no subscription id is present', async () => {
      const event: WebhookEvent = {
      ...
      await expect(service.handleWebhookEvent(event)).resolves.not.toThrow();
      expect(mockChapterRepo.findBySubscriptionId).not.toHaveBeenCalled();
      expect(mockChapterRepo.update).not.toHaveBeenCalled();
    });
  });
});