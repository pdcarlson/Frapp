 ...
 * covers both terminal shapes the service uses — `.order()` (definitions) and
 * `.eq()` (values).
 */
function makeSupabase(opts: {
  ...
  const from = jest.fn((table: string) => {
    if (table === 'chapter_custom_fields') {
      return builder({ data: opts.defs ?? [], error: opts.defsError ?? null });
    }
...
describe('CustomFieldService', () => {
  ...
  it('lists chapter definitions', async () => {
    const defs = [
      { id: 'f1', key: 'gpa', label: 'GPA', visibility: 'chapter' },
    ];
    const { client } = makeSupabase({ defs });
  ...
  it('short-circuits to empty when no visibility tier is allowed', async () => {
    ...
    expect(result).toEqual([]);
    // No query issued when the allowed set is empty.
